...
static int g_vxlan_ports[VXLAN_MAX_PORTS] = {
    VXLAN_DEFAULT_PORT, VXLAN_UNSET_PORT, VXLAN_UNSET_PORT, VXLAN_UNSET_PORT};
...
bool DecodeVXLANEnabledForPort(const uint16_t sp, const uint16_t dp) {
  SCLogDebug("ports %u->%u ports %d %d %d %d", sp, dp, g_vxlan_ports[0],
             g_vxlan_ports[1], g_vxlan_ports[2], g_vxlan_ports[3]);

  if (g_vxlan_enabled) {
    for (int i = 0; i < g_vxlan_ports_idx; i++) {
      if (g_vxlan_ports[i] == VXLAN_UNSET_PORT) return false;
...
int DecodeVXLAN(ThreadVars *tv, DecodeThreadVars *dtv, Packet *p,
  ...
  if (unlikely(!g_vxlan_enabled)) return TM_ECODE_FAILED;

  if (len < (VXLAN_HEADER_LEN + sizeof(EthernetHdr))) return TM_ECODE_FAILED;
...
#if DEBUG
  uint32_t vni =
      (vxlanh->vni[0] << 16) + (vxlanh->vni[1] << 8) + (vxlanh->vni[2]);
  ...
  switch (eth_type) {
    ...
    default:
      SCLogDebug(
          "VXLAN found unsupported Ethertype - expected IPv4, IPv6, VLAN, or "
          "ARP");
  ...
  if (decode_tunnel_proto != DECODE_TUNNEL_UNSET) {
    Packet *tp = PacketTunnelPktSetup(
        tv, dtv, p, pkt + VXLAN_HEADER_LEN + ETHERNET_HEADER_LEN,
