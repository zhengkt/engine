 ...
 *
 * This implementation is based upon RFC 4380:
...
static int g_teredo_ports[TEREDO_MAX_PORTS] = {
    TEREDO_UNSET_PORT, TEREDO_UNSET_PORT, TEREDO_UNSET_PORT, TEREDO_UNSET_PORT};

bool DecodeTeredoEnabledForPort(const uint16_t sp, const uint16_t dp) {
  SCLogDebug("ports %u->%u ports %d %d %d %d", sp, dp, g_teredo_ports[0],
             g_teredo_ports[1], g_teredo_ports[2], g_teredo_ports[3]);
  ...
  if (g_teredo_enabled) {
    ...
    for (int i = 0; i < g_teredo_ports_cnt; i++) {
      if (g_teredo_ports[i] == TEREDO_UNSET_PORT) return false;
 ...
 *
 * \retval TM_ECODE_FAILED if packet is not a Teredo packet, TM_ECODE_OK if it
 * is
 */
int DecodeTeredo(ThreadVars *tv, DecodeThreadVars *dtv, Packet *p,
  ...
  if (!g_teredo_enabled) return TM_ECODE_FAILED;
  ...
  if (start[0] == 0x0) {
    ...
    if (start[1] == 0x0) {
      ...
      /* either authentication negotiation not real tunnel or invalid second
       * byte */
    } else {
      return TM_ECODE_FAILED;
    }
  ...
  if (IP_GET_RAW_VER(start) == 6) {
    ...
    if (IPV6_GET_RAW_HLIM(thdr) == 0) return TM_ECODE_FAILED;
