...
typedef struct FlowWorkerThreadData_ {
  ...
  /** Queue to put pseudo packets that have been created by the stream (RST
   * response) and by the flush logic following a protocol change. */
...
static void CheckWorkQueue(ThreadVars *tv, FlowWorkerThreadData *fw,
                           FlowTimeoutCounters *counters, FlowQueuePrivate *fq,
                           const uint32_t max_work) {
  ...
  while ((f = FlowQueuePrivateGetFromTop(fq)) != NULL) {
    ...
    if (max_work != 0 && ++i == max_work) break;
  }
  if (ret_queue.len > 0) {
    FlowSparePoolReturnFlows(&ret_queue);
  }
...
static TmEcode FlowWorkerThreadInit(ThreadVars *tv, const void *initdata,
  ...
  fw->cnt.flows_injected_max =
      StatsRegisterMaxCounter("flow.wrk.flows_injected_max", tv);
...
static inline void FlowWorkerStreamTCPUpdate(ThreadVars *tv,
  ...
  while ((x = PacketDequeueNoLock(&fw->pq))) {
    ...
    StreamTcpPruneSession(x->flow, x->flowflags & FLOW_PKT_TOSERVER
                                       ? STREAM_TOSERVER
                                       : STREAM_TOCLIENT);
    ...
    if (timeout) {
      ...
      /* to support IPS verdict logic, in the non-timeout case we need to do a
       * bit more */
...
static inline void FlowWorkerProcessLocalFlows(ThreadVars *tv,
  ...
  if (p->pkt_src == PKT_SRC_SHUTDOWN_FLUSH ||
      p->pkt_src == PKT_SRC_CAPTURE_TIMEOUT)
    max_work = 0;
  ...
  switch ((enum StreamUpdateDir)p->app_update_direction) {
    ...
    case UPDATE_DIR_PACKET:
      if (PKT_IS_TOSERVER(p)) {
        ...
        SCLogDebug("pcap_cnt %" PRIu64 ", FLOW_TS_APP_UPDATED set",
                   p->pcap_cnt);
      } else {
        p->flow->flags |= FLOW_TC_APP_UPDATED;
        SCLogDebug("pcap_cnt %" PRIu64 ", FLOW_TC_APP_UPDATED set",
                   p->pcap_cnt);
      }
      break;
...
static bool FlowWorkerIsBusy(ThreadVars *tv, void *flow_worker) {
  FlowWorkerThreadData *fw = flow_worker;
  if (fw->pq.len) return true;
  if (fw->fls.work_queue.len) return true;
