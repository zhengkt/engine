  ...
  while (len) {
    ...
    switch (ttype) {
      ...
      default:
        ...
        ENGINE_SET_INVALID_EVENT(p, IPV4_OPT_MALFORMED);
        /** \todo May not want to return error here on unknown tag type (at
         * least not for 3|4) */
        return -1;
    }
  }
...
static int DecodeIPV4Options(Packet *p, const uint8_t *pkt, uint16_t len,
  ...
  if (SCLogDebugEnabled()) {
    ...
    for (i = 0; i < len; i++) {
      offset += snprintf(buf + offset, (sizeof(buf) - offset), "%02" PRIx8 " ",
                         pkt[i]);
    }
  ...
  while (plen) {
    ...
    /* single byte options */
    if (*pkt == IPV4_OPT_EOL) {
      /** \todo What if more data exist after EOL (possible covert channel or
       * data leakage)? */
      ...
      if (unlikely(plen < 2)) {
        /** \todo What if padding is non-zero (possible covert channel or data
         * leakage)? */
      ...
      switch (opt.type) {
        ...
        case IPV4_OPT_RTRALT:
          ...
          break;
        default:
          SCLogDebug("IPV4OPT <unknown> (%" PRIu8 ") len %" PRIu8, opt.type,
                     opt.len);
...
#ifdef DEBUG
  if (SCLogDebugEnabled()) { /* only convert the addresses if debug is really
                                enabled */
    /* debug print */
    char s[16], d[16];
    ...
    SCLogDebug("IPV4 %s->%s PROTO: %" PRIu32 " OFFSET: %" PRIu32 " RF: %" PRIu32
               " DF: %" PRIu32 " MF: %" PRIu32 " ID: %" PRIu32 "",
               s, d, IPV4_GET_IPPROTO(p), IPV4_GET_IPOFFSET(p), IPV4_GET_RF(p),
...
static int DecodeIPV4OptionsRRTest01(void) {
  uint8_t raw_opts[] = {IPV4_OPT_RR, 0x27, 0x08, 0xc0, 0xa8, 0x2a, 0x64, 0x00,
...
static int DecodeIPV4OptionsRRTest02(void) {
  uint8_t raw_opts[] = {IPV4_OPT_RR, 0xff, 0x08, 0xc0, 0xa8, 0x2a, 0x64, 0x00,
                        0x00,        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                        0x00,        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                        0x00,        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                        0x00,        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
...
static int DecodeIPV4OptionsRRTest03(void) {
  uint8_t raw_opts[] = {IPV4_OPT_RR, 0x27, 0xff, 0xc0, 0xa8, 0x2a, 0x64, 0x00,
                        0x00,        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                        0x00,        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                        0x00,        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                        0x00,        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
...
static int DecodeIPV4OptionsRRTest04(void) {
  uint8_t raw_opts[] = {IPV4_OPT_RR, 0x27, 0x05, 0xc0, 0xa8, 0x2a, 0x64, 0x00,
                        0x00,        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                        0x00,        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                        0x00,        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                        0x00,        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
...
static int DecodeIPV4OptionsTSTest01(void) {
  uint8_t raw_opts[] = {IPV4_OPT_TS, 0x24, 0x0d, 0x01, 0x0a, 0x0a, 0x0a, 0x69,
...
static int DecodeIPV4OptionsTSTest02(void) {
  uint8_t raw_opts[] = {IPV4_OPT_TS, 0x24, 0x04, 0x01, 0x0a, 0x0a, 0x0a, 0x69,
                        0x04,        0xce, 0x0d, 0x00, 0x00, 0x00, 0x00, 0x00,
                        0x00,        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                        0x00,        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                        0x00,        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
...
static int DecodeIPV4OptionsTSTest03(void) {
  uint8_t raw_opts[] = {IPV4_OPT_TS, 0x24, 0xff, 0x01, 0x0a, 0x0a, 0x0a, 0x69,
                        0x04,        0xce, 0x0d, 0x00, 0x00, 0x00, 0x00, 0x00,
                        0x00,        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                        0x00,        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                        0x00,        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
...
static int DecodeIPV4OptionsTSTest04(void) {
  uint8_t raw_opts[] = {IPV4_OPT_TS, 0x24, 0x0a, 0x01, 0x0a, 0x0a, 0x0a, 0x69,
                        0x04,        0xce, 0x0d, 0x00, 0x00, 0x00, 0x00, 0x00,
                        0x00,        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                        0x00,        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                        0x00,        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
...
static int DecodeIPV4OptionsSECTest01(void) {
  uint8_t raw_opts[] = {IPV4_OPT_SEC, 0x0b, 0xf1, 0x35, 0x00, 0x00,
...
static int DecodeIPV4OptionsSECTest02(void) {
  uint8_t raw_opts[] = {IPV4_OPT_SEC, 0x02, 0xf1, 0x35, 0x00, 0x00, 0x00, 0x00,
                        0x00,         0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
  Packet *p = PacketGetFromAlloc();
  FAIL_IF(unlikely(p == NULL));
...
static int DecodeIPV4OptionsESECTest01(void) {
  uint8_t raw_opts[] = {IPV4_OPT_ESEC, 0x0b, 0xf1, 0x35, 0x00, 0x00,
                        0x00,          0x00, 0x00, 0x00, 0x00, 0x00,
                        0x00,          0x00, 0x00, 0x00, 0x00};
  Packet *p = PacketGetFromAlloc();
  FAIL_IF(unlikely(p == NULL));
...
static int DecodeIPV4OptionsESECTest02(void) {
  uint8_t raw_opts[] = {IPV4_OPT_ESEC, 0x02, 0xf1, 0x35, 0x00, 0x00,
                        0x00,          0x00, 0x00, 0x00, 0x00, 0x00,
                        0x00,          0x00, 0x00, 0x00};
  Packet *p = PacketGetFromAlloc();
  FAIL_IF(unlikely(p == NULL));
...
static int DecodeIPV4OptionsLSRRTest01(void) {
  uint8_t raw_opts[] = {
      IPV4_OPT_LSRR, 0x27, 0x08, 0xc0, 0xa8, 0x2a, 0x64, 0x00, 0x00, 0x00,
...
static int DecodeIPV4OptionsLSRRTest02(void) {
  uint8_t raw_opts[] = {
      IPV4_OPT_LSRR, 0xff, 0x08, 0xc0, 0xa8, 0x2a, 0x64, 0x00, 0x00, 0x00,
      0x00,          0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00,          0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00,          0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
  Packet *p = PacketGetFromAlloc();
...
static int DecodeIPV4OptionsLSRRTest03(void) {
  uint8_t raw_opts[] = {
      IPV4_OPT_LSRR, 0x27, 0xff, 0xc0, 0xa8, 0x2a, 0x64, 0x00, 0x00, 0x00,
      0x00,          0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00,          0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00,          0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
  Packet *p = PacketGetFromAlloc();
...
static int DecodeIPV4OptionsLSRRTest04(void) {
  uint8_t raw_opts[] = {
      IPV4_OPT_LSRR, 0x27, 0x05, 0xc0, 0xa8, 0x2a, 0x64, 0x00, 0x00, 0x00,
      0x00,          0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00,          0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00,          0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
  Packet *p = PacketGetFromAlloc();
...
static int DecodeIPV4OptionsCIPSOTest01(void) {
  uint8_t raw_opts[] = {IPV4_OPT_CIPSO, 0x18, 0x00, 0x00, 0x00, 0x05,
                        0x05,           0x12, 0x00, 0x03, 0x00, 0xef,
                        0x00,           0xef, 0x00, 0x06, 0x00, 0x04,
...
static int DecodeIPV4OptionsSSRRTest01(void) {
  uint8_t raw_opts[] = {
      IPV4_OPT_SSRR, 0x27, 0x08, 0xc0, 0xa8, 0x2a, 0x64, 0x00, 0x00, 0x00,
      0x00,          0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00,          0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00,          0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
  Packet *p = PacketGetFromAlloc();
...
static int DecodeIPV4OptionsSSRRTest02(void) {
  uint8_t raw_opts[] = {
      IPV4_OPT_SSRR, 0xff, 0x08, 0xc0, 0xa8, 0x2a, 0x64, 0x00, 0x00, 0x00,
      0x00,          0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00,          0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00,          0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
  Packet *p = PacketGetFromAlloc();
...
static int DecodeIPV4OptionsSSRRTest03(void) {
  uint8_t raw_opts[] = {
      IPV4_OPT_SSRR, 0x27, 0xff, 0xc0, 0xa8, 0x2a, 0x64, 0x00, 0x00, 0x00,
      0x00,          0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00,          0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00,          0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
  Packet *p = PacketGetFromAlloc();
...
static int DecodeIPV4OptionsSSRRTest04(void) {
  uint8_t raw_opts[] = {
      IPV4_OPT_SSRR, 0x27, 0x05, 0xc0, 0xa8, 0x2a, 0x64, 0x00, 0x00, 0x00,
      0x00,          0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00,          0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00,          0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
  Packet *p = PacketGetFromAlloc();
...
static int IPV4CalculateValidChecksumtest01(void) {
  ...
  uint8_t raw_ipv4[] = {0x45, 0x00, 0x00, 0x54, 0x00, 0x00, 0x40,
                        0x00, 0x40, 0x01, 0xb7, 0x52, 0xc0, 0xa8,
                        0x01, 0x03, 0xc0, 0xa8, 0x01, 0x03};
...
static int DecodeIPV4DefragTest01(void) {
  uint8_t pkt1[] = {0x00, 0x50, 0x56, 0x00, 0x03, 0x05, 0xde, 0xad, 0x01,
                    0xa3, 0xa2, 0x2f, 0x08, 0x00, 0x45, 0x00, 0x00, 0x1c,
                    0xe9, 0xef, 0x20, 0x00, 0x40, 0x06, 0x9a, 0xc8, 0x0a,
                    0x00, 0xe1, 0x17, 0x0a, 0x00, 0xe1, 0x0c, 0x6e, 0x12,
                    0x01, 0xbd, 0x5b, 0xa3, 0x81, 0x5e};
  uint8_t pkt2[] = {0x00, 0x50, 0x56, 0x00, 0x03, 0x05, 0xde, 0xad, 0x01,
                    0xa3, 0xa2, 0x2f, 0x08, 0x00, 0x45, 0x00, 0x00, 0x1c,
                    0xe9, 0xef, 0x20, 0x01, 0x40, 0x06, 0x9a, 0xc7, 0x0a,
                    0x00, 0xe1, 0x17, 0x0a, 0x00, 0xe1, 0x0c, 0xac, 0xb0,
                    0xae, 0x8a, 0x50, 0x10, 0x80, 0x00};
  uint8_t pkt3[] = {0x00, 0x50, 0x56, 0x00, 0x03, 0x05, 0xde, 0xad, 0x01, 0xa3,
                    0xa2, 0x2f, 0x08, 0x00, 0x45, 0x00, 0x00, 0x18, 0xe9, 0xef,
                    0x00, 0x02, 0x40, 0x06, 0xba, 0xca, 0x0a, 0x00, 0xe1, 0x17,
                    0x0a, 0x00, 0xe1, 0x0c, 0xb1, 0xa3, 0x00, 0x00};
  uint8_t tunnel_pkt[] = {0x00, 0x50, 0x56, 0x00, 0x03, 0x05, 0xde, 0xad, 0x01,
                          0xa3, 0xa2, 0x2f, 0x08, 0x00, 0x45, 0x00, 0x00, 0x28,
                          0xe9, 0xef, 0x00, 0x00, 0x40, 0x06, 0xba, 0xbc, 0x0a,
                          0x00, 0xe1, 0x17, 0x0a, 0x00, 0xe1, 0x0c, 0x6e, 0x12,
                          0x01, 0xbd, 0x5b, 0xa3, 0x81, 0x5e, 0xac, 0xb0, 0xae,
                          0x8a, 0x50, 0x10, 0x80, 0x00, 0xb1, 0xa3, 0x00, 0x00};
...
static int DecodeIPV4DefragTest02(void) {
  ...
  uint8_t pkt2[] = {
      0x00, 0x50, 0x56, 0x00, 0x03, 0x05, 0xde, 0xad, 0x01, 0xa3, 0xa2, 0x2f,
      0x08, 0x00, 0x45, 0x00, 0x00, 0x2c, 0xe9, 0xef, 0x20, 0x02, 0x40, 0x06,
      0xba, 0xca, 0x0a, 0x00, 0xe1, 0x17, 0x0a, 0x00, 0xe1, 0x0c,
      /* second frag */
      0xb1, 0xa3, 0x00, 0x10, 0x5b, 0xa3, 0x81, 0x5e, 0xac, 0xb0, 0xae, 0x8a,
      0x50, 0x10, 0x80, 0x00, 0xb1, 0xa3, 0x00, 0x10, 0x01, 0x02, 0x03, 0x04};
  uint8_t pkt3[] = {
      0x00,
      0x50,
      0x56,
  ...
  uint8_t tunnel_pkt[] = {
      0x00, 0x50, 0x56, 0x00, 0x03, 0x05, 0xde, 0xad, 0x01, 0xa3, 0xa2,
      0x2f, 0x08, 0x00, 0x45, 0x00, 0x00, 0x3e, 0xe9, 0xef, 0x00, 0x00,
      0x40, 0x06, 0xba, 0xae, 0x0a, 0x00, 0xe1, 0x17, 0x0a, 0x00, 0xe1,
      0x0c, 0x6e, 0x12, 0x01, 0xbd, 0x5b, 0xa3, 0x81, 0x5e, 0xac, 0xb0,
      0xae, 0x8a, 0x50, 0x10, 0x80, 0x00, 0xb1, 0xa3, 0x00, 0x10, 0x5b,
      0xa3, 0x81, 0x5e, 0xac, 0xb0, 0xae, 0x8a, 0x50, 0x10, 0x80, 0x00,
      0xb1, 0xa3, 0x00, 0x10, 0x01, 0x02, 0x03, 0x04, 0xb1, 0xa3,
  };
...
static int DecodeIPV4DefragTest03(void) {
  uint8_t pkt[] = {0x00, 0x50, 0x56, 0x00, 0x03, 0x05, 0xde, 0xad, 0x01,
                   0xa3, 0xa2, 0x2f, 0x08, 0x00, 0x45, 0x00, 0x00, 0x28,
                   0xe9, 0xee, 0x00, 0x00, 0x40, 0x06, 0xba, 0xbd, 0x0a,
                   0x00, 0xe1, 0x17, 0x0a, 0x00, 0xe1, 0x0c, 0x6e, 0x12,
                   0x01, 0xbd, 0x5b, 0xa3, 0x81, 0x5d, 0x00, 0x00, 0x00,
                   0x00, 0x50, 0x02, 0x80, 0x00, 0x0c, 0xee, 0x00, 0x00};
  uint8_t pkt1[] = {0x00, 0x50, 0x56, 0x00, 0x03, 0x05, 0xde, 0xad, 0x01,
                    0xa3, 0xa2, 0x2f, 0x08, 0x00, 0x45, 0x00, 0x00, 0x1c,
                    0xe9, 0xef, 0x20, 0x00, 0x40, 0x06, 0x9a, 0xc8, 0x0a,
                    0x00, 0xe1, 0x17, 0x0a, 0x00, 0xe1, 0x0c, 0x6e, 0x12,
...
static int DecodeEthernetTestIPv4Opt(void) {
  uint8_t raw_eth[] = {
      0xae, 0x71, 0x00, 0x00, 0x00, 0x4b, 0x06, 0x90, 0x61, 0x02, 0x00, 0xcd,
      0x88, 0x64, 0x11, 0x00, 0x15, 0x00, 0x80, 0x64, 0x00, 0x21, 0x4c, 0x00,
      0x00, 0x30, 0x42, 0xd6, 0xff, 0xff, 0xbd, 0x2f, 0x02, 0x02, 0x00, 0x02,
      0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02,
      0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02,
      0x02, 0x02, 0x02, 0x02, 0x01, 0x44, 0x05, 0x22, 0x02, 0x01};
