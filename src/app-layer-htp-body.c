...
void HtpBodyPrune(HtpState *state, HtpBody *body, int direction) {
  ...
  const HTPCfgDir *cfg = (direction == STREAM_TOCLIENT) ? &state->cfg->response
                                                        : &state->cfg->request;
  ...
  if (left_edge <= min_size || left_edge <= window) left_edge = 0;
  if (left_edge) left_edge -= window;
