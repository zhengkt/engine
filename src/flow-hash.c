 ...
 */
static inline int FlowHashRawAddressIPv6GtU32(const uint32_t *a,
                                              const uint32_t *b) {
  for (int i = 0; i < 4; i++) {
    if (a[i] > b[i]) return 1;
    if (a[i] < b[i]) break;
...
uint32_t FlowKeyGetHash(FlowKey *fk) {
  ...
  if (fk->src.family == AF_INET) {
    ...
    int ai = (fk->src.address.address_un_data32[0] >
              fk->dst.address.address_un_data32[0]);
...
static inline bool CmpAddrsAndPorts(const uint32_t src1[4],
                                    const uint32_t dst1[4], Port src_port1,
                                    Port dst_port1, const uint32_t src2[4],
                                    const uint32_t dst2[4], Port src_port2,
                                    Port dst_port2) {
...
static inline bool CmpVlanIds(const uint16_t vlan_id1[VLAN_MAX_LAYERS],
                              const uint16_t vlan_id2[VLAN_MAX_LAYERS]) {
...
static inline bool CmpAddrsAndICMPTypes(
    const uint32_t src1[4], const uint32_t dst1[4], uint8_t icmp_s_type1,
    uint8_t icmp_d_type1, const uint32_t src2[4], const uint32_t dst2[4],
...
static inline bool CmpFlowICMPPacket(const Flow *f, const Packet *p) {
  ...
  const uint32_t *p_src = p->src.address.address_un_data32;
  const uint32_t *p_dst = p->dst.address.address_un_data32;
  return CmpAddrsAndICMPTypes(f_src, f_dst, f->icmp_s.type, f->icmp_d.type,
                              p_src, p_dst, p->icmp_s.type, p->icmp_d.type) &&
         f->proto == p->proto && f->recursion_level == p->recursion_level &&
         CmpVlanIds(f->vlan_id, p->vlan_id) &&
         (f->livedev == p->livedev || g_livedev_mask == 0);
}
...
static inline int FlowCreateCheck(const Packet *p, const bool emerg) {
  ...
  if (emerg) {
    if (PKT_IS_TCP(p)) {
      if (((p->tcph->th_flags & (TH_SYN | TH_ACK | TH_RST | TH_FIN)) ==
           TH_SYN) ||
...
static inline Flow *FlowSpareSync(ThreadVars *tv, FlowLookupStruct *fls,
  ...
  if (emerg) {
    if ((uint32_t)SCTIME_SECS(p->ts) > fls->emerg_spare_sync_stamp) {
      fls->spare_queue =
          FlowSpareGetFromPool(); /* local empty, (re)populate and try again */
      spare_sync = true;
  ...
  if (tv && fls->dtv) {
    ...
    if (spare_sync) {
      if (f != NULL) {
        StatsAddUI64(tv, fls->dtv->counter_flow_spare_sync_avg,
...
static inline void NoFlowHandleIPS(Packet *p) {
  ExceptionPolicyApply(p, flow_config.memcap_policy,
...
static Flow *TcpReuseReplace(ThreadVars *tv, FlowLookupStruct *fls,
                             FlowBucket *fb, Flow *old_f, const uint32_t hash,
                             Packet *p) {
#ifdef UNITTESTS
  if (tv != NULL && fls->dtv != NULL) {
#endif
...
static inline void MoveToWorkQueue(ThreadVars *tv, FlowLookupStruct *fls,
  ...
  if (f->proto != IPPROTO_TCP ||
      FlowBelongsToUs(tv, f)) {  // TODO thread_id[] direction
    f->fb = NULL;
    f->next = NULL;
...
static inline bool FlowIsTimedOut(const Flow *f, const uint32_t sec,
                                  const bool emerg) {
  if (unlikely(f->timeout_at < sec)) {
    ...
    int64_t timeout_at =
        f->timeout_at - FlowGetFlowTimeoutDirect(flow_timeouts_delta,
                                                 f->flow_state, f->protomap);
    if ((int64_t)sec >= timeout_at) return true;
  }
  return false;
}
 ...
 */
Flow *FlowGetFlowFromHash(ThreadVars *tv, FlowLookupStruct *fls, Packet *p,
                          Flow **dest) {
  Flow *f = NULL;
  ...
  do {
    Flow *next_f = NULL;
    const bool timedout =
        (fb_nextts < (uint32_t)SCTIME_SECS(p->ts) &&
    ...
    if (timedout) {
      ...
      if (unlikely(TcpSessionPacketSsnReuse(p, f, f->protoctx) == 1)) {
        ...
        if (prev_f ==
            NULL) /* if we have no prev it means new_f is now our prev */
          prev_f = new_f;
...
static inline bool FlowCompareKey(Flow *f, FlowKey *key) {
  if ((f->proto != IPPROTO_TCP) && (f->proto != IPPROTO_UDP)) return false;
...
Flow *FlowGetFromFlowKey(FlowKey *key, struct timespec *ttime,
                         const uint32_t hash) {
...
static Flow *FlowGetUsedFlow(ThreadVars *tv, DecodeThreadVars *dtv,
                             const SCTime_t ts) {
  uint32_t idx =
      GetUsedAtomicUpdate(FLOW_GET_NEW_TRIES) % flow_config.hash_size;
  uint32_t tried = 0;
  ...
  while (1) {
    ...
    if (++idx >= flow_config.hash_size) idx = 0;

    FlowBucket *fb = &flow_hash[idx];

    if (SC_ATOMIC_GET(fb->next_ts) == UINT_MAX) continue;
