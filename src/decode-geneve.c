...
#define GENEVE_TOTAL_OPT_LEN(hdr_ptr) \
  ((uint8_t)((hdr_ptr->ver_plus_len & 0x3F) << 2))
...
#define GENEVE_SINGLE_OPT_LEN(option_ptr) \
  ((uint8_t)((option_ptr->flags_plus_len & 0x1F) << 2))
...
static int g_geneve_ports[GENEVE_MAX_PORTS] = {
    GENEVE_DEFAULT_PORT, GENEVE_UNSET_PORT, GENEVE_UNSET_PORT,
    GENEVE_UNSET_PORT};
...
bool DecodeGeneveEnabledForPort(const uint16_t sp, const uint16_t dp) {
  SCLogDebug("ports %u->%u ports %d %d %d %d", sp, dp, g_geneve_ports[0],
             g_geneve_ports[1], g_geneve_ports[2], g_geneve_ports[3]);

  if (g_geneve_enabled) {
    for (int i = 0; i < g_geneve_ports_idx; i++) {
      if (g_geneve_ports[i] == GENEVE_UNSET_PORT) return false;

      const int port = g_geneve_ports[i];
      if (port == (const int)sp || port == (const int)dp) return true;
    }
  }
  return false;
}
...
static inline bool IsValidGeneveVersion(const GeneveHeader *geneve_hdr) {
  ...
  for (int i = 0; i < num_versions; i++) {
    if (valid_versions[i] == cur_version) return true;
  }

  return false;
}

/* Performs a check to ensure that option lens add up to total length specified
 * in the fixed header
 */
static inline bool IsHeaderLengthConsistentWithOptions(
  ...
  while (remaining_hdr_len > 0) {
    ...
    remaining_hdr_len -= cur_option_len; /* cur_option_len will always be
                                            between 4-128, inclusive */
  }
...
int DecodeGeneve(ThreadVars *tv, DecodeThreadVars *dtv, Packet *p,
  ...
  if (unlikely(!g_geneve_enabled)) return TM_ECODE_FAILED;

  if (unlikely(len < GENEVE_MIN_HEADER_LEN)) return TM_ECODE_FAILED;
  ...
  if (len < geneve_hdr_len) return TM_ECODE_FAILED;

  if (!IsValidGeneveVersion(geneve_hdr)) return TM_ECODE_FAILED;

  if (GENEVE_RESERVED_FLAGS(geneve_hdr) != 0 || geneve_hdr->res != 0)
    return TM_ECODE_FAILED;

  if (!IsHeaderLengthConsistentWithOptions(geneve_hdr)) return TM_ECODE_FAILED;

#if DEBUG
  /* Print the VNI for debugging purposes */
  uint32_t vni = (geneve_hdr->vni[0] << 16) + (geneve_hdr->vni[1] << 8) +
                 (geneve_hdr->vni[2]);
  ...
  switch (eth_type) {
    ...
    default:
      SCLogDebug(
          "Geneve found unsupported Ethertype - expected IPv4, IPv6, ARP, or "
          "Ethernet");
  ...
  if (decode_tunnel_proto != DECODE_TUNNEL_UNSET) {
    Packet *tp =
        PacketTunnelPktSetup(tv, dtv, p, pkt + geneve_hdr_len,
...
static int DecodeGeneveTest01(void) {
  uint8_t raw_geneve[] = {
      ...
      0xfe, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x0a, 0x00, 0x27, 0xff,
      0xfe, 0xfe, 0x8f, 0x95, 0xff, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00, 0x02, 0x02, 0x22, 0x02, 0x23,
      0x00, 0x6b, 0x9c, 0xfb,                         /* UDP src port 546 */
      0x03, 0x49, 0x17, 0x4e, 0x00, 0x01, 0x00, 0x0e, /* DHCP request payload */
      0x00, 0x01, 0x00, 0x01, 0x1c, 0x39, 0xcf, 0x88, 0x08, 0x00, 0x27, 0xfe,
      0x8f, 0x95, 0x00, 0x02, 0x00, 0x0e, 0x00, 0x01, 0x00, 0x01, 0x1c, 0x38,
      0x25, 0xe8, 0x08, 0x00, 0x27, 0xd4, 0x10, 0xbb, 0x00, 0x06, 0x00, 0x04,
      0x00, 0x17, 0x00, 0x18, 0x00, 0x08, 0x00, 0x02, 0x00, 0x00, 0x00, 0x19,
      0x00, 0x29, 0x27, 0xfe, 0x8f, 0x95, 0x00, 0x00, 0x0e, 0x10, 0x00, 0x00,
      0x15, 0x18, 0x00, 0x1a, 0x00, 0x19, 0x00, 0x00, 0x1c, 0x20, 0x00, 0x00,
      0x1d, 0x4c, 0x40, 0x20, 0x01, 0x00, 0x00, 0x00, 0x00, 0xfe, 0x00, 0x00,
...
static int DecodeGeneveTest04(void) {
  ...
  DecodeGeneveConfigPorts(
      "1"); /* Set Suricata to use a non-default port for Geneve*/
  ...
  FAIL_IF(p->udph == NULL);
  FAIL_IF(tv.decode_pq.top !=
          NULL); /* Geneve packet should not have been processed */

  DecodeGeneveConfigPorts(
      GENEVE_DEFAULT_PORT_S); /* Reset Geneve port list for future calls */
  FlowShutdown();
  PacketFree(p);
  PASS;
}
...
/**
 * \test DecodeGeneveTest05 tests if Geneve header has inconsistent option len
 * values.
 */
