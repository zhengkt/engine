...
static int StatsThreadRegister(const char *thread_name,
...
bool StatsEnabled(void) { return stats_enabled; }
...
static void StatsInitCtxPreOutput(void) {
  ...
  if (stats != NULL) {
    ...
    if (gstats == NULL) {
      SCLogWarning(
          "global stats config is missing. "
...
static void StatsReleaseCtx(void) {
  ...
  while (sts != NULL) {
    if (sts->head != NULL) SCFree(sts->head);
...
static void *StatsMgmtThread(void *arg) {
  ...
  if (tv_local->thread_setup_flags != 0) TmThreadSetupOptions(tv_local);
...
static uint16_t StatsRegisterQualifiedCounter(const char *name,
  ...
  if ((pc = SCMalloc(sizeof(StatsCounter))) == NULL) return 0;
 ...
 *        StatsPrivateThreadContext to its corresponding global counterpart.
 * Used internally by StatsUpdateCounterArray()
 *
 * \param pcae     Pointer to the StatsPrivateThreadContext which holds the
 * local versions of the counters
 */
...
static int StatsOutput(ThreadVars *tv) {
  ...
  if (counters_global_id == 0) return -1;
    ...
    while (pc != NULL) {
      ...
      switch (pc->type) {
        case STATS_TYPE_FUNC:
          if (pc->Func != NULL) thread_table[pc->gid].value = pc->Func();
          break;
    ...
    for (uint16_t c = 0; c < max_id; c++) {
      ...
      switch (e->type) {
        case STATS_TYPE_MAXIMUM:
          if (e->value > merge_table[c].value) merge_table[c].value = e->value;
          break;
        case STATS_TYPE_FUNC:
  ...
  for (uint16_t x = 0; x < max_id; x++) {
    ...
    switch (m->type) {
      case STATS_TYPE_MAXIMUM:
        if (m->value > table[x].value) table[x].value = m->value;
        break;
      case STATS_TYPE_AVERAGE:
...
TmEcode StatsOutputCounterSocket(json_t *cmd, json_t *answer, void *data) {
  ...
  if (!stats_enabled) {
    ...
    if (stats_table.start_time == 0) {
      ...
      message =
          StatsToJSON(&stats_table, JSON_STATS_TOTALS | JSON_STATS_THREADS);
    }
    SCMutexUnlock(&stats_table_mutex);
  }
...
static void StatsLogSummary(void) {
  ...
  if (stats_table.start_time != 0) {
    ...
    for (uint32_t u = 0; u < st->nstats; u++) {
      ...
      if (name == NULL || strcmp(name, "detect.alert") != 0) continue;
...
void StatsSetupPostConfigPreOutput(void) { StatsInitCtxPreOutput(); }

void StatsSetupPostConfigPostOutput(void) { StatsInitCtxPostOutput(); }
...
void StatsSpawnThreads(void) {
  ...
  if (TmThreadSpawn(tv_wakeup) != 0) {
    FatalError(
        "TmThreadSpawn failed for "
        "StatsWakeupThread");
  }

  /* spawn the stats mgmt thread */
  tv_mgmt =
      TmThreadCreateMgmtThread(thread_name_counter_stats, StatsMgmtThread, 1);
  if (tv_mgmt == NULL) {
...
uint16_t StatsRegisterCounter(const char *name, struct ThreadVars_ *tv) {
  uint16_t id = StatsRegisterQualifiedCounter(
      name,
      (tv->thread_group_name != NULL) ? tv->thread_group_name
                                      : tv->printable_name,
...
static uint32_t CountersIdHashFunc(HashTable *ht, void *data,
                                   uint16_t datalen) {
  CountersIdType *t = (CountersIdType *)data;
  uint32_t hash = 0;
  int len = strlen(t->string);

  for (int i = 0; i < len; i++) hash += u8_tolower((unsigned char)t->string[i]);
...
static void CountersIdHashFreeFunc(void *data) { SCFree(data); }
...
static int StatsThreadRegister(const char *thread_name,
  ...
  if (stats_ctx->counters_id_hash == NULL) {
    stats_ctx->counters_id_hash =
        HashTableInit(256, CountersIdHashFunc, CountersIdHashCompareFunc,
...
static int StatsGetCounterArrayRange(uint16_t s_id, uint16_t e_id,
  ...
  if ((pca->head = SCMalloc(sizeof(StatsLocalCounter) * (e_id - s_id + 2))) ==
      NULL) {
    return -1;
  }
...
static int StatsGetAllCountersArray(StatsPublicThreadContext *pctx,
  ...
  if (pctx == NULL || private == NULL) return -1;
 ...
 * \param pctx    StatsPublicThreadContext corresponding to the tm_name key
 * under which the key has to be registered
 *
 * \retval id Counter id for the newly registered counter, or the already
 *            present counter
 */
