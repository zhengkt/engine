...
static void AlertDebugLogPktVars(AlertDebugLogThread *aft, const Packet *p) {
  ...
  while (pv != NULL) {
    ...
    PrintRawDataToBuffer(aft->buffer->buffer, &aft->buffer->offset,
                         aft->buffer->size, pv->value, pv->value_len);
    pv = pv->next;
  }
}
...
static int AlertDebugPrintStreamSegmentCallback(const Packet *p,
                                                TcpSegment *seg, void *data,
                                                const uint8_t *buf,
                                                uint32_t buflen) {
  ...
  PrintRawDataToBuffer(aft->buffer->buffer, &aft->buffer->offset,
                       aft->buffer->size, buf, buflen);

  return 1;
}

static TmEcode AlertDebugLogger(ThreadVars *tv, const Packet *p,
                                void *thread_data) {
  ...
  if (p->alerts.cnt == 0) return TM_ECODE_OK;
  ...
  if (p->flow != NULL) {
    ...
    MemBufferWriteString(
        ...
        applayer ? "TRUE" : "FALSE",
        (p->flow->alproto != ALPROTO_UNKNOWN) ? "TRUE" : "FALSE",
        p->flow->alproto);
  ...
  MemBufferWriteString(aft->buffer,
                       ...
                       GET_PKT_LEN(p));
  PrintRawDataToBuffer(aft->buffer->buffer, &aft->buffer->offset,
                       aft->buffer->size, GET_PKT_DATA(p), GET_PKT_LEN(p));
  ...
  for (i = 0; i < p->alerts.cnt; i++) {
    ...
    if (p->payload_len > 0) {
      MemBufferWriteString(aft->buffer,
                           ...
                           p->payload_len);
      PrintRawDataToBuffer(aft->buffer->buffer, &aft->buffer->offset,
                           aft->buffer->size, p->payload, p->payload_len);
    }
...
static TmEcode AlertDebugLogDecoderEvent(ThreadVars *tv, const Packet *p,
                                         void *thread_data) {
  AlertDebugLogThread *aft = (AlertDebugLogThread *)thread_data;
  int i;
  char timebuf[64];
  ...
  for (i = 0; i < p->alerts.cnt; i++) {
    ...
    MemBufferWriteString(aft->buffer,
                         ...
                         "ALERT CLASS [%02d]:  %s\n"
                         "ALERT PRIO [%02d]:   %" PRIu32 "\n",
                         i, pa->s->msg, i, pa->s->gid, i, pa->s->id, i,
                         pa->s->rev, i, pa->s->class_msg, i, pa->s->prio);
  }
...
static OutputInitResult AlertDebugLogInitCtx(ConfNode *conf) {
  ...
  if (unlikely(output_ctx == NULL)) goto error;
...
static int AlertDebugLogCondition(ThreadVars *tv, void *thread_data,
                                  const Packet *p) {
  return (p->alerts.cnt ? TRUE : FALSE);
}

static int AlertDebugLogLogger(ThreadVars *tv, void *thread_data,
                               const Packet *p) {
...
void AlertDebugLogRegister(void) {
  OutputRegisterPacketModule(LOGGER_ALERT_DEBUG, MODULE_NAME, "alert-debug",
                             AlertDebugLogInitCtx, AlertDebugLogLogger,
                             AlertDebugLogCondition, AlertDebugLogThreadInit,
