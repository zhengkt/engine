...
static void DefragFragReset(Frag *frag) {
  if (frag->pkt != NULL) SCFree(frag->pkt);
...
static DefragContext *DefragContextNew(void) {
  ...
  if (unlikely(dc == NULL)) return NULL;
  ...
  dc->frag_pool = PoolInit(frag_pool_size, frag_pool_prealloc, sizeof(Frag),
  ...
  SCLogDebug("\tPreallocated fragments: %" PRIuMAX,
 ...
 */
static Packet *Defrag4Reassemble(ThreadVars *tv, DefragTracker *tracker,
                                 Packet *p) {
  Packet *rp = NULL;
  ...
  RB_FOREACH(frag, IP_FRAGMENTS, &tracker->fragment_tree) {
    SCLogDebug("frag %p, data_len %u, offset %u, pcap_cnt %" PRIu64, frag,
               frag->data_len, frag->offset, frag->pcap_cnt);

    if (frag->skip) continue;
    if (frag->ltrim >= frag->data_len) continue;
    if (frag->offset == 0) {
  ...
  rp->ip4h->ip_off = 0;
  rp->ip4h->ip_csum =
      FixChecksum(rp->ip4h->ip_csum, old, rp->ip4h->ip_len + rp->ip4h->ip_off);
...
error_remove_tracker:
  tracker->remove = 1;
  DefragTrackerFreeFrags(tracker);
  if (rp != NULL) PacketFreeOrRelease(rp);
  return NULL;
}
 ...
 * \param tracker The defragmentation tracker to reassemble from.
 */
static Packet *Defrag6Reassemble(ThreadVars *tv, DefragTracker *tracker,
                                 Packet *p) {
  Packet *rp = NULL;

  /* Should not be here unless we have seen the last fragment. */
  if (!tracker->seen_last) return NULL;
  ...
  RB_FOREACH(frag, IP_FRAGMENTS, &tracker->fragment_tree) {
    if (frag->skip) continue;
    if (frag->data_len - frag->ltrim <= 0) continue;
    if (frag->offset == 0) {
      IPV6FragHdr *frag_hdr =
          (IPV6FragHdr *)(frag->pkt + frag->frag_hdr_offset);
      ...
      if (PacketCopyData(rp, frag->pkt, frag->frag_hdr_offset) == -1)
        goto error_remove_tracker;
      if (PacketCopyDataOffset(
              rp, frag->frag_hdr_offset,
       ...
       * and the frag header. */
      DEBUG_VALIDATE_BUG_ON(fragmentable_offset <
                            ip_hdr_offset + IPV6_HEADER_LEN);
      DEBUG_VALIDATE_BUG_ON(
          fragmentable_offset - ip_hdr_offset - IPV6_HEADER_LEN > UINT16_MAX);
      unfragmentable_len =
          (uint16_t)(fragmentable_offset - ip_hdr_offset - IPV6_HEADER_LEN);
      if (unfragmentable_len >= fragmentable_offset) goto error_remove_tracker;
    } else {
      if (PacketCopyDataOffset(rp,
                               fragmentable_offset + frag->offset + frag->ltrim,
                               frag->pkt + frag->data_offset + frag->ltrim,
  ...
  if (unfragmentable_len == 0) rp->ip6h->s_ip6_nxt = next_hdr;
  SET_PKT_LEN(rp, ip_hdr_offset + sizeof(IPV6Hdr) + unfragmentable_len +
                      fragmentable_len);
 ...
 */
static Packet *DefragInsertFrag(ThreadVars *tv, DecodeThreadVars *dtv,
  ...
  if (tracker->af == AF_INET) {
    ...
    SCLogDebug(
        ...
        more_frags ? "true" : "false", frag_offset, data_offset, data_len,
        frag_end, ip_hdr_offset, frag_hdr_offset);
  ...
  /* Update timeout. */
  tracker->timeout =
      SCTIME_FROM_SECS(SCTIME_SECS(p->ts) + tracker->host_timeout);
  ...
  if (tracker->seen_last) {
    if (tracker->af == AF_INET) {
      ...
      if (r != NULL && tv != NULL && dtv != NULL) {
        ...
        if (DecodeIPV4(tv, dtv, r, (void *)r->ip4h, IPV4_GET_IPLEN(r)) !=
            TM_ECODE_OK) {
          UNSET_TUNNEL_PKT(r);
          r->root = NULL;
...
static DefragTracker *DefragGetTracker(ThreadVars *tv, DecodeThreadVars *dtv,
                                       Packet *p) {
...
static Packet *BuildTestPacket(uint8_t proto, uint16_t id, uint16_t off, int mf,
  ...
  if (unlikely(pcontent == NULL)) return NULL;
  ...
  /* Self test. */
  if (IPV4_GET_VER(p) != 4) goto error;
  if (IPV4_GET_HLEN(p) != hlen) goto error;
  if (IPV4_GET_IPLEN(p) != hlen + content_len) goto error;
  if (IPV4_GET_IPID(p) != id) goto error;
  if (IPV4_GET_IPOFFSET(p) != off) goto error;
  if (IPV4_GET_MF(p) != mf) goto error;
  if (IPV4_GET_IPTTL(p) != ttl) goto error;
  if (IPV4_GET_IPPROTO(p) != proto) goto error;
...
static Packet *IPV6BuildTestPacket(uint8_t proto, uint32_t id, uint16_t off,
  ...
  if (unlikely(pcontent == NULL)) return NULL;
  memset(pcontent, content, content_len);
  PacketCopyDataOffset(p, sizeof(IPV6Hdr) + sizeof(IPV6FragHdr), pcontent,
                       content_len);
  ...
  /* Self test. */
  if (IPV6_GET_VER(p) != 6) goto error;
  if (IPV6_GET_NH(p) != 44) goto error;
  if (IPV6_GET_PLEN(p) != sizeof(IPV6FragHdr) + content_len) goto error;
...
static int DefragSturgesNovakBsdTest(void) {
  ...
  FAIL_IF_NOT(
      DefragDoSturgesNovakTest(DEFRAG_POLICY_BSD, expected, sizeof(expected)));
  PASS;
}
...
static int DefragSturgesNovakLastTest(void) {
  ...
  FAIL_IF_NOT(
      DefragDoSturgesNovakTest(DEFRAG_POLICY_LAST, expected, sizeof(expected)));
  PASS;
}
