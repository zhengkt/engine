    ...
    if (type == TCP_OPT_EOL) {
      ...
      switch (type) {
        ...
        case TCP_OPT_TFO:
          ...
          if ((olen != 2) &&
              (olen < TCP_OPT_TFO_MIN_LEN || olen > TCP_OPT_TFO_MAX_LEN ||
               !(((olen - 2) & 0x1) == 0))) {
            ENGINE_SET_EVENT(p, TCP_OPT_INVALID_LEN);
          } else {
...
static int DecodeTCPPacket(ThreadVars *tv, Packet *p, const uint8_t *pkt,
                           uint16_t len) {
...
#ifdef DEBUG
  SCLogDebug("TCP sp: %" PRIu32 " -> dp: %" PRIu32 " - HLEN: %" PRIu32
             " LEN: %" PRIu32 " %s%s%s%s%s%s",
