...
int DecodeGRE(ThreadVars *tv, DecodeThreadVars *dtv, Packet *p,
  ...
  switch (GRE_GET_VERSION(p->greh)) {
    case GRE_VERSION_0:
      ...
      if (GRE_FLAG_ISSET_KY(p->greh)) header_len += GRE_KEY_LEN;

      if (GRE_FLAG_ISSET_SQ(p->greh)) header_len += GRE_SEQ_LEN;
      ...
      if (GRE_FLAG_ISSET_ROUTE(p->greh)) {
        while (1) {
          ...
          if ((SCNtohs(gsre->af) == 0) && (gsre->sre_length == 0)) break;
    ...
    case GRE_VERSION_1:
      ...
      if (GREV1_FLAG_ISSET_ACK(p->greh)) header_len += GREV1_ACK_LEN;
  ...
  switch (GRE_GET_PROTO(p->greh)) {
    ...
    case GRE_PROTO_PPP: {
      if (gre_pptp_h && !gre_pptp_h->payload_length) return TM_ECODE_OK;
    ...
    case ETHERNET_TYPE_ERSPAN: {
      // Determine if it's Type I or Type II based on the flags in the GRE
      // header. Type I:  0|0|0|0|0|00000|000000000|00000 Type II:
      // 0|0|0|1|0|00000|000000000|00000
      //                Seq
      Packet *tp = PacketTunnelPktSetup(
          tv, dtv, p, pkt + header_len, len - header_len,
    ...
    case ETHERNET_TYPE_BRIDGE: {
      Packet *tp =
          PacketTunnelPktSetup(tv, dtv, p, pkt + header_len, len - header_len,
...
static int DecodeGREtest02(void) {
  uint8_t raw_gre[] = {
      0x00, 0x6e, 0x62, 0xac, 0x40, 0x00, 0x40, 0x2f, 0xc2, 0xc7, 0x0a, 0x00,
      0x00, 0x64, 0x0a, 0x00, 0x00, 0x8a, 0x30, 0x01, 0x0b, 0x00, 0x4e, 0x00,
      0x00, 0x00, 0x18, 0x4a, 0x50, 0xff, 0x03, 0x00, 0x21, 0x45, 0x00, 0x00,
      0x4a, 0x00, 0x00, 0x40, 0x00, 0x40, 0x11, 0x94, 0x22, 0x50, 0x7e, 0x2b,
      0x2d, 0xc2, 0x6d, 0x68, 0x68, 0x80, 0x0e, 0x00, 0x35, 0x00, 0x36, 0x9f,
      0x18, 0xdb, 0xc4, 0x01, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x01, 0x03, 0x73, 0x31, 0x36, 0x09, 0x73, 0x69, 0x74, 0x65, 0x6d, 0x65,
      0x74, 0x65, 0x72, 0x03, 0x63, 0x6f, 0x6d, 0x00, 0x00, 0x01, 0x00, 0x01,
...
static int DecodeGREtest03(void) {
  uint8_t raw_gre[] = {
      0x00, 0x6e, 0x62, 0xac, 0x40, 0x00, 0x40, 0x2f, 0xc2, 0xc7, 0x0a, 0x00,
      0x00, 0x64, 0x0a, 0x00, 0x00, 0x8a, 0x30, 0x01, 0x88, 0x0b, 0x00, 0x4e,
      0x00, 0x00, 0x00, 0x18, 0x4a, 0x50, 0xff, 0x03, 0x00, 0x21, 0x45, 0x00,
      0x00, 0x4a, 0x00, 0x00, 0x40, 0x00, 0x40, 0x11, 0x94, 0x22, 0x50, 0x7e,
      0x2b, 0x2d, 0xc2, 0x6d, 0x68, 0x68, 0x80, 0x0e, 0x00, 0x35, 0x00, 0x36,
      0x9f, 0x18, 0xdb, 0xc4, 0x01, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x01, 0x03, 0x73, 0x31, 0x36, 0x09, 0x73, 0x69, 0x74, 0x65, 0x6d,
      0x65, 0x74, 0x65, 0x72, 0x03, 0x63, 0x6f, 0x6d, 0x00, 0x00, 0x01, 0x00,
