...
typedef struct AppLayerParserProtoCtx_ {
  ...
  /** get FileContainer reference from the TX. MUST return a non-NULL reference
   * if the TX has or may have files in the requested direction at some point.
   */
...
static void AppLayerConfig(void) {
  g_applayerparser_error_policy =
      ExceptionPolicyParse("app-layer.error-policy", true);
}
...
void AppLayerFramesFreeContainer(Flow *f) {
  if (f == NULL || f->alparser == NULL || f->alparser->frames == NULL) return;
  AppLayerParserFramesFreeContainer(f->alparser->frames);
  f->alparser->frames = NULL;
}

FramesContainer *AppLayerFramesGetContainer(Flow *f) {
  if (f == NULL || f->alparser == NULL) return NULL;
...
FramesContainer *AppLayerFramesSetupContainer(Flow *f) {
  ...
  if (f == NULL || f->alparser == NULL ||
      (f->proto == IPPROTO_TCP && f->protoctx == NULL))
    return NULL;
#endif
...
static inline void AppLayerParserStreamTruncated(AppLayerParserState *pstate,
...
#ifdef UNITTESTS
void UTHAppLayerParserStateGetIds(void *ptr, uint64_t *i1, uint64_t *i2,
                                  uint64_t *log, uint64_t *min) {
...
AppLayerParserState *AppLayerParserStateAlloc(void) {
  ...
  AppLayerParserState *pstate =
      (AppLayerParserState *)SCMalloc(sizeof(*pstate));
  if (pstate == NULL) goto end;
...
int AppLayerParserConfParserEnabled(const char *ipproto,
  ...
  if (RunmodeIsUnittests()) goto enabled;
...
int AppLayerParserRegisterParser(uint8_t ipproto, AppProto alproto,
                                 uint8_t direction, AppLayerParserFPtr Parser) {
  SCEnter();
...
void AppLayerParserRegisterLoggerBits(uint8_t ipproto, AppProto alproto,
                                      LoggerId bits) {
  SCEnter();
...
void AppLayerParserRegisterGetStateProgressFunc(
    uint8_t ipproto, AppProto alproto,
    int (*StateGetProgress)(void *alstate, uint8_t direction)) {
  SCEnter();

  alp_ctx.ctxs[FlowGetProtoMapping(ipproto)][alproto].StateGetProgress =
      StateGetProgress;
...
void AppLayerParserRegisterTxFreeFunc(uint8_t ipproto, AppProto alproto,
                                      void (*StateTransactionFree)(void *,
                                                                   uint64_t)) {
  SCEnter();

  alp_ctx.ctxs[FlowGetProtoMapping(ipproto)][alproto].StateTransactionFree =
...
void AppLayerParserRegisterGetTxCnt(uint8_t ipproto, AppProto alproto,
  ...
  alp_ctx.ctxs[FlowGetProtoMapping(ipproto)][alproto].StateGetTxCnt =
      StateGetTxCnt;
...
void AppLayerParserRegisterGetFrameFuncs(
    uint8_t ipproto, AppProto alproto,
    AppLayerParserGetFrameIdByNameFn GetIdByNameFunc,
    AppLayerParserGetFrameNameByIdFn GetNameByIdFunc) {
  SCEnter();
  alp_ctx.ctxs[FlowGetProtoMapping(ipproto)][alproto].GetFrameIdByName =
      GetIdByNameFunc;
  alp_ctx.ctxs[FlowGetProtoMapping(ipproto)][alproto].GetFrameNameById =
      GetNameByIdFunc;
  SCReturn;
}
...
void AppLayerParserRegisterGetEventInfo(
  ...
  alp_ctx.ctxs[FlowGetProtoMapping(ipproto)][alproto].StateGetEventInfo =
      StateGetEventInfo;
...
void AppLayerParserRegisterStateDataFunc(
  ...
  alp_ctx.ctxs[FlowGetProtoMapping(ipproto)][alproto].GetStateData =
      GetStateData;

  SCReturn;
}

void AppLayerParserRegisterApplyTxConfigFunc(
    uint8_t ipproto, AppProto alproto,
    bool (*ApplyTxConfig)(void *state, void *tx, int mode, AppLayerTxConfig)) {
  SCEnter();

  alp_ctx.ctxs[FlowGetProtoMapping(ipproto)][alproto].ApplyTxConfig =
      ApplyTxConfig;
...
void AppLayerParserRegisterSetStreamDepthFlag(
  ...
  alp_ctx.ctxs[FlowGetProtoMapping(ipproto)][alproto].SetStreamDepthFlag =
      SetStreamDepthFlag;
...
void *AppLayerParserGetProtocolParserLocalStorage(uint8_t ipproto,
  ...
  if (alp_ctx.ctxs[FlowGetProtoMapping(ipproto)][alproto].LocalStorageAlloc !=
      NULL) {
...
void AppLayerParserDestroyProtocolParserLocalStorage(uint8_t ipproto,
  ...
  if (alp_ctx.ctxs[FlowGetProtoMapping(ipproto)][alproto].LocalStorageFree !=
      NULL) {
    alp_ctx.ctxs[FlowGetProtoMapping(ipproto)][alproto].LocalStorageFree(
        local_data);
  }
...
void AppLayerParserSetTransactionLogId(AppLayerParserState *pstate,
  ...
  if (pstate != NULL) pstate->log_id = tx_id;
...
uint64_t AppLayerParserGetTransactionInspectId(AppLayerParserState *pstate,
  ...
  if (pstate == NULL) SCReturnCT(0ULL, "uint64_t");

  SCReturnCT(pstate->inspect_id[(direction & STREAM_TOSERVER) ? 0 : 1],
             "uint64_t");
}

inline uint64_t AppLayerParserGetTxDetectFlags(AppLayerTxData *txd,
                                               const uint8_t dir) {
...
static inline void SetTxDetectFlags(AppLayerTxData *txd, const uint8_t dir,
...
void AppLayerParserSetTransactionInspectId(const Flow *f,
  ...
  SCLogDebug("called: %s, tag_txs_as_inspected %s",
             direction == 0 ? "toserver" : "toclient",
  ...
  while (1) {
    ...
    if (ires.tx_ptr == NULL) break;

    void *tx = ires.tx_ptr;
    idx = ires.tx_id;

    int state_progress =
        AppLayerParserGetStateProgress(ipproto, alproto, tx, flags);
    if (state_progress < state_done_progress) break;
...
AppLayerDecoderEvents *AppLayerParserGetDecoderEvents(
    AppLayerParserState *pstate) {
  SCEnter();
...
AppLayerGetFileState AppLayerParserGetTxFiles(const Flow *f, void *state,
                                              void *tx,
                                              const uint8_t direction) {
  SCEnter();

  if (alp_ctx.ctxs[f->protomap][f->alproto].GetTxFiles != NULL) {
    return alp_ctx.ctxs[f->protomap][f->alproto].GetTxFiles(state, tx,
                                                            direction);
  }
...
static void AppLayerParserFileTxHousekeeping(const Flow *f, void *tx,
                                             const uint8_t pkt_dir,
                                             const bool trunc) {
  AppLayerGetFileState files =
      AppLayerParserGetTxFiles(f, FlowGetAppState(f), tx, pkt_dir);
  if (files.fc) {
...
void AppLayerParserTransactionsCleanup(Flow *f, const uint8_t pkt_dir) {
  ...
  if (unlikely(p->StateTransactionFree == NULL)) SCReturn;
  ...
  if (alstate == NULL || alparser == NULL) SCReturn;

  const uint64_t min = alparser->min_id;
  const uint64_t total_txs = AppLayerParserGetTxCnt(f, alstate);
  const LoggerId logger_expectation =
      AppLayerParserProtocolGetLoggerBits(ipproto, alproto);
  ...
  bool skipped = false;
  // const bool support_files = AppLayerParserSupportsFiles(f->proto,
  // f->alproto);

  while (1) {
    AppLayerGetTxIterTuple ires =
        IterFunc(ipproto, alproto, alstate, i, total_txs, &state);
    if (ires.tx_ptr == NULL) break;
    ...
    if (txd != NULL && AppLayerParserHasFilesInDir(txd, pkt_dir)) {
      if (pkt_dir_trunc == -1)
        pkt_dir_trunc = AppLayerParserStateIssetFlag(
                            f->alparser, (pkt_dir == STREAM_TOSERVER)
    ...
    if (txd && has_tx_detect_flags) {
      if (!IS_DISRUPTED(ts_disrupt_flags) && f->sgh_toserver != NULL) {
        uint64_t detect_flags_ts =
            AppLayerParserGetTxDetectFlags(txd, STREAM_TOSERVER);
        if (!(detect_flags_ts &
      ...
      if (!IS_DISRUPTED(tc_disrupt_flags) && f->sgh_toclient != NULL) {
        ...
        if (!(detect_flags_tc &
              (APP_LAYER_TX_INSPECTED_FLAG | APP_LAYER_TX_SKIP_INSPECT_FLAG))) {
          SCLogDebug("%p/%" PRIu64
                     " skipping: TC inspect not done: ts:%" PRIx64,
    ...
    /* if file logging is enabled, we keep a tx active while some of the files
     * aren't logged yet. */
    if (txd) {
      SCLogDebug("files_opened %u files_logged %u files_stored %u",
                 txd->files_opened, txd->files_logged, txd->files_stored);
      ...
      if (txd->files_opened) {
        ...
        if (g_filedata_logger_enabled &&
            txd->files_opened != txd->files_stored) {
          skipped = true;
          goto next;
        }
      }
    ...
    SCLogDebug("skipped? %s i %" PRIu64 ", new_min %" PRIu64,
               skipped ? "true" : "false", i, new_min);
    if (!skipped) new_min = i + 1;
  ...
  SCLogDebug("update f->alparser->min_id? %" PRIu64 " vs %" PRIu64, new_min,
             alparser->min_id);
...
static inline int StateGetProgressCompletionStatus(const AppProto alproto,
                                                   const uint8_t flags) {
...
void *AppLayerParserGetTx(uint8_t ipproto, AppProto alproto, void *alstate,
                          uint64_t tx_id) {
  SCEnter();
  void *r = alp_ctx.ctxs[FlowGetProtoMapping(ipproto)][alproto].StateGetTx(
      alstate, tx_id);
  SCReturnPtr(r, "void *");
}
...
int AppLayerParserGetEventInfo(uint8_t ipproto, AppProto alproto,
...
int AppLayerParserGetEventInfoById(uint8_t ipproto, AppProto alproto,
                                   int event_id, const char **event_name,
                                   AppLayerEventType *event_type) {
  SCEnter();
  const int ipproto_map = FlowGetProtoMapping(ipproto);
...
uint64_t AppLayerParserGetTransactionActive(const Flow *f,
  ...
  uint64_t inspect_id =
      pstate->inspect_id[(direction & STREAM_TOSERVER) ? 0 : 1];
...
AppLayerTxData *AppLayerParserGetTxData(uint8_t ipproto, AppProto alproto,
                                        void *tx) {
  SCEnter();
  AppLayerTxData *d =
      alp_ctx.ctxs[FlowGetProtoMapping(ipproto)][alproto].GetTxData(tx);
  SCReturnPtr(d, "AppLayerTxData");
}

AppLayerStateData *AppLayerParserGetStateData(uint8_t ipproto, AppProto alproto,
                                              void *state) {
  SCEnter();
...
void AppLayerParserApplyTxConfig(uint8_t ipproto, AppProto alproto, void *state,
                                 void *tx, enum ConfigAction mode,
...
static inline void SetEOFFlags(AppLayerParserState *pstate,
                               const uint8_t flags) {
  if ((flags & (STREAM_EOF | STREAM_TOSERVER)) ==
      (STREAM_EOF | STREAM_TOSERVER)) {
    SCLogDebug("setting APP_LAYER_PARSER_EOF_TS");
    AppLayerParserStateSetFlag(pstate, APP_LAYER_PARSER_EOF_TS);
  } else if ((flags & (STREAM_EOF | STREAM_TOCLIENT)) ==
             (STREAM_EOF | STREAM_TOCLIENT)) {
 ...
 *  On first invocation of TCP parser in a direction, create a <alproto>.stream
 * frame. On STREAM_EOF, set the final length. */
static void HandleStreamFrames(Flow *f, StreamSlice stream_slice,
  ...
  if (((direction == 0 && (pstate->flags & APP_LAYER_PARSER_SFRAME_TS) == 0) ||
    ...
    if (frame == NULL) {
      int64_t frame_len = -1;
      if (flags & STREAM_EOF) frame_len = input_len;

      frame = AppLayerFrameNewByAbsoluteOffset(f, &stream_slice,
                                               stream_slice.offset, frame_len,
                                               direction, FRAME_STREAM_TYPE);
      if (frame) {
        SCLogDebug("opened: frame %p id %" PRIi64, frame, frame->id);
        frame->flags =
            FRAME_FLAG_ENDS_AT_EOF;  // TODO logic is not yet implemented
        DEBUG_VALIDATE_BUG_ON(
            frame->id !=
            1);  // should always be the first frame that is created
      }
      if (direction == 0) {
    ...
    if (frame) {
      ...
      SCLogDebug("%s: EOF frame->offset %" PRIu64 " -> %" PRIi64 ": o %" PRIi64,
                 AppProtoToString(f->alproto), frame->offset, frame_len,
                 slice_o);
      frame->len = frame_len;
    }
  }
...
static void Setup(Flow *f, const uint8_t direction, const uint8_t *input,
                  uint32_t input_len, const uint8_t flags, StreamSlice *as) {
  ...
  if (f->proto == IPPROTO_TCP && f->protoctx != NULL) {
    ...
    TcpStream *stream =
        (direction & STREAM_TOSERVER) ? &ssn->client : &ssn->server;
    as->offset = STREAM_APP_PROGRESS(stream);
  }
}

/** \retval int -1 in case of unrecoverable error. App-layer tracking stops for
 * this flow. \retval int 0 ok: we did not update app_progress \retval int 1 ok:
 * we updated app_progress */
int AppLayerParserParse(ThreadVars *tv, AppLayerParserThreadCtx *alp_tctx,
                        Flow *f, AppProto alproto, uint8_t flags,
  ...
  if (flags & STREAM_GAP) {
    if (!(p->option_flags & APP_LAYER_PARSER_OPT_ACCEPT_GAPS)) {
      ...
      if (f->alstate != NULL && !FlowChangeProto(f)) {
        AppLayerParserStreamTruncated(pstate, f->proto, alproto, f->alstate,
                                      flags);
      }
  ...
  if (alstate == NULL || FlowChangeProto(f)) {
    ...
    if (f->file_flags != 0) {
      AppLayerStateData *sd =
          AppLayerParserGetStateData(f->proto, f->alproto, f->alstate);
      if (sd != NULL) {
  ...
  if (input_len > 0 || (flags & STREAM_EOF)) {
    Setup(f, flags & (STREAM_TOSERVER | STREAM_TOCLIENT), input, input_len,
          flags, &stream_slice);
...
#endif
    /* invoke the parser */
    AppLayerResult res = p->Parser[direction](
        f, alstate, pstate, stream_slice,
  ...
  /* In cases like HeartBleed for TLS we need to inspect AppLayer but not
   * Payload */
...
bool AppLayerParserHasDecoderEvents(AppLayerParserState *pstate) {
  ...
  if (decoder_events && decoder_events->cnt) return true;
...
void AppLayerParserSetStreamDepth(uint8_t ipproto, AppProto alproto,
                                  uint32_t stream_depth) {
  SCEnter();

  alp_ctx.ctxs[FlowGetProtoMapping(ipproto)][alproto].stream_depth =
      stream_depth;
...
void AppLayerParserSetStreamDepthFlag(uint8_t ipproto, AppProto alproto,
...
int AppLayerParserGetFrameIdByName(uint8_t ipproto, AppProto alproto,
                                   const char *name) {
  if (alp_ctx.ctxs[FlowGetProtoMapping(ipproto)][alproto].GetFrameIdByName !=
      NULL) {
    return alp_ctx.ctxs[FlowGetProtoMapping(ipproto)][alproto].GetFrameIdByName(
        name);
  } else {
    return -1;
  }
...
const char *AppLayerParserGetFrameNameById(uint8_t ipproto, AppProto alproto,
                                           const uint8_t id) {
  if (alp_ctx.ctxs[FlowGetProtoMapping(ipproto)][alproto].GetFrameNameById !=
      NULL) {
    return alp_ctx.ctxs[FlowGetProtoMapping(ipproto)][alproto].GetFrameNameById(
        id);
  } else {
    return NULL;
  }
...
void AppLayerParserStateProtoCleanup(uint8_t protomap, AppProto alproto,
                                     void *alstate,
                                     AppLayerParserState *pstate) {
  SCEnter();
  ...
  if (ctx->StateFree != NULL && alstate != NULL) ctx->StateFree(alstate);

  /* free the app layer parser api state */
  if (pstate != NULL) AppLayerParserStateFree(pstate);
...
static void ValidateParserProtoDump(AppProto alproto, uint8_t ipproto) {
  ...
  printf("- StateGetEventInfo %p StateGetEventInfoById %p\n",
         ctx->StateGetEventInfo, ctx->StateGetEventInfoById);
}
...
static void ValidateParserProto(AppProto alproto, uint8_t ipproto) {
  ...
  if (ctx->Parser[0] == NULL && ctx->Parser[1] == NULL) return;
  ...
  if (!(THREE_SET(ctx->StateGetTx, ctx->StateGetTxCnt,
  ...
  if (!(BOTH_SET_OR_BOTH_UNSET(ctx->LocalStorageAlloc,
                               ctx->LocalStorageFree))) {
    goto bad;
  }
...
static inline void AppLayerParserStreamTruncated(AppLayerParserState *pstate,
  ...
  if (alp_ctx.ctxs[FlowGetProtoMapping(ipproto)][alproto].Truncate != NULL) {
    alp_ctx.ctxs[FlowGetProtoMapping(ipproto)][alproto].Truncate(alstate,
                                                                 direction);
  }
 ...
 */
static AppLayerResult TestProtocolParser(Flow *f, void *test_state,
                                         AppLayerParserState *pstate,
                                         StreamSlice stream_slice,
                                         void *local_data) {
  SCEnter();
 ...
 */
static void TestProtocolStateFree(void *s) { SCFree(s); }
...
void AppLayerParserRegisterProtocolUnittests(uint8_t ipproto, AppProto alproto,
                                             void (*RegisterUnittests)(void)) {
  SCEnter();
  alp_ctx.ctxs[FlowGetProtoMapping(ipproto)][alproto].RegisterUnittests =
      RegisterUnittests;
  SCReturn;
}
...
static int AppLayerParserTest01(void) {
  ...
  AppLayerParserRegisterTxFreeFunc(IPPROTO_TCP, ALPROTO_TEST,
  ...
  int r = AppLayerParserParse(NULL, alp_tctx, f, ALPROTO_TEST,
                              STREAM_TOSERVER | STREAM_EOF, testbuf, testlen);
  FAIL_IF(r != -1);
...
void AppLayerParserRegisterUnittests(void) {
  ...
  for (ip = 0; ip < FLOW_PROTO_DEFAULT; ip++) {
    for (alproto = 0; alproto < ALPROTO_MAX; alproto++) {
      ...
      if (ctx->RegisterUnittests == NULL) continue;
