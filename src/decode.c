...
static int DecodeTunnel(ThreadVars *, DecodeThreadVars *, Packet *,
...
static int DecodeTunnel(ThreadVars *tv, DecodeThreadVars *dtv, Packet *p,
  ...
  switch (proto) {
    ...
    default:
      SCLogDebug("FIXME: DecodeTunnel: protocol %" PRIu32 " not supported.",
                 proto);
      break;
  }
  return TM_ECODE_OK;
...
void DecodeRegisterPerfCounters(DecodeThreadVars *dtv, ThreadVars *tv) {
  ...
  dtv->counter_ethertype_unknown =
      StatsRegisterCounter("decoder.unknown_ethertype", tv);
  ...
  dtv->counter_tcp_active_sessions =
