...
void EnableTcpSessionDumping(void) { g_tcp_session_dump_enabled = 1; }

/* prototypes */
TcpSegment *StreamTcpGetSegment(ThreadVars *tv, TcpReassemblyThreadCtx *);
void StreamTcpCreateTestPacket(uint8_t *, uint8_t, uint8_t, uint8_t);

void StreamTcpReassembleInitMemuse(void) { SC_ATOMIC_INIT(ra_memuse); }

/**
 *  \brief  Function to Increment the memory usage counter for the TCP
 * reassembly segments
 *
...
void StreamTcpReassembleIncrMemuse(uint64_t size) {
  ...
  SCLogDebug("REASSEMBLY %" PRIu64 ", incr %" PRIu64,
             StreamTcpReassembleMemuseGlobalCounter(), size);
  return;
}
...
#endif
  SCLogDebug("REASSEMBLY %" PRIu64 ", decr %" PRIu64,
             StreamTcpReassembleMemuseGlobalCounter(), size);
  return;
}
...
void *StreamTcpReassembleRealloc(void *optr, size_t orig_size, size_t size) {
  if (size > orig_size) {
    if (StreamTcpReassembleCheckMemcap(size - orig_size) == 0) {
      SCLogDebug("memcap hit at %" PRIu64,
...
static void *TcpSegmentPoolAlloc(void) {
  ...
  if (unlikely(seg == NULL)) return NULL;

  if (IsTcpSessionDumpingEnabled()) {
    uint32_t memuse = sizeof(TcpSegmentPcapHdrStorage) +
                      sizeof(uint8_t) * TCPSEG_PKT_HDR_DEFAULT_SIZE;
...
static inline int SEGMENT_BEFORE_OFFSET(TcpStream *stream, TcpSegment *seg,
                                        uint64_t offset) {
  if (seg->sbseg.stream_offset + seg->sbseg.segment_len <= offset) return 1;
  return 0;
}
#endif
...
int StreamTcpAppLayerIsDisabled(Flow *f) {
  if (f->protoctx == NULL || f->proto != IPPROTO_TCP) return 0;
...
static int StreamTcpReassemblyConfig(bool quiet) {
  ...
  if (seg) {
    uint32_t prealloc = 0;
    if (StringParseUint32(&prealloc, 10, (uint16_t)strlen(seg->val), seg->val) <
        0) {
      SCLogError(
          "segment-prealloc of "
          "%s is invalid",
  ...
  (void)ConfGetBool("stream.reassembly.check-overlap-different-data",
                    &overlap_diff_data);
  ...
  if (!quiet) SCLogConfig("stream.reassembly \"max-regions\": %u", max_regions);
...
int StreamTcpReassembleInit(bool quiet) {
  ...
  if (StreamTcpReassemblyConfig(quiet) < 0) return -1;
...
TcpReassemblyThreadCtx *StreamTcpReassembleInitThreadCtx(ThreadVars *tv) {
  ...
  if (unlikely(ra_ctx == NULL)) return NULL;
  ...
  if (segment_thread_pool == NULL) {
    segment_thread_pool =
        PoolThreadInit(1, /* thread */
                       0, /* unlimited */
                       stream_config.prealloc_segments, sizeof(TcpSegment),
                       TcpSegmentPoolAlloc, TcpSegmentPoolInit, NULL,
  ...
  if (ra_ctx->segment_thread_pool_id < 0 || segment_thread_pool == NULL) {
    SCLogError(
        "failed to setup/expand stream segment pool. Expand "
 ...
 */
static uint32_t StreamTcpReassembleCheckDepth(TcpSession *ssn,
                                              TcpStream *stream, uint32_t seq,
                                              uint32_t size) {
  SCEnter();
   ...
   * wraps as well */
  SCLogDebug("seq + size %u, base %u, seg_depth %" PRIu64 " limit %u",
             (seq + size), stream->base_seq, seg_depth, ssn->reassembly_depth);
...
#endif
  if (SEQ_GEQ(seq, stream->isn) &&
      SEQ_LT(seq, (stream->isn + ssn->reassembly_depth))) {
...
uint32_t StreamDataAvailableForProtoDetect(TcpStream *stream) {
  if (RB_EMPTY(&stream->sb.sbb_tree)) {
    if (stream->sb.region.stream_offset != 0) return 0;
...
int StreamTcpReassembleHandleSegmentHandleData(ThreadVars *tv,
  ...
  if ((ssn->flags & STREAMTCP_FLAG_APP_LAYER_DISABLED) &&
    ...
    SCLogDebug("ssn %p: both app and raw reassembly disabled, not reassembling",
               ssn);
    SCReturnInt(0);
  }
  ...
  uint32_t size = StreamTcpReassembleCheckDepth(ssn, stream, TCP_GET_SEQ(p),
                                                p->payload_len);
  ...
  if (size > p->payload_len) size = p->payload_len;
...
#undef STREAMTCP_STREAM_FLAG_FLUSH_FLAGS
  ...
  if (ssn->state > TCP_TIME_WAIT) SCReturnInt(1);

  if (p->flags & PKT_PSEUDO_STREAM_END) SCReturnInt(1);

  const uint64_t last_ack_abs = GetAbsLastAck(stream);
  int64_t diff = last_ack_abs - STREAM_RAW_PROGRESS(stream);
  int64_t chunk_size =
      PKT_IS_TOSERVER(p)
          ? (int64_t)stream_config.reassembly_toserver_chunk_size
  ...
  if (chunk_size <= diff) {
    ...
    SCLogDebug(
        ...
        " < "
        "%" PRIi64,
        PKT_IS_TOSERVER(p) ? "toserver" : "toclient", stream->last_ack,
        stream->base_seq, (stream->last_ack - stream->base_seq), chunk_size);
    SCReturnInt(0);
  }
...
uint8_t StreamNeedsReassembly(const TcpSession *ssn, uint8_t direction) {
  ...
  if (use_raw) {
    ...
    if (right_edge > STREAM_RAW_PROGRESS(stream)) {
      SCLogDebug("%s: STREAM_HAS_UNPROCESSED_SEGMENTS_NEED_ONLY_DETECTION",
                 dirstr);
  ...
  if (use_app) {
    const uint64_t right_edge =
        StreamingBufferGetConsecutiveDataRightEdge(&stream->sb);
    SCLogDebug("%s: app %" PRIu64 " (use: %s), raw %" PRIu64
               " (use: %s). Stream right edge: %" PRIu64,
               dirstr, STREAM_APP_PROGRESS(stream), use_app ? "yes" : "no",
               STREAM_RAW_PROGRESS(stream), use_raw ? "yes" : "no", right_edge);
  ...
  if (stream) {
    ...
    RB_FOREACH(seg, TCPSEG, &stream->seg_tree) {
      ...
      if (last_re != 0 && last_re < seg_abs) {
        ...
        SCLogDebug(" -> gap of size %" PRIu64 ", ack:%s", seg_abs - last_re,
                   gacked);
      }
      ...
      SCLogDebug("%u -> seg %p seq %u abs %" PRIu64 " size %u abs %" PRIu64
                 " (%" PRIu64 ") ack:%s",
                 cnt, seg, seg->seq, seg_abs, TCP_SEG_LEN(seg),
                 seg_abs + (uint64_t)TCP_SEG_LEN(seg),
                 STREAM_BASE_OFFSET(stream), acked);
...
static StreamingBufferBlock *GetBlock(const StreamingBuffer *sb,
...
static inline bool GapAhead(const TcpStream *stream,
                            StreamingBufferBlock *cur_blk) {
...
static bool GetAppBuffer(const TcpStream *stream, const uint8_t **data,
                         uint32_t *data_len, uint64_t offset,
  ...
  if (RB_EMPTY(&stream->sb.sbb_tree)) {
    ...
    *data_len = mydata_len;
    ...
    SCLogDebug("blk %p blk->offset %" PRIu64 ", blk->len %u", blk, blk->offset,
               blk->len);
    ...
    if (blk->offset == offset) {
      ...
      *data_len = blk->offset - offset;
      ...
      SCLogDebug("get data from offset %" PRIu64 ". SBB %" PRIu64 "/%u", offset,
                 blk->offset, blk->len);
      StreamingBufferSBBGetDataAtOffset(&stream->sb, blk, data, data_len,
                                        offset);
...
static inline uint32_t AdjustToAcked(const Packet *p, const TcpSession *ssn,
  ...
  if (StreamTcpInlineMode() == FALSE) {
    ...
    if (data_len == 0 ||
      ...
      if (app_progress <= last_ack_abs &&
          app_progress + data_len > last_ack_abs) {
...
static int ReassembleUpdateAppLayer(ThreadVars *tv,
...
#ifdef DEBUG
  uint64_t last_ack_abs = GetAbsLastAck(*stream);
  SCLogDebug("last_ack %u (abs %" PRIu64 "), base_seq %u", (*stream)->last_ack,
             last_ack_abs, (*stream)->base_seq);
#endif
  const uint8_t *mydata;
  uint32_t mydata_len;
  ...
  while (1) {
    ...
    bool gap_ahead = GetAppBuffer(*stream, &mydata, &mydata_len, app_progress,
                                  check_for_gap_ahead);
    ...
    if (mydata == NULL && mydata_len > 0 && CheckGap(ssn, *stream, p)) {
      ...
      int r = AppLayerHandleTCPData(
          tv, ra_ctx, p, p->flow, ssn, stream, NULL, mydata_len,
      ...
      if (mydata == NULL && mydata_len > 0) {
        mydata_len = 0;
               ...
               (mydata_len == 0 && ((flags & STREAM_EOF) == 0))) {
      SCLogDebug("GAP?1");
    ...
    (void)AppLayerHandleTCPData(tv, ra_ctx, p, p->flow, ssn, stream,
                                (uint8_t *)mydata, mydata_len, flags, dir);
    AppLayerProfilingStore(ra_ctx->app_tctx, p);
    AppLayerFrameDump(p->flow);
    uint64_t new_app_progress = STREAM_APP_PROGRESS(*stream);
    if (new_app_progress == app_progress || FlowChangeProto(p->flow)) break;
    app_progress = new_app_progress;
    if (flags & STREAM_DEPTH) break;
  }
...
int StreamTcpReassembleAppLayer(ThreadVars *tv, TcpReassemblyThreadCtx *ra_ctx,
  ...
  if (!STREAM_HAS_SEEN_DATA(stream) ||
      STREAM_RIGHT_EDGE(stream) <= STREAM_APP_PROGRESS(stream)) {
 ...
 *  \brief get stream data from offset
 *  \param offset stream offset */
static int GetRawBuffer(TcpStream *stream, const uint8_t **data,
                        uint32_t *data_len, StreamingBufferBlock **iter,
  ...
  if (RB_EMPTY(&stream->sb.sbb_tree)) {
    ...
    *data_offset = roffset;
    ...
    SCLogDebug("getting multiple blobs. Iter %p, %" PRIu64 "/%u", *iter,
 ...
 */
void StreamReassembleRawUpdateProgress(TcpSession *ssn, Packet *p,
  ...
  if (progress > STREAM_RAW_PROGRESS(stream)) {
    ...
    SCLogDebug("p->pcap_cnt %" PRIu64 ": progress %" PRIu64 " app %" PRIu64
...
static int StreamReassembleRawInline(TcpSession *ssn, const Packet *p,
                                     ...
                                     void *cb_data, uint64_t *progress_out) {
  SCEnter();
  int r = 0;
  ...
  uint64_t packet_leftedge_abs =
      STREAM_BASE_OFFSET(stream) + (TCP_GET_SEQ(p) - stream->base_seq);
  ...
  if (RB_EMPTY(&stream->sb.sbb_tree)) {
    ...
    /* find our block */
    StreamingBufferBlock key = {.offset = packet_leftedge_abs,
                                .len = p->payload_len};
    StreamingBufferBlock *sbb =
        SBB_RB_FIND_INCLUSIVE(&stream->sb.sbb_tree, &key);
    if (sbb) {
      SCLogDebug("found %p offset %" PRIu64 " len %u", sbb, sbb->offset,
                 sbb->len);
  ...
  if ((mydata == NULL || mydata_len == 0) ||    /* no data */
      (mydata_offset >= packet_rightedge_abs || /* data all to the right */
       packet_leftedge_abs >=
           mydata_rightedge_abs) ||           /* data all to the left */
      (packet_leftedge_abs < mydata_offset || /* data missing at the start */
       packet_rightedge_abs >
           mydata_rightedge_abs)) /* data missing at the end */
  {
    ...
    if (mydata_len > chunk_size) {
      ...
      SCLogDebug("chunk_size %u mydata_len %u excess %u", chunk_size,
                 mydata_len, excess);
      ...
      if (mydata_rightedge_abs == packet_rightedge_abs) {
        ...
        uint32_t after =
            (uint32_t)(mydata_rightedge_abs - packet_rightedge_abs);
        ...
        /* adjust the buffer */
        uint32_t skip =
            (uint32_t)(packet_leftedge_abs - mydata_offset) - before;
        uint32_t cut =
            (uint32_t)(mydata_rightedge_abs - packet_rightedge_abs) - after;
...
static int StreamReassembleRawDo(TcpSession *ssn, TcpStream *stream,
                                 StreamReassembleRawFunc Callback,
                                 void *cb_data, const uint64_t progress_in,
                                 const uint64_t re, uint64_t *progress_out,
                                 bool eof, bool respect_inspect_depth) {
  SCEnter();
  int r = 0;
  ...
  while (1) {
    ...
    if (eof) {
      ...
      SCLogDebug("raw_progress + mydata_len %" PRIu64 ", re %" PRIu64,
                 progress + mydata_len, re);
...
int StreamReassembleForFrame(TcpSession *ssn, TcpStream *stream,
                             StreamReassembleRawFunc Callback, void *cb_data,
                             const uint64_t offset, const bool eof) {
  /* take app progress as the right edge of used data. */
  const uint64_t app_progress = STREAM_APP_PROGRESS(stream);
  SCLogDebug("app_progress %" PRIu64, app_progress);

  uint64_t unused = 0;
  return StreamReassembleRawDo(ssn, stream, Callback, cb_data, offset,
                               app_progress, &unused, eof, false);
}
...
int StreamReassembleRaw(TcpSession *ssn, const Packet *p,
  ...
  if (respect_inspect_depth &&
    ...
    SCLogDebug("stream app %" PRIu64 ", raw %" PRIu64,
               STREAM_APP_PROGRESS(stream), STREAM_RAW_PROGRESS(stream));

    progress = MIN(progress, STREAM_RAW_PROGRESS(stream));
    SCLogDebug(
        "applied min inspect depth due to STREAMTCP_STREAM_FLAG_TRIGGER_RAW: "
        "progress "
        "%" PRIu64,
        progress);
  }

  SCLogDebug("progress %" PRIu64 ", min inspect depth %u %s", progress,
             stream->min_inspect_depth,
             stream->flags & STREAMTCP_STREAM_FLAG_TRIGGER_RAW
  ...
  return StreamReassembleRawDo(
      ssn, stream, Callback, cb_data, progress, last_ack_abs, progress_out,
...
int StreamReassembleLog(TcpSession *ssn, TcpStream *stream,
                        ...
                        bool eof) {
  if (stream->flags & (STREAMTCP_STREAM_FLAG_NOREASSEMBLY)) return 0;

  /* absolute right edge of ack'd data */
  const uint64_t last_ack_abs = GetAbsLastAck(stream);
  SCLogDebug("last_ack_abs %" PRIu64, last_ack_abs);

  return StreamReassembleRawDo(ssn, stream, Callback, cb_data, progress_in,
                               last_ack_abs, progress_out, eof, false);
}
 ...
 */
static int StreamTcpReassembleHandleSegmentUpdateACK(
    ThreadVars *tv, TcpReassemblyThreadCtx *ra_ctx, TcpSession *ssn,
    TcpStream *stream, Packet *p) {
  SCEnter();

  if (StreamTcpReassembleAppLayer(tv, ra_ctx, ssn, stream, p,
                                  UPDATE_DIR_OPPOSING) < 0)
    SCReturnInt(-1);
...
int StreamTcpReassembleHandleSegment(ThreadVars *tv,
  ...
  SCLogDebug("ssn %p, stream %p, p %p, p->payload_len %" PRIu16 "", ssn, stream,
             p, p->payload_len);
  ...
  if ((dir == UPDATE_DIR_OPPOSING || dir == UPDATE_DIR_BOTH)) {
    ...
    const bool reversed_before_ack_handling =
        (p->flow->flags & FLOW_DIR_REVERSED) != 0;

    if (StreamTcpReassembleHandleSegmentUpdateACK(tv, ra_ctx, ssn,
                                                  opposing_stream, p) != 0) {
    ...
    const bool reversed_after_ack_handling =
        (p->flow->flags & FLOW_DIR_REVERSED) != 0;
  ...
  if (p->payload_len > 0 &&
    ...
    if (StreamTcpReassembleHandleSegmentHandleData(tv, ra_ctx, ssn, stream,
                                                   p) != 0) {
      SCLogDebug("StreamTcpReassembleHandleSegmentHandleData error");
      /* failure can only be because of memcap hit, so see if this should lead
       * to a drop */
      ExceptionPolicyApply(p, stream_config.reassembly_memcap_policy,
...
void StreamTcpReassemblySetMinInspectDepth(TcpSession *ssn, int direction,
                                           uint32_t depth) {
#ifdef DEBUG
  BUG_ON(ssn == NULL);
#endif
 ...
 *                      current packet.
 *  \param  value       The value which current payload will have for this
 * packet \param  payload_len The length of the filed payload for current
 * packet. \param  len         Length of the payload array
 */

void StreamTcpCreateTestPacket(uint8_t *payload, uint8_t value,
                               uint8_t payload_len, uint8_t len) {
  uint8_t i;
  for (i = 0; i < payload_len; i++) payload[i] = value;
  for (; i < len; i++) payload = NULL;
}

/** \brief  The Function Checks the reassembled stream contents against
 * predefined stream contents according to OS policy used.
 *
 *  \param  stream_policy   Predefined value of stream for different OS policies
 *  \param  stream          Reassembled stream returned from the reassembly
 * functions
 */

int StreamTcpCheckStreamContents(uint8_t *stream_policy, uint16_t sp_size,
                                 TcpStream *stream) {
  if (StreamingBufferCompareRawData(&stream->sb, stream_policy,
                                    (uint32_t)sp_size) == 0) {
...
#define MISSED_STEP(seq, seg, seglen, buf, buflen)                          \
  StreamTcpUTAddPayload(&tv, ra_ctx, &ssn, stream, (seq), (uint8_t *)(seg), \
                        (seglen));                                          \
...
int UTHCheckGapAtPosition(TcpStream *stream, int pos, uint64_t offset,
                          uint32_t len);
...
int UTHCheckDataAtPosition(TcpStream *stream, int pos, uint64_t offset,
...
static int StreamTcpReassembleTest33(void) {
  ...
  FAIL_IF(StreamTcpReassembleHandleSegment(&tv, ra_ctx, &ssn, &ssn.client, p) ==
          -1);
...
static int StreamTcpReassembleTest39(void) {
  ...
  uint8_t request2[] = {
      0x54, 0x20, 0x2f, 0x69, 0x6e, 0x64, 0x65, 0x78, 0x2e, 0x68, 0x74,
      0x6d, 0x6c, 0x20, 0x48, 0x54, 0x54, 0x50, 0x2f, 0x31, 0x2e, 0x30,
      0x0d, 0x0a, 0x48, 0x6f, 0x73, 0x74, 0x3a, 0x20, 0x6c, 0x6f, 0x63,
      0x61, 0x6c, 0x68, 0x6f, 0x73, 0x74, 0x0d, 0x0a, 0x55, 0x73, 0x65,
      0x72, 0x2d, 0x41, 0x67, 0x65, 0x6e, 0x74, 0x3a, 0x20, 0x41, 0x70,
      0x61, 0x63, 0x68, 0x65, 0x42, 0x65, 0x6e, 0x63, 0x68, 0x2f, 0x32,
      0x2e, 0x33, 0x0d, 0x0a, 0x41, 0x63, 0x63, 0x65, 0x70, 0x74, 0x3a,
  ...
  FAIL_IF(!TCPSEG_RB_NEXT(RB_MIN(TCPSEG, &ssn->client.seg_tree)));
  FAIL_IF(
      TCPSEG_RB_NEXT(TCPSEG_RB_NEXT(RB_MIN(TCPSEG, &ssn->client.seg_tree))));
  FAIL_IF(!RB_EMPTY(&ssn->server.seg_tree));
  FAIL_IF(ssn->data_first_seen_dir != STREAM_TOSERVER);
  ...
  uint8_t response[] = {
      ...
      ,
      ...
      0x6c, 0x0d, 0x0a, 0x58, 0x2d, 0x50, 0x61, 0x64, 0x3a, 0x20, 0x61, 0x76,
      0x6f, 0x69, 0x64, 0x20, 0x62, 0x72, 0x6f, 0x77, 0x73, 0x65, 0x72, 0x20,
      0x62, 0x75, 0x67, 0x0d, 0x0a, 0x0d, 0x0a, 0x3c, 0x68, 0x74, 0x6d, 0x6c,
      0x3e, 0x3c, 0x62, 0x6f, 0x64, 0x79, 0x3e, 0x3c, 0x68, 0x31, 0x3e, 0x49,
      0x74, 0x20, 0x77, 0x6f, 0x72, 0x6b, 0x73, 0x21, 0x3c, 0x2f, 0x68, 0x31,
...
static int StreamTcpReassembleTest40(void) {
  ...
  FAIL_IF_NULL(seg);
  FAIL_IF(SEGMENT_BEFORE_OFFSET(&ssn.client, seg,
                                STREAM_APP_PROGRESS(&ssn.client)));
...
static int StreamTcpReassembleTest44(void) {
  ...
  FAIL_IF(StreamTcpReassembleCheckMemcap(
              (1 + memuse + SC_ATOMIC_GET(stream_config.reassembly_memcap))) !=
          0);
  StreamTcpFreeConfig(true);
...
static int StreamTcpReassembleTest45(void) {
  ...
  int r = StreamTcpUTAddPayload(&tv, ra_ctx, &ssn, &ssn.client, 101, payload,
                                payload_size);
  FAIL_IF(r != 0);
  FAIL_IF(ssn.client.flags & STREAMTCP_STREAM_FLAG_DEPTH_REACHED);

  r = StreamTcpUTAddPayload(&tv, ra_ctx, &ssn, &ssn.client, 201, payload,
                            payload_size);
  FAIL_IF(r != 0);
  FAIL_IF(!(ssn.client.flags & STREAMTCP_STREAM_FLAG_DEPTH_REACHED));
...
static int StreamTcpReassembleInlineTest01(void) {
  ...
  Packet *p = UTHBuildPacketReal(payload, 5, IPPROTO_TCP, "1.1.1.1", "2.2.2.2",
                                 1024, 80);
  if (p == NULL) {
  ...
  if (StreamTcpUTAddSegmentWithByte(&tv, ra_ctx, &ssn.client, 2, 'A', 5) ==
      -1) {
    printf("failed to add segment 1: ");
    goto end;
  }
  if (StreamTcpUTAddSegmentWithByte(&tv, ra_ctx, &ssn.client, 7, 'B', 5) ==
      -1) {
    printf("failed to add segment 2: ");
    goto end;
  }
  if (StreamTcpUTAddSegmentWithByte(&tv, ra_ctx, &ssn.client, 12, 'C', 5) ==
      -1) {
...
/** \test 3 in order segments, then reassemble, add one more and reassemble
 * again. test the sliding window reassembly.
 */
static int StreamTcpReassembleInlineTest02(void) {
  ...
  ssn.client.next_seq = 17;
  if (StreamTcpUTAddSegmentWithByte(&tv, ra_ctx, &ssn.client, 17, 'D', 5) ==
      -1) {
...
/** \test 3 in order segments, then reassemble, add one more and reassemble
 * again. test the sliding window reassembly with a small window size so that we
...
static int StreamTcpReassembleInlineTest08(void) {
  ...
  FAIL_IF(StreamTcpUTAddSegmentWithByte(&tv, ra_ctx, &ssn.client, 2, 'A', 5) ==
          -1);
  FAIL_IF(StreamTcpUTAddSegmentWithByte(&tv, ra_ctx, &ssn.client, 7, 'B', 5) ==
          -1);
  FAIL_IF(StreamTcpUTAddSegmentWithByte(&tv, ra_ctx, &ssn.client, 12, 'C', 5) ==
          -1);
  ssn.client.next_seq = 17;
  FAIL_IF(StreamTcpUTAddSegmentWithByte(&tv, ra_ctx, &ssn.client, 17, 'D', 5) ==
          -1);
  ssn.client.next_seq = 22;
  p->tcph->th_seq = htonl(17);
  StreamTcpPruneSession(&f, STREAM_TOSERVER);
...
static int StreamTcpReassembleInlineTest10(void) {
  ...
  p = UTHBuildPacketReal(stream_payload3, 12, IPPROTO_TCP, "1.1.1.1", "2.2.2.2",
                         1024, 80);
  if (p == NULL) {
    printf("couldn't get a packet: ");
    goto end;
  ...
  if (StreamTcpUTAddSegmentWithPayload(&tv, ra_ctx, &ssn.client, 2,
                                       stream_payload1, 2) == -1) {
    printf("failed to add segment 1: ");
    goto end;
  }
  ...
  int r = StreamTcpReassembleAppLayer(&tv, ra_ctx, &ssn, &ssn.client, p,
                                      UPDATE_DIR_PACKET);
  if (r < 0) {
  ...
  if (StreamTcpUTAddSegmentWithPayload(&tv, ra_ctx, &ssn.client, 4,
                                       stream_payload2, 3) == -1) {
    printf("failed to add segment 2: ");
    goto end;
  }
  if (StreamTcpUTAddSegmentWithPayload(&tv, ra_ctx, &ssn.client, 7,
                                       stream_payload3, 12) == -1) {
    printf("failed to add segment 3: ");
    goto end;
  }
  ssn.client.next_seq = 19;

  r = StreamTcpReassembleAppLayer(&tv, ra_ctx, &ssn, &ssn.client, p,
                                  UPDATE_DIR_PACKET);
  if (r < 0) {
    printf("StreamTcpReassembleAppLayer failed: ");
    goto end;
...
static int StreamTcpReassembleInsertTest01(void) {
  ...
  FAIL_IF(StreamTcpUTAddSegmentWithByte(&tv, ra_ctx, &ssn.client, 7, 'B', 5) ==
          -1);
  FAIL_IF(StreamTcpUTAddSegmentWithByte(&tv, ra_ctx, &ssn.client, 14, 'D', 2) ==
          -1);
  FAIL_IF(StreamTcpUTAddSegmentWithByte(&tv, ra_ctx, &ssn.client, 16, 'D', 6) ==
          -1);
  FAIL_IF(StreamTcpUTAddSegmentWithByte(&tv, ra_ctx, &ssn.client, 12, 'C', 5) ==
          -1);
...
static int StreamTcpReassembleInsertTest02(void) {
  ...
  for (i = 2; i < 10; i++) {
    ...
    if (StreamTcpUTAddSegmentWithByte(&tv, ra_ctx, &ssn.client, seq, 'A',
                                      len) == -1) {
      printf("failed to add segment 1: ");
      goto end;
    }
  }
  if (StreamTcpUTAddSegmentWithByte(&tv, ra_ctx, &ssn.client, 2, 'B', 1024) ==
      -1) {
    printf("failed to add segment 2: ");
    goto end;
  }
...
static int StreamTcpReassembleInsertTest03(void) {
  ...
  if (StreamTcpUTAddSegmentWithByte(&tv, ra_ctx, &ssn.client, 2, 'A', 1024) ==
      -1) {
    printf("failed to add segment 2: ");
    goto end;
  }
  ...
  for (i = 2; i < 10; i++) {
    ...
    if (StreamTcpUTAddSegmentWithByte(&tv, ra_ctx, &ssn.client, seq, 'B',
                                      len) == -1) {
      printf("failed to add segment 2: ");
      goto end;
    }
...
void StreamTcpReassembleRegisterTests(void) {
  ...
  UtRegisterTest(
      "StreamTcpReassembleTest28 -- Gap at Start IDS missed packet Reassembly "
      "Test",
      StreamTcpReassembleTest28);
  UtRegisterTest(
      "StreamTcpReassembleTest29 -- Gap at Middle IDS missed packet Reassembly "
      "Test",
