...
void TmThreadSetPrio(ThreadVars *tv) {
  ...
  if (0 == SetThreadPriority(GetCurrentThread(), tv->thread_priority)) {
    ...
    SCLogDebug("Priority set to %" PRId32 " for thread %s", tv->thread_priority,
               tv->name);
  }
#else
...
static bool ThreadBusy(ThreadVars *tv) {
  for (TmSlot *s = tv->tm_slots; s != NULL; s = s->slot_next) {
    ...
    if (tm && tm->ThreadBusy != NULL) {
      if (tm->ThreadBusy(tv, SC_ATOMIC_GET(s->slot_data))) return true;
    }
  }
  return false;
}
...
TmEcode TmThreadSpawn(ThreadVars *tv) {
  ...
  if (threading_set_stack_size) {
    SCLogDebug("Setting per-thread stack size to %" PRIu64,
               threading_set_stack_size);
    if (pthread_attr_setstacksize(&attr, (size_t)threading_set_stack_size)) {
      FatalError("Unable to increase stack size to %" PRIu64
  ...
  if (threading_set_stack_size) {
    if (pthread_getattr_np(tv->t, &attr) == 0) {
      ...
      SCLogDebug("stack: %p;  size %" PRIu64, stack_addr,
...
again:
  ...
  for (int i = 0; i < TVT_MAX; i++) {
    for (ThreadVars *tv = tv_root[i]; tv != NULL; tv = tv->next) {
      if (strncmp(thread_name_autofp, tv->name, strlen(thread_name_autofp)) ==
          0)
        RX_num++;
      else if (strncmp(thread_name_workers, tv->name,
                       strlen(thread_name_workers)) == 0)
        W_num++;
      else if (strncmp(thread_name_verdict, tv->name,
                       strlen(thread_name_verdict)) == 0)
        TX_num++;
      else if (strncmp(thread_name_flow_mgr, tv->name,
                       strlen(thread_name_flow_mgr)) == 0)
        FM_num++;
      else if (strncmp(thread_name_flow_rec, tv->name,
  ...
  strlcpy(thread_counts, "Threads created -> ",
          strlen("Threads created -> ") + 1);
  if (RX_num > 0) {
...
void TmThreadsListThreads(void) {
  ...
  for (size_t s = 0; s < thread_store.threads_size; s++) {
    ...
    if (t->tv) {
      ThreadVars *tv = t->tv;
      const uint32_t flags = SC_ATOMIC_GET(tv->flags);
      SCLogNotice("tv %p type %u name %s tmm_flags %02X flags %X", tv, tv->type,
                  tv->name, tv->tmm_flags, flags);
    }
  }
...
uint16_t TmThreadsGetWorkerThreadMax(void) {
  ...
  if (thread_max > 1024) {
    SCLogWarning("limited number of 'worker' threads to 1024. Wanted %d",
                 thread_max);
    thread_max = 1024;
  }
