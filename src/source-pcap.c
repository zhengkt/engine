 ...
 * 32 bit pcap_stat.
 */
static inline void UpdatePcapStats64(PcapStats64 *last,
...
static int PcapOpenInterface(PcapThreadVars *ptv) {
  ...
  if (ptv->pcap_handle == NULL) {
    if (strlen(errbuf)) {
      SCLogError("%s: could not create a new pcap handler, error %s", iface,
                 errbuf);
    } else {
  ...
  if (ptv->pcap_snaplen > 0) {
    ...
    int pcap_set_snaplen_r =
        pcap_set_snaplen(ptv->pcap_handle, ptv->pcap_snaplen);
    if (pcap_set_snaplen_r != 0) {
      SCLogError("%s: could not set snaplen, error: %s", iface,
  ...
  if (pcap_set_timeout_r != 0) {
    SCLogError("%s: could not set timeout, error %s", iface,
               pcap_geterr(ptv->pcap_handle));
    SCReturnInt(TM_ECODE_FAILED);
  }
#ifdef HAVE_PCAP_SET_BUFF
  if (ptv->pcap_buffer_size > 0) {
    SCLogInfo("%s: going to use pcap buffer size of %" PRId32, iface,
              ptv->pcap_buffer_size);

    int pcap_set_buffer_size_r =
        pcap_set_buffer_size(ptv->pcap_handle, ptv->pcap_buffer_size);
  ...
  if (ptv->bpf_filter) {
    ...
    if (pcap_compile(ptv->pcap_handle, &ptv->filter, (char *)ptv->bpf_filter, 1,
                     0) < 0) {
      SCLogError("%s: bpf compilation error %s", iface,
                 pcap_geterr(ptv->pcap_handle));
    ...
    if (pcap_setfilter(ptv->pcap_handle, &ptv->filter) < 0) {
      SCLogError("%s: could not set bpf filter %s", iface,
                 pcap_geterr(ptv->pcap_handle));
      SCMutexUnlock(&pcap_bpf_compile_lock);
      return TM_ECODE_FAILED;
    }
...
static int PcapTryReopen(PcapThreadVars *ptv) {
  ...
  if (PcapOpenInterface(ptv) != TM_ECODE_OK) return -1;
  ...
  switch (ptv->checksum_mode) {
    case CHECKSUM_VALIDATION_AUTO:
      if (ChecksumAutoModeCheck(
              ptv->pkts, SC_ATOMIC_GET(ptv->livedev->pkts),
...
static TmEcode ReceivePcapThreadInit(ThreadVars *tv, const void *initdata,
  ...
  ptv->capture_kernel_ifdrops =
      StatsRegisterCounter("capture.kernel_ifdrops", ptv->tv);
...
static void ReceivePcapThreadExitStats(ThreadVars *tv, void *data) {
  ...
  if (pcap_stats(ptv->pcap_handle, &pcap_s) < 0) {
    SCLogError("%s: failed to get pcap_stats: %s", ptv->livedev->dev,
               pcap_geterr(ptv->pcap_handle));
    SCLogInfo("%s: packets %" PRIu64 ", bytes %" PRIu64 "", ptv->livedev->dev,
              ptv->pkts, ptv->bytes);
  } else {
    SCLogInfo("%s: packets %" PRIu64 ", bytes %" PRIu64 "", ptv->livedev->dev,
              ptv->pkts, ptv->bytes);
...
static TmEcode DecodePcapThreadInit(ThreadVars *tv, const void *initdata,
                                    void **data) {
  SCEnter();
 ...
 */
static void SourcePcapRegisterTests(void) { SourcePcapRegisterStatsTests(); }
#endif /* UNITTESTS */
