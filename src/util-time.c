...
void TimeDeinit(void) { SCSpinDestroy(&current_time_spinlock); }

bool TimeModeIsReady(void) {
  if (live_time_tracking) return true;
...
bool TimeModeIsLive(void) { return live_time_tracking; }

void TimeSetByThread(const int thread_id, SCTime_t tv) {
  if (live_time_tracking) return;
 ...
 */
static inline void WinStrftime(const SCTime_t ts, const struct tm *t, char *str,
                               size_t size) {
  char time_fmt[64] = {0};
  char tz[6] = {0};
...
void CreateIsoTimeString(const SCTime_t ts, char *str, size_t size) {
  ...
  if (likely(t != NULL)) {
    ...
    strftime(time_fmt, sizeof(time_fmt), "%Y-%m-%dT%H:%M:%S.%%06" PRIi64 "%z",
             t);
...
void CreateFormattedTimeString(const struct tm *t, const char *fmt, char *str,
                               size_t size) {
  if (likely(t != NULL)) {
...
void CreateTimeString(const SCTime_t ts, char *str, size_t size) {
  ...
  if (likely(t != NULL)) {
    snprintf(str, size, "%02d/%02d/%02d-%02d:%02d:%02d.%06u", t->tm_mon + 1,
             t->tm_mday, t->tm_year + 1900, t->tm_hour, t->tm_min, t->tm_sec,
             (uint32_t)SCTIME_USECS(ts));
  } else {
    snprintf(str, size, "ts-error");
  }
}
...
struct tm *SCLocalTime(time_t timep, struct tm *result) {
  ...
  if (cached_minute_start[mru] == 0 && cached_minute_start[lru] == 0) {
    ...
    if (localtime_r(&timep, &cached_local_tm[lru]) == NULL) return NULL;
    /* Subtract seconds to get back to the start of the minute. */
    new_seconds = cached_local_tm[lru].tm_sec;
    cached_minute_start[lru] = timep - new_seconds;
    mru = lru;
...
static int UpdateCachedTime(int n, time_t time) {
  ...
  struct tm *t = (struct tm *)SCLocalTime(time, &local_tm);
  int cached_len =
      snprintf(cached_local_time[n], MAX_LOCAL_TIME_STRING,
               "%02d/%02d/%02d-%02d:%02d:", t->tm_mon + 1, t->tm_mday,
...
void CreateTimeString(const SCTime_t ts, char *str, size_t size) {
  ...
  if (cached_len >= (int)size) cached_len = size;
  memcpy(str, cached_str, cached_len);
  snprintf(str + cached_len, size - cached_len, "%02d.%06u", seconds,
           (uint32_t)SCTIME_USECS(ts));
}
...
#define MONTHSPERYEAR 12
  static const int mdays[MONTHSPERYEAR] = {0,   31,  59,  90,  120, 151,
  ...
  result += tp->tm_sec;
#ifndef OS_WIN32
  if (tp->tm_gmtoff) result -= tp->tm_gmtoff;
#endif
  return result;
}
...
uint64_t TimeDifferenceMicros(struct timeval t0, struct timeval t1) {
  return (uint64_t)(t1.tv_sec - t0.tv_sec) * 1000000L +
         (t1.tv_usec - t1.tv_usec);
}
