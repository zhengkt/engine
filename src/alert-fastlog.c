...
void AlertFastLogRegister(void) {
  OutputRegisterPacketModule(LOGGER_ALERT_FAST, MODULE_NAME, "fast",
                             AlertFastLogInitCtx, AlertFastLogger,
                             AlertFastLogCondition, AlertFastLogThreadInit,
...
static inline void AlertFastLogOutputAlert(AlertFastLogThread *aft,
                                           char *buffer, int alert_size) {
...
int AlertFastLogger(ThreadVars *tv, void *data, const Packet *p) {
  ...
  if (IP_GET_IPPROTO(p) == IPPROTO_ICMP ||
      IP_GET_IPPROTO(p) == IPPROTO_ICMPV6) {
  ...
  for (i = 0; i < p->alerts.cnt; i++) {
    ...
    if (likely(decoder_event == 0)) {
      PrintBufferData(alert_buffer, &size, MAX_FASTLOG_ALERT_SIZE,
                      ...
                      "]"
                      " {%s} %s:%" PRIu32 " -> %s:%" PRIu32 "\n",
                      timebuf, action, pa->s->gid, pa->s->id, pa->s->rev,
                      pa->s->msg, pa->s->class_msg, pa->s->prio, protoptr,
                      srcip, src_port_or_icmp, dstip, dst_port_or_icmp);
    } else {
      PrintBufferData(alert_buffer, &size, MAX_FASTLOG_ALERT_SIZE,
                      "%s  %s[**] [%" PRIu32 ":%" PRIu32 ":%" PRIu32
                      ...
                      "%" PRIu32 "] [**] [Raw pkt: ",
                      timebuf, action, pa->s->gid, pa->s->id, pa->s->rev,
                      pa->s->msg, pa->s->class_msg, pa->s->prio);
...
TmEcode AlertFastLogThreadInit(ThreadVars *t, const void *initdata,
                               void **data) {
