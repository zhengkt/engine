...
static TAILQ_HEAD(, OutputFreeList_)
    output_free_list = TAILQ_HEAD_INITIALIZER(output_free_list);
 ...
 */
static RunMode *RunModeGetCustomMode(enum RunModes runmode,
 ...
 */
char *RunmodeGetActive(void) { return active_runmode; }
  ...
  for (; i < RUNMODE_USER_MAX; i++) {
    ...
    for (j = 0; j < runmodes[i].cnt; j++) {
      if (mode_displayed == 1) {
        ...
        RunMode *runmode = &runmodes[i].runmodes[j];
        printf("| %-17s | %-17s | %-27s \n",
               RunModeTranslateModeToName(runmode->runmode), runmode->name,
               runmode->description);
      }
      if (mode_displayed == 0) mode_displayed = 1;
    }
    if (mode_displayed == 1) {
      printf(
          "|-----------------------------------------------------------------"
...
static const char *RunModeGetConfOrDefault(int capture_mode,
  ...
  if ((custom_mode == NULL) || (strcmp(custom_mode, "auto") == 0)) {
    switch (capture_mode) {
      ...
      case RUNMODE_PLUGIN: {
#ifdef HAVE_PLUGINS
        SCCapturePlugin *plugin =
            SCPluginFindCaptureByName(capture_plugin_name);
        if (plugin == NULL) {
          FatalError("No capture plugin found with name %s",
                     capture_plugin_name);
        }
...
void RunModeEngineIsIPS(int capture_mode, const char *runmode,
                        const char *capture_plugin_name) {
  if (runmode == NULL) {
...
void RunModeDispatch(int runmode, const char *custom_mode,
  ...
  if (custom_mode == NULL) {
    ...
    if (custom_mode == NULL) FatalError("Unknown runtime mode. Aborting");
  }
  ...
  if (local_custom_mode != NULL) SCFree(local_custom_mode);
...
void RunModeEnablesBypassManager(void) { g_runmode_needs_bypass = 1; }

int RunModeNeedsBypassManager(void) { return g_runmode_needs_bypass; }
 ...
 */
void RunModeRegisterNewRunMode(enum RunModes runmode, const char *name,
                               const char *description,
                               int (*RunModeFunc)(void),
                               void (*RunModeIsIPSEnabled)(void)) {
  if (RunModeGetCustomMode(runmode, name) != NULL) {
    FatalError(
        "runmode '%s' has already "
...
int RunModeOutputFileEnabled(void) { return file_logger_count > 0; }

int RunModeOutputFiledataEnabled(void) { return filedata_logger_count > 0; }
...
static void AddOutputToFreeList(OutputModule *module, OutputCtx *output_ctx) {
  ...
  if (unlikely(fl_output == NULL)) return;
...
static void SetupOutput(const char *name, OutputModule *module,
                        OutputCtx *output_ctx) {
  ...
  if (module->PacketLogFunc) {
    ...
    OutputRegisterPacketLogger(
        module->logger_id, module->name, module->PacketLogFunc,
    ...
    OutputRegisterTxLogger(module->logger_id, module->name, module->alproto,
                           ...
                           module->tc_log_progress, module->ts_log_progress,
                           module->TxLogCondition, module->ThreadInit,
                           module->ThreadDeinit, module->ThreadExitPrintStats);
    ...
    OutputRegisterFileLogger(
        module->logger_id, module->name, module->FileLogFunc, output_ctx,
        module->ThreadInit, module->ThreadDeinit, module->ThreadExitPrintStats);
    file_logger_count++;
  } else if (module->StreamingLogFunc) {
    SCLogDebug("%s is a streaming logger", module->name);
    OutputRegisterStreamingLogger(
        module->logger_id, module->name, module->StreamingLogFunc, output_ctx,
        module->stream_type, module->ThreadInit, module->ThreadDeinit,
        module->ThreadExitPrintStats);
  } else {
...
static void RunModeInitializeEveOutput(ConfNode *conf, OutputCtx *parent_ctx) {
  ...
  TAILQ_FOREACH(type, &types->head, next) {
    ...
    if (sub_output_config != NULL) {
      const char *enabled =
          ConfNodeLookupChildValue(sub_output_config, "enabled");
...
void RunModeInitializeOutputs(void) {
  ...
  TAILQ_FOREACH(output, &outputs->head, next) {
    ...
    if (strcmp(output->val, "file-log") == 0) {
      ...
      SCLogWarning(
          ...
          " for an explanation)");
      continue;
    } else if (strncmp(output->val, "unified2-", sizeof("unified2-") - 1) ==
               0) {
  ...
  for (a = 0; a < ALPROTO_MAX; a++) {
    if (AppLayerParserSupportsFiles(IPPROTO_TCP, a)) {
      if (g_file_logger_enabled) logger_bits[a] |= BIT_U32(LOGGER_FILE);
      if (g_filedata_logger_enabled) logger_bits[a] |= BIT_U32(LOGGER_FILEDATA);
      SCLogDebug(
          "IPPROTO_TCP::%s: g_file_logger_enabled %d g_filedata_logger_enabled "
          "%d -> "
          "%08x",
    ...
    if (logger_bits[a] == 0) continue;

    const int tcp = AppLayerParserProtocolHasLogger(IPPROTO_TCP, a) |
                    (g_file_logger_enabled) | (g_filedata_logger_enabled);
    ...
    if (tcp) AppLayerParserRegisterLoggerBits(IPPROTO_TCP, a, logger_bits[a]);
    if (udp) AppLayerParserRegisterLoggerBits(IPPROTO_UDP, a, logger_bits[a]);
  }
...
void RunModeInitialize(void) {
  ...
  if ((ConfGetBool("threading.set-cpu-affinity",
                   &threading_set_cpu_affinity)) == 0) {
    threading_set_cpu_affinity = FALSE;
  }
  ...
  if ((ConfGetFloat("threading.detect-thread-ratio",
  ...
  if ((ConfGet("threading.stack-size", &ss)) == 1) {
    if (ss != NULL) {
      if (ParseSizeStringU64(ss, &threading_set_stack_size) < 0) {
        FatalError(
            "Failed to initialize thread_stack_size output, invalid limit: %s",
            ss);
      }
    }
  }
