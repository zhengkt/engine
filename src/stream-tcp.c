...
static int StreamTcpHandleFin(ThreadVars *tv, StreamTcpThread *, TcpSession *,
                              Packet *);
...
static uint64_t ssn_pool_cnt =
    0; /** counts ssns, protected by ssn_pool_mutex */
#endif
...
void StreamTcpInitMemuse(void) { SC_ATOMIC_INIT(st_memuse); }

void StreamTcpIncrMemuse(uint64_t size) {
  (void)SC_ATOMIC_ADD(st_memuse, size);
  SCLogDebug("STREAM %" PRIu64 ", incr %" PRIu64, StreamTcpMemuseCounter(),
             size);
  return;
}
...
#endif
  SCLogDebug("STREAM %" PRIu64 ", decr %" PRIu64, StreamTcpMemuseCounter(),
             size);
  return;
}
...
static void *StreamTcpSessionPoolAlloc(void) {
  ...
  if (StreamTcpCheckMemcap((uint32_t)sizeof(TcpSession)) == 0) return NULL;
...
void StreamTcpInitConfig(bool quiet) {
  ...
  if ((ConfGetInt("stream.prealloc-sessions", &value)) == 1) {
    ...
    if (RunmodeIsUnittests()) {
      ...
      if (ConfGetNode("stream.prealloc-sessions") != NULL) {
        WarnInvalidConfEntry("stream.prealloc_sessions", "%" PRIu32,
  ...
  if (!quiet) {
    SCLogConfig("stream \"memcap\": %" PRIu64,
                SC_ATOMIC_GET(stream_config.memcap));
  }
  ...
  stream_config.ssn_memcap_policy =
      ExceptionPolicyParse("stream.memcap-policy", true);
  ...
  stream_config.midstream_policy =
      ExceptionPolicyMidstreamParse(stream_config.midstream);

  if (!quiet) {
    SCLogConfig("stream.\"inline\": %s",
                stream_config.flags & STREAMTCP_INIT_FLAG_INLINE ? "enabled"
                                                                 : "disabled");
  }
  ...
  if (!quiet) {
    SCLogConfig("stream \"max-syn-queued\": %" PRIu8,
                stream_config.max_syn_queued);
  }
  ...
  if ((ConfGetInt("stream.max-synack-queued", &value)) == 1) {
    if (value >= 0 && value <= 255) {
      ...
      stream_config.max_synack_queued =
  ...
  if (!quiet) {
    SCLogConfig("stream \"max-synack-queued\": %" PRIu8,
                stream_config.max_synack_queued);
  }

  const char *temp_stream_reassembly_memcap_str;
  if (ConfGet("stream.reassembly.memcap", &temp_stream_reassembly_memcap_str) ==
      1) {
    ...
    if (ParseSizeStringU64(temp_stream_reassembly_memcap_str,
      ...
      SC_ATOMIC_SET(stream_config.reassembly_memcap,
                    stream_reassembly_memcap_copy);
    }
  } else {
    SC_ATOMIC_SET(stream_config.reassembly_memcap,
  ...
  if (ConfGet("stream.reassembly.depth", &temp_stream_reassembly_depth_str) ==
      1) {
  ...
  if (!quiet) {
    SCLogConfig("stream.reassembly \"depth\": %" PRIu32 "",
                stream_config.reassembly_depth);
  }

  int randomize = 0;
  if ((ConfGetBool("stream.reassembly.randomize-chunk-size", &randomize)) ==
      0) {
    /* randomize by default if value not set
     * In ut mode we disable, to get predictable test results */
    if (!(RunmodeIsUnittests())) randomize = 1;
  }

  if (randomize) {
    const char *temp_rdrange;
    if (ConfGet("stream.reassembly.randomize-chunk-range", &temp_rdrange) ==
        1) {
  ...
  if (!quiet)
    SCLogConfig("stream.reassembly.raw: %s",
                enable_raw ? "enabled" : "disabled");

  /* default to true. Not many ppl (correctly) set up host-os policies, so be
   * permissive. */
  ...
  if (!quiet)
    SCLogConfig("stream.liberal-timestamps: %s",
                liberal_timestamps ? "enabled" : "disabled");
  ...
  if (RunmodeIsUnittests()) {
    ...
    if (ssn_pool == NULL) {
      ssn_pool =
          PoolThreadInit(1, /* thread */
                         0, /* unlimited */
                         stream_config.prealloc_sessions, sizeof(TcpSession),
                         StreamTcpSessionPoolAlloc, StreamTcpSessionPoolInit,
                         NULL, StreamTcpSessionPoolCleanup, NULL);
    }
    SCMutexUnlock(&ssn_pool_mutex);
  }
#endif
 ...
 */
static TcpSession *StreamTcpNewSession(ThreadVars *tv, StreamTcpThread *stt,
                                       Packet *p, int id) {
  TcpSession *ssn = (TcpSession *)p->flow->protoctx;
...
#ifdef DEBUG
    SCMutexLock(&ssn_pool_mutex);
    if (p->flow->protoctx != NULL) ssn_pool_cnt++;
    SCMutexUnlock(&ssn_pool_mutex);

    if (unlikely((g_eps_stream_ssn_memcap != UINT64_MAX &&
                  g_eps_stream_ssn_memcap == t_pcapcnt))) {
      SCLogNotice("simulating memcap reached condition for packet %" PRIu64,
                  t_pcapcnt);
      ExceptionPolicyApply(p, stream_config.ssn_memcap_policy,
...
static void StreamTcpPacketSetState(Packet *p, TcpSession *ssn, uint8_t state) {
  if (state == ssn->state || PKT_IS_PSEUDOPKT(p)) return;
...
#define StreamTcpUpdateLastAck(ssn, stream, ack)                             \
  {                                                                          \
    if (SEQ_GT((ack), (stream)->last_ack)) {                                 \
      SCLogDebug("ssn %p: last_ack set to %" PRIu32 ", moved %u forward",    \
                 (ssn), (ack), (ack) - (stream)->last_ack);                  \
      ...
      SCLogDebug("ssn %p: no update: ack %u, last_ack %" PRIu32              \
                 ", next_seq %u (state %u)",                                 \
                 (ssn), (ack), (stream)->last_ack, (stream)->next_seq,       \
                 (ssn)->state);                                              \
    }                                                                        \
  }
  ...
  {                                                                  \
    if ((ssn)->flags & STREAMTCP_FLAG_ASYNC) {                       \
      if (SEQ_GT((stream)->next_seq, (stream)->last_ack)) {          \
        ...
        SCLogDebug("ssn %p: ASYNC last_ack set to %" PRIu32          \
                   ", moved %u forward",                             \
...
#define StreamTcpUpdateNextWin(ssn, stream, win)               \
  {                                                            \
    ...
    if (SEQ_GT(((win) + sacked_size__), (stream)->next_win)) { \
      ...
      SCLogDebug("ssn %p: next_win set to %" PRIu32, (ssn),    \
                 (stream)->next_win);                          \
    }                                                          \
  }
...
static int StreamTcpPacketIsRetransmission(TcpStream *stream, Packet *p) {
  if (p->payload_len == 0) SCReturnInt(0);
  ...
  SCLogDebug("seq %u payload_len %u => %u, last_ack %u, next_seq %u",
             TCP_GET_SEQ(p), p->payload_len, (TCP_GET_SEQ(p) + p->payload_len),
             stream->last_ack, stream->next_seq);
  SCReturnInt(0);
}
...
/**
 *  \internal
 *  \brief  Function to handle the TCP_CLOSED or NONE state. The function
 * handles packets while the session state is None which means a newly
 ...
 *  \param  stt     Stream Thread module registered to handle the stream
 * handling
 *
 *  \retval 0 ok
 *  \retval -1 error
...
static int StreamTcpPacketStateNone(ThreadVars *tv, Packet *p,
  ...
  if (p->tcph->th_flags & TH_RST) {
     ...
     */
    ExceptionPolicyApply(p, stream_config.midstream_policy,
    ...
    SCLogDebug("ssn %p: ssn->client.isn %u, ssn->client.next_seq %u", ssn,
    ...
    if (TCP_HAS_WSCALE(p)) {
      ...
      ssn->server.wscale = TCP_WSCALE_MAX;
      SCLogDebug("ssn %p: wscale enabled. client %u server %u", ssn,
                 ssn->client.wscale, ssn->server.wscale);
    }
    ...
    SCLogDebug("ssn %p: ssn->server.isn %" PRIu32
               ...
               ssn, ssn->server.isn, ssn->server.next_seq,
               ssn->server.last_ack);
    ...
    if (TCP_HAS_TFO(p)) {
      ...
      if (p->payload_len) {
        StreamTcpUpdateNextSeq(ssn, &ssn->client,
                               (ssn->client.next_seq + p->payload_len));
        SCLogDebug(
            "ssn: %p (TFO) [len: %d] isn %u base_seq %u next_seq %u payload "
            "len %u",
 ...
 */
static inline void StreamTcp3whsSynAckToStateQueue(Packet *p,
                                                   TcpStateQueue *q) {
  q->flags = 0;
  q->wscale = 0;
  q->ts = 0;
  ...
  if (TCP_GET_SACKOK(p) == 1) q->flags |= STREAMTCP_QUEUE_FLAG_SACK;
 ...
 *  \retval q or NULL */
static TcpStateQueue *StreamTcp3whsFindSynAckBySynAck(TcpSession *ssn,
                                                      Packet *p) {
  ...
  while (q != NULL) {
    if (search.flags == q->flags && search.wscale == q->wscale &&
        search.win == q->win && search.seq == q->seq && search.ack == q->ack &&
        search.ts == q->ts) {
      return q;
    }
...
static int StreamTcp3whsQueueSynAck(TcpSession *ssn, Packet *p) {
  ...
  if (StreamTcp3whsFindSynAckBySynAck(ssn, p) != NULL) return 0;
...
static inline bool StateSynSentValidateTimestamp(TcpSession *ssn, Packet *p) {
  ...
  if ((receiver_stream->flags & STREAMTCP_STREAM_FLAG_TIMESTAMP) != 0) {
    if (receiver_stream->last_ts != 0 && ts_echo != 0 &&
      ...
      SCLogDebug("ssn %p: BAD TSECR echo %u recv %u", ssn, ts_echo,
...
static void TcpStateQueueInitFromSsnSyn(const TcpSession *ssn,
                                        TcpStateQueue *q) {
  ...
  SCLogDebug("ssn %p: state:%p, isn:%u/win:%u/has_ts:%s/tsval:%u", ssn, q,
             q->seq, q->win, BOOL2STR(q->flags & STREAMTCP_QUEUE_FLAG_TS),
             q->ts);
}
 ...
 *  \retval q or NULL */
static const TcpStateQueue *StreamTcp3whsFindSyn(const TcpSession *ssn,
                                                 TcpStateQueue *s) {
  SCLogDebug("ssn %p: search state:%p, isn:%u/win:%u/has_ts:%s/tsval:%u", ssn,
             s, s->seq, s->win, BOOL2STR(s->flags & STREAMTCP_QUEUE_FLAG_TS),
             s->ts);

  for (const TcpStateQueue *q = ssn->queue; q != NULL; q = q->next) {
    SCLogDebug("ssn %p: queue state:%p, isn:%u/win:%u/has_ts:%s/tsval:%u", ssn,
               q, q->seq, q->win, BOOL2STR(q->flags & STREAMTCP_QUEUE_FLAG_TS),
               q->ts);
...
static inline void StreamTcp3whsStoreSynApplyToSsn(TcpSession *ssn,
                                                   const TcpStateQueue *q) {
  if (q->flags & STREAMTCP_QUEUE_FLAG_TS) {
    ssn->client.last_pkt_ts = q->pkt_ts;
    ssn->client.last_ts = q->ts;
    ssn->client.flags |= STREAMTCP_STREAM_FLAG_TIMESTAMP;
    SCLogDebug("ssn: %p client.last_ts updated to %u", ssn,
               ssn->client.last_ts);
  }
...
static int StreamTcpPacketStateSynSent(ThreadVars *tv, Packet *p,
  ...
  SCLogDebug("ssn %p: pkt received: %s", ssn,
             PKT_IS_TOCLIENT(p) ? "toclient" : "toserver");

  /* common case: SYN/ACK from server to client */
  if ((p->tcph->th_flags & (TH_SYN | TH_ACK)) == (TH_SYN | TH_ACK) &&
      PKT_IS_TOCLIENT(p)) {
    SCLogDebug("ssn %p: SYN/ACK on SYN_SENT state for packet %" PRIu64, ssn,
               p->pcap_cnt);
    ...
    if (!(TCP_HAS_TFO(p) || (ssn->flags & STREAMTCP_FLAG_TCP_FAST_OPEN))) {
      ...
      if (SEQ_EQ(TCP_GET_ACK(p), ssn->client.next_seq)) {
        ...
        SCLogDebug("ssn %p: (TFO) next_seq reset to isn (%u)", ssn,
                   ssn->client.next_seq);
    ...
    if (ts_mismatch) {
      ...
      if (ssn->queue) {
        ...
        SCLogDebug(
            "ssn %p: found queued SYN state:%p, "
            "isn:%u/win:%u/has_ts:%s/tsval:%u",
            ssn, q, q->seq, q->win,
  ...
  /* RST */
  if (p->tcph->th_flags & TH_RST) {
    if (!StreamTcpValidateRst(ssn, p)) return -1;
    ...
    if (!stream_config.async_oneside) return 0;
...
static int StreamTcpPacketStateSynRecv(ThreadVars *tv, Packet *p,
  ...
  if (p->tcph->th_flags & TH_RST) {
    ...
    if (ssn->flags & STREAMTCP_FLAG_TIMESTAMP) {
      if (!StreamTcpValidateTimestamp(ssn, p)) return -1;
    }

    if ((StreamTcpHandleFin(tv, stt, ssn, p)) == -1) return -1;
    ...
    if (!(SEQ_EQ(TCP_GET_SEQ(p), ssn->server.isn))) {
      ...
      if (StreamTcp3whsQueueSynAck(ssn, p) == -1) return -1;
    ...
    if (ssn->queue_len) {
      ...
      if (q != NULL) {
        ...
        SCLogDebug(
            "ssn %p: none found, now checking ACK against original SYN/ACK "
            "(state)",
            ssn);
      }
    }
    ...
    if ((ssn->flags & STREAMTCP_FLAG_4WHS) && PKT_IS_TOCLIENT(p)) {
      ...
      StreamTcpUpdateNextSeq(ssn, &ssn->server,
    ...
    if ((SEQ_EQ(TCP_GET_SEQ(p), ssn->client.next_seq)) &&
      ...
      if (ack_indicates_missed_3whs_ack_packet) {
        SCLogDebug("ssn %p: packet fits perfectly after a missed 3whs-ACK",
                   ssn);
      } else {
        SCLogDebug("ssn %p: (TFO) expected packet fits perfectly after SYN/ACK",
                   ssn);
      }
 ...
 *
 *  \param  tv      Thread Variable containing  input/output queue, cpu affinity
 * etc. \param  ssn     Pointer to the current TCP session \param  p Packet
 * which has to be handled in this TCP state. \param  stt     Stream Thread
 * module registered to handle the stream handling
 */
static int HandleEstablishedPacketToServer(ThreadVars *tv, TcpSession *ssn,
                                           Packet *p, StreamTcpThread *stt) {
  SCLogDebug("ssn %p: =+ pkt (%" PRIu32 ") is to server: SEQ %" PRIu32
             ","
  ...
  if (has_ack) {
    if ((ssn->flags & STREAMTCP_FLAG_ZWP_TC) &&
        TCP_GET_ACK(p) == ssn->server.next_seq + 1) {
      SCLogDebug("ssn %p: accepting ACK as it ACKs the one byte from the ZWP",
                 ssn);
  ...
  if ((p->payload_len == 0 || p->payload_len == 1) &&
    ...
    /* normal pkt */
  } else if (!(SEQ_GEQ((TCP_GET_SEQ(p) + p->payload_len),
                       ssn->client.last_ack))) {
    if (ssn->flags & STREAMTCP_FLAG_ASYNC) {
      SCLogDebug(
      ...
      /* if last ack is beyond next_seq, we have accepted ack's for missing
       * data. In this case we do accept the data before last_ack if it is
       * (partly) beyond next seq */
    } else if (SEQ_GT(ssn->client.last_ack, ssn->client.next_seq) &&
               SEQ_GT((TCP_GET_SEQ(p) + p->payload_len),
                      ssn->client.next_seq)) {
  ...
  /* zero window probe */
  if (p->payload_len == 1 && TCP_GET_SEQ(p) == ssn->client.next_seq &&
      ssn->client.window == 0) {
  ...
  if (zerowindowprobe) {
    ...
    SCLogDebug("ack %u last_ack %u next_seq %u", TCP_GET_ACK(p),
               ssn->server.last_ack, ssn->server.next_seq);
    ...
    /* update next_win */
    StreamTcpUpdateNextWin(ssn, &ssn->server,
 ...
 * module registered to handle the stream handling
 */
static int HandleEstablishedPacketToClient(ThreadVars *tv, TcpSession *ssn,
                                           Packet *p, StreamTcpThread *stt) {
  ...
  if (has_ack) {
    if ((ssn->flags & STREAMTCP_FLAG_ZWP_TS) &&
        TCP_GET_ACK(p) == ssn->client.next_seq + 1) {
      SCLogDebug("ssn %p: accepting ACK as it ACKs the one byte from the ZWP",
                 ssn);
      StreamTcpSetEvent(p, STREAM_EST_ACK_ZWP_DATA);
  ...
  int zerowindowprobe = 0;
  /* zero window probe */
  if (p->payload_len == 1 && TCP_GET_SEQ(p) == ssn->server.next_seq &&
      ssn->server.window == 0) {
    SCLogDebug("ssn %p: zero window probe", ssn);
    zerowindowprobe = 1;
    STREAM_PKT_FLAG_SET(p, STREAM_PKT_FLAG_TCP_ZERO_WIN_PROBE);
  ...
  if (zerowindowprobe) {
    ...
    StreamTcpUpdateNextWin(ssn, &ssn->client,
                           (ssn->client.last_ack + ssn->client.window));
 ...
 *  \internal
 *
 *  \brief Find the highest sequence number needed to consider all segments as
 * ACK'd
 *
 ...
 */
static inline uint32_t StreamTcpResetGetMaxAck(TcpStream *stream,
                                               uint32_t seq) {
  uint32_t ack = seq;
...
static bool StreamTcpPacketIsZeroWindowProbeAck(const TcpSession *ssn,
                                                const Packet *p) {
  if (ssn->state < TCP_ESTABLISHED) return false;
  ...
  if (PKT_IS_TOCLIENT(p)) {
    snd = &ssn->server;
    rcv = &ssn->client;
    if (!(ssn->flags & STREAMTCP_FLAG_ZWP_TS)) return false;
  } else {
    snd = &ssn->client;
    rcv = &ssn->server;
    if (!(ssn->flags & STREAMTCP_FLAG_ZWP_TC)) return false;
  }

  const uint32_t pkt_win = TCP_GET_WINDOW(p) << snd->wscale;
  if (pkt_win != 0) return false;
  if (pkt_win != rcv->window) return false;

  if (TCP_GET_SEQ(p) != snd->next_seq) return false;
  if (TCP_GET_ACK(p) != rcv->last_ack) return false;
  SCLogDebug("ssn %p: packet %" PRIu64 " is a Zero Window Probe ACK", ssn,
             p->pcap_cnt);
  return true;
}
...
static bool StreamTcpPacketIsDupAck(const TcpSession *ssn, const Packet *p) {
  ...
  const uint32_t pkt_win = TCP_GET_WINDOW(p) << snd->wscale;
  if (pkt_win == 0 || rcv->window == 0) return false;
  if (pkt_win != rcv->window) return false;

  if (TCP_GET_SEQ(p) != snd->next_seq) return false;
  if (TCP_GET_ACK(p) != rcv->last_ack) return false;

  SCLogDebug("ssn %p: packet:%" PRIu64
             " seq:%u ack:%u win:%u snd %u:%u:%u rcv %u:%u:%u",
             ssn, p->pcap_cnt, TCP_GET_SEQ(p), TCP_GET_ACK(p), pkt_win,
             snd->next_seq, snd->last_ack, rcv->window, snd->next_seq,
             rcv->last_ack, rcv->window);
  return true;
}
...
static bool StreamTcpPacketIsOutdatedAck(TcpSession *ssn, Packet *p) {
  ...
  if (PKT_IS_TOSERVER(p)) {
    if (SEQ_EQ(TCP_GET_SEQ(p), ssn->client.next_seq) &&
      ...
      if (!TCP_HAS_SACK(p)) {
        SCLogDebug("outdated ACK (no SACK, SEQ %u vs next_seq %u)",
                   TCP_GET_SEQ(p), ssn->client.next_seq);
        return true;
      }

      if (StreamTcpSackPacketIsOutdated(&ssn->server, p)) {
        SCLogDebug("outdated ACK (have SACK, SEQ %u vs next_seq %u)",
                   TCP_GET_SEQ(p), ssn->client.next_seq);
        return true;
      }
    }
 ...
 */
static int StreamTcpPacketIsSpuriousRetransmission(const TcpSession *ssn,
                                                   Packet *p) {
  ...
  if (!(stream->flags & STREAMTCP_STREAM_FLAG_NOREASSEMBLY)) {
    if ((SEQ_LEQ(TCP_GET_SEQ(p) + p->payload_len, stream->base_seq))) {
      SCLogDebug(
          "ssn %p: spurious retransmission; packet entirely before base_seq: "
          "SEQ %u(%u) "
          "last_ack %u base_seq %u",
          ssn, TCP_GET_SEQ(p), TCP_GET_SEQ(p) + p->payload_len,
  ...
  if ((SEQ_LEQ(TCP_GET_SEQ(p) + p->payload_len, stream->last_ack))) {
    SCLogDebug(
        "ssn %p: spurious retransmission; packet entirely before last_ack: SEQ "
        "%u(%u) "
        "last_ack %u",
  ...
  SCLogDebug(
      "ssn %p: NOT spurious retransmission; packet NOT entirely before "
      "last_ack: SEQ "
 ...
 *
 *  \param  tv      Thread Variable containing  input/output queue, cpu affinity
 * etc. \param  p       Packet which has to be handled in this TCP state. \param
 * stt     Stream Thread module registered to handle the stream handling
 */
...
static int StreamTcpPacketStateEstablished(ThreadVars *tv, Packet *p,
  ...
  if (p->tcph->th_flags & TH_RST) {
    ...
    if (PKT_IS_TOSERVER(p)) {
      ...
      if ((p->tcph->th_flags & TH_ACK) &&
          StreamTcpValidateAck(ssn, &ssn->server, p) == 0)
      ...
      StreamTcpReassembleHandleSegment(tv, stt->ra_ctx, ssn, &ssn->client, p);
      SCLogDebug("ssn %p: =+ next SEQ %" PRIu32
                 ", last ACK "
                 "%" PRIu32 "",
      ...
      if ((p->tcph->th_flags & TH_ACK) &&
          StreamTcpValidateAck(ssn, &ssn->client, p) == 0)
        StreamTcpUpdateLastAck(
            ssn, &ssn->client,
...
static int StreamTcpHandleFin(ThreadVars *tv, StreamTcpThread *stt,
  ...
  if (PKT_IS_TOSERVER(p)) {
    ...
    SCLogDebug("ssn %p: -> SEQ %u, re %u. last_ack %u next_win %u", ssn,
               TCP_GET_SEQ(p), pkt_re, ssn->client.last_ack,
               ssn->client.next_win);
    ...
    const uint32_t pkt_re = TCP_GET_SEQ(p) + p->payload_len;
    SCLogDebug("ssn %p: -> SEQ %u, re %u. last_ack %u next_win %u", ssn,
               TCP_GET_SEQ(p), pkt_re, ssn->server.last_ack,
               ssn->server.next_win);
    ...
    /* if we accept the FIN, next_seq needs to reflect the FIN */
    ssn->server.next_seq = TCP_GET_SEQ(p) + p->payload_len + 1;
    SCLogDebug("ssn %p: ssn->server.next_seq %" PRIu32 " updated", ssn,
               ssn->server.next_seq);
  ...
  if (p->tcph->th_flags & TH_RST) {
    ...
    if (PKT_IS_TOSERVER(p)) {
      ...
      if (SEQ_LT(TCP_GET_ACK(p), ssn->server.next_seq)) {
        SCLogDebug("ssn %p: ACK's older segment as %u < %u", ssn,
                   TCP_GET_ACK(p), ssn->server.next_seq);
        ...
        if (SEQ_EQ(TCP_GET_ACK(p), ssn->server.next_seq)) {
          if (SEQ_LEQ(TCP_GET_SEQ(p) + p->payload_len, ssn->client.next_win) ||
              (ssn->flags &
               (STREAMTCP_FLAG_MIDSTREAM | STREAMTCP_FLAG_ASYNC))) {
            SCLogDebug("ssn %p: seq %" PRIu32
                       " in window, ssn->client.next_win "
                       "%" PRIu32 "",
                       ssn, TCP_GET_SEQ(p), ssn->client.next_win);
            SCLogDebug("seq %u client.next_seq %u", TCP_GET_SEQ(p),
                       ssn->client.next_seq);
...
static int StreamTcpPacketStateCloseWait(ThreadVars *tv, Packet *p,
  ...
  if (p->tcph->th_flags & TH_RST) {
    ...
    if (ssn->flags & STREAMTCP_FLAG_TIMESTAMP) {
      if (!StreamTcpValidateTimestamp(ssn, p)) SCReturnInt(-1);
    }
...
static int StreamTcpPacketStateLastAck(ThreadVars *tv, Packet *p,
  ...
  if (p->tcph->th_flags & TH_RST) {
    ...
    if (PKT_IS_TOSERVER(p)) {
      ...
      if (!retransmission) {
        if (SEQ_LT(TCP_GET_SEQ(p), ssn->client.next_seq)) {
          SCLogDebug("ssn %p: not updating state as packet is before next_seq",
                     ssn);
...
static int StreamTcpPacketStateClosed(ThreadVars *tv, Packet *p,
  ...
  if ((stream->flags & STREAMTCP_STREAM_FLAG_RST_RECV) == 0) {
    if (ostream->flags & STREAMTCP_STREAM_FLAG_RST_RECV) {
      if (StreamTcpStateDispatch(tv, p, stt, ssn, ssn->pstate) < 0) return -1;
      /* if state is still "closed", it wasn't updated by our dispatch. */
      if (ssn->state == TCP_CLOSED) ssn->state = ssn->pstate;
    }
  }
  return 0;
}
...
static void StreamTcpPacketCheckPostRst(TcpSession *ssn, Packet *p) {
  ...
  if (ostream->flags & STREAMTCP_STREAM_FLAG_RST_RECV) {
    SCLogDebug("regular packet %" PRIu64
               " from same sender as "
...
static int StreamTcpPacketIsKeepAlive(TcpSession *ssn, Packet *p) {
  if (p->flags & PKT_PSEUDO_STREAM_END) return 0;
  ...
  SCLogDebug("seq %u (%u), ack %u (%u)", seq, (stream->next_seq - 1), ack,
             ostream->last_ack);
  return 0;
}
...
static int StreamTcpPacketIsKeepAliveACK(TcpSession *ssn, Packet *p) {
  ...
  if (pkt_win != ostream->window) return 0;

  if ((ostream->flags & STREAMTCP_STREAM_FLAG_KEEPALIVE) &&
      ack == ostream->last_ack && seq == stream->next_seq) {
  ...
  SCLogDebug(
      "seq %u (%u), ack %u (%u) FLAG_KEEPALIVE: %s", seq, stream->next_seq, ack,
      ostream->last_ack,
...
static void StreamTcpClearKeepAliveFlag(TcpSession *ssn, Packet *p) {
  ...
  if (p->flags & PKT_PSEUDO_STREAM_END) return;
...
static int StreamTcpPacketIsWindowUpdate(TcpSession *ssn, Packet *p) {
  ...
  SCLogDebug("seq %u (%u), ack %u (%u)", seq, stream->next_seq, ack,
             ostream->last_ack);
  return 0;
}
  ...
  if (pkt_win < ostream->window) {
    ...
    if (diff > p->payload_len && SEQ_GT(ack, ostream->next_seq) &&
      ...
      SCLogDebug("%" PRIu64
                 ...
                 p->pcap_cnt, seq, ack, ostream->next_seq, ostream->last_ack,
      ...
      if (((pkt_win > 1024) && (diff > (adiff + 32))) ||
        ...
        SCLogDebug("%u - %u = %u (state %u)", diff, adiff, diff - adiff,
                   ssn->state);
...
static inline void HandleThreadId(ThreadVars *tv, Packet *p,
                                  StreamTcpThread *stt) {
  ...
  if (unlikely(p->flow->thread_id[idx] == 0)) {
    ...
    SCLogDebug("wrong thread: flow has %u, we are %d", p->flow->thread_id[idx],
               tv->id);
...
int StreamTcpPacket(ThreadVars *tv, Packet *p, StreamTcpThread *stt,
  ...
  if (ssn != NULL) {
    ...
    if (ssn->flags & STREAMTCP_FLAG_ASYNC && ssn->client.tcp_flags != 0 &&
        ssn->server.tcp_flags != 0) {
      SCLogDebug("ssn %p: removing ASYNC flag as we have packets on both sides",
                 ssn);
      ssn->flags &= ~STREAMTCP_FLAG_ASYNC;
    }
  }
  ...
  if (ssn == NULL || ssn->state == TCP_NONE) {
    ...
    if (ssn != NULL) SCLogDebug("ssn->alproto %" PRIu16 "", p->flow->alproto);
  } else {
    ...
    if (StreamTcpPacketIsFinShutdownAck(ssn, p) == 0) {
      if (StreamTcpPacketIsWindowUpdate(ssn, p) == 0) {
        if (StreamTcpPacketIsBadWindowUpdate(ssn, p)) goto skip;
        if (StreamTcpPacketIsOutdatedAck(ssn, p)) goto skip;
      }
    }
    ...
    if (StreamTcpStateDispatch(tv, p, stt, ssn, ssn->state) < 0) goto error;
...
static inline int StreamTcpValidateChecksum(Packet *p) {
  ...
  if (p->flags & PKT_IGNORE_CHECKSUM) return ret;
...
static int TcpSessionPacketIsStreamStarter(const Packet *p) {
  ...
  if ((p->tcph->th_flags & (TH_SYN | TH_ACK)) == TH_SYN) {
    SCLogDebug("packet %" PRIu64 " is a stream starter: %02x", p->pcap_cnt,
               p->tcph->th_flags);
    return 1;
  }
...
static int TcpSessionReuseDoneEnoughSyn(const Packet *p, const Flow *f,
  ...
  if (FlowGetPacketDirection(f, p) == TOSERVER) {
    if (ssn == NULL) {
      ...
      SCLogDebug("steam starter packet %" PRIu64 ", ssn %p null. Reuse.",
                 p->pcap_cnt, ssn);
      return 1;
    }
 ...
 */
static int TcpSessionReuseDoneEnoughSynAck(const Packet *p, const Flow *f,
                                           const TcpSession *ssn) {
...
static int TcpSessionReuseDoneEnough(const Packet *p, const Flow *f,
                                     const TcpSession *ssn) {
  if ((p->tcph->th_flags & (TH_SYN | TH_ACK)) == TH_SYN) {
...
int TcpSessionPacketSsnReuse(const Packet *p, const Flow *f,
                             const void *tcp_ssn) {
...
TmEcode StreamTcp(ThreadVars *tv, Packet *p, void *data,
                  PacketQueueNoLock *pq) {
  ...
  SCLogDebug("p->pcap_cnt %" PRIu64 " direction %s pkt_src %s", p->pcap_cnt,
             p->flow
                 ? (FlowGetPacketDirection(p->flow, p) == TOSERVER ? "toserver"
                                                                   : "toclient")
                 : "noflow",
...
TmEcode StreamTcpThreadInit(ThreadVars *tv, void *initdata, void **data) {
  ...
  if (unlikely(stt == NULL)) SCReturnInt(TM_ECODE_FAILED);
  ...
  *data = (void *)stt;
  ...
  stt->counter_tcp_active_sessions =
      StatsRegisterCounter("tcp.active_sessions", tv);
  ...
  stt->counter_tcp_ssn_from_cache =
      StatsRegisterCounter("tcp.ssn_from_cache", tv);
  stt->counter_tcp_ssn_from_pool =
      StatsRegisterCounter("tcp.ssn_from_pool", tv);
  stt->counter_tcp_pseudo = StatsRegisterCounter("tcp.pseudo", tv);
  stt->counter_tcp_pseudo_failed =
      StatsRegisterCounter("tcp.pseudo_failed", tv);
  stt->counter_tcp_invalid_checksum =
      StatsRegisterCounter("tcp.invalid_checksum", tv);
  stt->counter_tcp_midstream_pickups =
      StatsRegisterCounter("tcp.midstream_pickups", tv);
  stt->counter_tcp_wrong_thread =
      StatsRegisterCounter("tcp.pkt_on_wrong_thread", tv);
  stt->counter_tcp_ack_unseen_data =
      StatsRegisterCounter("tcp.ack_unseen_data", tv);

  /* init reassembly ctx */
  stt->ra_ctx = StreamTcpReassembleInitThreadCtx(tv);
  if (stt->ra_ctx == NULL) SCReturnInt(TM_ECODE_FAILED);

  stt->ra_ctx->counter_tcp_segment_memcap =
      StatsRegisterCounter("tcp.segment_memcap_drop", tv);
  ...
  stt->ra_ctx->counter_tcp_segment_from_pool =
      StatsRegisterCounter("tcp.segment_from_pool", tv);
  stt->ra_ctx->counter_tcp_stream_depth =
      StatsRegisterCounter("tcp.stream_depth_reached", tv);
  stt->ra_ctx->counter_tcp_reass_gap =
      StatsRegisterCounter("tcp.reassembly_gap", tv);
  stt->ra_ctx->counter_tcp_reass_overlap =
      StatsRegisterCounter("tcp.overlap", tv);
  ...
  if (ssn_pool == NULL) {
    ssn_pool =
        PoolThreadInit(1, /* thread */
                       ...
                       NULL, StreamTcpSessionPoolCleanup, NULL);
    stt->ssn_pool_id = 0;
    SCLogDebug("pool size %d, thread ssn_pool_id %d", PoolThreadSize(ssn_pool),
               stt->ssn_pool_id);
  } else {
...
static int StreamTcpValidateRst(TcpSession *ssn, Packet *p) {
  ...
  /* Set up the os_policy to be used in validating the RST packets based on
     target system */
  if (PKT_IS_TOSERVER(p)) {
    if (ssn->server.os_policy == 0) StreamTcpSetOSPolicy(&ssn->server, p);
    ...
    if (ssn->client.os_policy == 0) StreamTcpSetOSPolicy(&ssn->client, p);
  ...
  /* RFC 2385 md5 signature header or RFC 5925 TCP AO headerpresent. Since we
   * can't validate these (requires key that is set/transferred out of band), we
   * can't know if the RST will be accepted or rejected by the end host. We
   * accept it, but keep tracking if the sender of it ignores it, which would be
   * a sign of injection. */
  if (p->tcpvars.md5_option_present || p->tcpvars.ao_option_present) {
    ...
    SCLogDebug(
        "ssn %p: setting STREAMTCP_STREAM_FLAG_RST_RECV on receiver stream",
        ssn);
...
static int StreamTcpValidateTimestamp(TcpSession *ssn, Packet *p) {
  ...
  if (TCP_HAS_TS(p)) {
    ...
    if (receiver_stream->os_policy == OS_POLICY_HPUX11) {
      ...
      if (!SEQ_EQ(sender_stream->next_seq, TCP_GET_SEQ(p))) check_ts = 0;
    }
    ...
    if (check_ts) {
      ...
      if (receiver_stream->os_policy == OS_POLICY_LINUX ||
          stream_config.liberal_timestamps) {
      ...
      if (result < 0) {
        SCLogDebug(
            "timestamp is not valid last_ts "
            "%" PRIu32 " p->tcpvars->ts %" PRIu32
            " result "
            "%" PRId32 "",
            last_ts, ts, result);
...
static int StreamTcpHandleTimestamp(TcpSession *ssn, Packet *p) {
  ...
  if (TCP_HAS_TS(p)) {
    ...
    if (check_ts) {
      ...
      SCLogDebug("ts %" PRIu32 ", last_ts %" PRIu32 "", ts,
                 sender_stream->last_ts);
 ...
 */
static inline int StreamTcpValidateAck(TcpSession *ssn, TcpStream *stream,
                                       Packet *p) {
  SCEnter();

  if (!(p->tcph->th_flags & TH_ACK)) SCReturnInt(0);

  const uint32_t ack = TCP_GET_ACK(p);

  /* fast track */
  if (SEQ_GT(ack, stream->last_ack) && SEQ_LEQ(ack, stream->next_win)) {
    SCLogDebug("ssn %p: ACK %u in bounds > %u <= %u", ssn, ack,
               stream->last_ack, stream->next_win);
    SCReturnInt(0);
  }
  /* fast track */
  else if (SEQ_EQ(ack, stream->last_ack)) {
    SCLogDebug("ssn %p: pkt ACK %" PRIu32 " == stream last ACK %" PRIu32, ssn,
               TCP_GET_ACK(p), stream->last_ack);
    SCReturnInt(0);
  }

  /* exception handling */
  if (SEQ_LT(ack, stream->last_ack)) {
    SCLogDebug("pkt ACK %" PRIu32 " < stream last ACK %" PRIu32, TCP_GET_ACK(p),
               stream->last_ack);
    ...
    if (stream->window != 0 &&
        SEQ_LT(ack, (stream->last_ack - stream->window))) {
      SCLogDebug("ACK %" PRIu32 " is before last_ack %" PRIu32
                 " - window "
                 "%" PRIu32 " = %" PRIu32,
                 ack, stream->last_ack, stream->window,
                 stream->last_ack - stream->window);
      goto invalid;
    }
  ...
  if (ssn->state > TCP_SYN_SENT && SEQ_GT(ack, stream->next_win)) {
    SCLogDebug("ACK %" PRIu32 " is after next_win %" PRIu32, ack,
               stream->next_win);
    goto invalid;
    /* a toclient RST as a response to SYN, next_win is 0, ack will be isn+1,
     * just like the syn ack */
  } else if (ssn->state == TCP_SYN_SENT && PKT_IS_TOCLIENT(p) &&
             p->tcph->th_flags & TH_RST && SEQ_EQ(ack, stream->isn + 1)) {
    SCReturnInt(0);
  }

  SCLogDebug("default path leading to invalid: ACK %" PRIu32
             ", last_ack %" PRIu32 " next_win %" PRIu32,
 ...
 *  \param dir 0 ts 1 tc
 */
static void StreamTcpPseudoPacketCreateDetectLogFlush(
    ThreadVars *tv, StreamTcpThread *stt, Packet *parent, TcpSession *ssn,
    PacketQueueNoLock *pq, int dir) {
  SCEnter();
  Flow *f = parent->flow;
 ...
 */
void StreamTcpDetectLogFlush(ThreadVars *tv, StreamTcpThread *stt, Flow *f,
 ...
 * \var flag determines the direction to run callback on (either to server or to
 * client).
 *
...
int StreamTcpSegmentForEach(const Packet *p, uint8_t flag,
                            StreamSegmentCallback CallbackFunc, void *data) {
  TcpStream *stream = NULL;
  int cnt = 0;
  ...
  RB_FOREACH(seg, TCPSEG, &stream->seg_tree) {
    ...
    StreamingBufferSegmentGetData(&stream->sb, &seg->sbseg, &seg_data,
                                  &seg_datalen);
...
/**
 * \brief Run callback function on each TCP segment in both directions of a
 * session.
 *
 * \note when stream engine is running in inline mode all segments are used,
 *       in IDS/non-inline mode only ack'd segments are iterated.
 ...
 *
 */
int StreamTcpSegmentForSession(const Packet *p, uint8_t flag,
                               StreamSegmentCallback CallbackFunc, void *data) {
  int ret = 0;
  int cnt = 0;
  ...
  while (server_node != NULL || client_node != NULL) {
    ...
    if (server_node == NULL) {
       ...
       * side.
       */
      StreamingBufferSegmentGetData(&client_stream->sb, &client_node->sbseg,
                                    &seg_data, &seg_datalen);
       ...
       * side.
       */
      StreamingBufferSegmentGetData(&server_stream->sb, &server_node->sbseg,
                                    &seg_data, &seg_datalen);
      ...
      if (TimevalEarlier(&client_node->pcap_hdr_storage->ts,
