...
char *RunmodeAutoFpCreatePickupQueuesString(int n) {
  ...
  for (int thread = 0; thread < n; thread++) {
    if (strlen(queues) > 0) strlcat(queues, ",", queues_size);
...
int RunModeSetLiveCaptureAutoFp(ConfigIfaceParserFunc ConfigParser,
                                ...
                                const char *thread_name, const char *live_dev) {
  ...
  if ((nlive <= 1) && (live_dev != NULL)) {
    ...
    SCLogInfo("Going to use %" PRId32 " %s receive thread(s)", threads_count,
              recv_mod_name);
    ...
    for (int lthread = 0; lthread < nlive; lthread++) {
      ...
      if (aconf == NULL) {
        FatalError("Multidev: Failed to allocate config for %s (%d)", dev,
                   lthread);
      }
...
/**
 */
static int RunModeSetLiveCaptureWorkersForDevice(
    ...
    void *aconf, unsigned char single_mode) {
  int threads_count;
  uint16_t thread_max = TmThreadsGetWorkerThreadMax();
  ...
  for (int thread = 0; thread < threads_count; thread++) {
    ...
    if (single_mode) {
      ...
      snprintf(printable_threadname, strlen(thread_name) + 5 + strlen(live_dev),
               "%s#01-%s", thread_name, live_dev);
    } else {
      snprintf(tname, sizeof(tname), "%s#%02d-%s", thread_name, thread + 1,
               visual_devname);
      snprintf(printable_threadname, strlen(thread_name) + 5 + strlen(live_dev),
               "%s#%02d-%s", thread_name, thread + 1, live_dev);
    }
    ThreadVars *tv =
...
int RunModeSetLiveCaptureWorkers(ConfigIfaceParserFunc ConfigParser,
  ...
  for (ldev = 0; ldev < nlive; ldev++) {
    ...
    RunModeSetLiveCaptureWorkersForDevice(ModThreadsCount, recv_mod_name,
                                          decode_mod_name, thread_name,
                                          live_dev_c, aconf, 0);
  }
...
int RunModeSetLiveCaptureSingle(ConfigIfaceParserFunc ConfigParser,
  ...
  return RunModeSetLiveCaptureWorkersForDevice(ModThreadsCount, recv_mod_name,
                                               decode_mod_name, thread_name,
                                               live_dev_c, aconf, 1);
}
...
int RunModeSetIPSAutoFp(ConfigIPSParserFunc ConfigParser,
                        const char *recv_mod_name, const char *verdict_mod_name,
                        const char *decode_mod_name) {
  SCEnter();
  char tname[TM_THREAD_NAME_MAX];
  TmModule *tm_module;
  ...
  for (int i = 0; i < nqueue; i++) {
    ...
    ThreadVars *tv_verdict =
        TmThreadCreatePacketHandler(tname, "verdict-queue", "simple",
