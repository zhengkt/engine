 ...
 */
static int ENIPGetAlstateProgress(void *tx, uint8_t direction) { return 1; }
...
static void *ENIPGetTx(void *alstate, uint64_t tx_id) {
  ...
  if (enip->curr && enip->curr->tx_num == tx_id + 1) return enip->curr;

  TAILQ_FOREACH(tx, &enip->tx_list, next) {
    if (tx->tx_num != (tx_id + 1)) continue;
...
static int ENIPStateGetEventInfo(const char *event_name, int *event_id,
...
static void ENIPTransactionFree(ENIPTransaction *tx, ENIPState *state) {
  ...
  if (state->iter == tx) state->iter = NULL;
...
static ENIPTransaction *ENIPTransactionAlloc(ENIPState *state) {
  ...
  if (unlikely(tx == NULL)) return NULL;
...
static void ENIPStateTransactionFree(void *state, uint64_t tx_id) {
  ...
  TAILQ_FOREACH(tx, &enip_state->tx_list, next) {
    ...
    if (tx == enip_state->curr) enip_state->curr = NULL;
 ...
 */
static AppLayerResult ENIPParse(Flow *f, void *state,
  ...
  while (input_len > 0) {
    ...
    if (tx == NULL) SCReturnStruct(APP_LAYER_OK);
...
static AppLayerResult ENIPParseRequest(Flow *f, void *state,
                                       AppLayerParserState *pstate,
...
static AppLayerResult ENIPParseResponse(Flow *f, void *state,
                                        AppLayerParserState *pstate,
                                        StreamSlice stream_slice,
                                        void *local_data) {
...
static uint16_t ENIPProbingParser(Flow *f, uint8_t direction,
  ...
  int ret = ByteExtractUint32(&status, BYTE_LITTLE_ENDIAN, sizeof(uint32_t),
  ...
  ret = ByteExtractUint16(&cmd, BYTE_LITTLE_ENDIAN, sizeof(uint16_t),
                          (const uint8_t *)(input));
  if (ret < 0) {
    return ALPROTO_FAILED;
  }
  ...
  switch (cmd) {
    ...
    case LIST_INTERFACES:
      ...
      ret = ByteExtractUint16(&nbitems, BYTE_LITTLE_ENDIAN, sizeof(uint16_t),
                              (const uint8_t *)(input));
      if (ret < 0) {
        return ALPROTO_FAILED;
      }
...
static AppLayerGetTxIterTuple ENIPGetTxIterator(
...
void RegisterENIPUDPParsers(void) {
  ...
  if (AppLayerProtoDetectConfProtoDetectionEnabledDefault("udp", proto_name,
                                                          false)) {
    ...
    if (RunmodeIsUnittests()) {
      AppLayerProtoDetectPPRegister(IPPROTO_UDP, "44818", ALPROTO_ENIP, 0,
                                    sizeof(ENIPEncapHdr), STREAM_TOSERVER,
                                    ENIPProbingParser, NULL);

      AppLayerProtoDetectPPRegister(IPPROTO_UDP, "44818", ALPROTO_ENIP, 0,
                                    sizeof(ENIPEncapHdr), STREAM_TOCLIENT,
                                    ENIPProbingParser, NULL);

    } else {
      if (!AppLayerProtoDetectPPParseConfPorts(
              "udp", IPPROTO_UDP, proto_name, ALPROTO_ENIP, 0,
              sizeof(ENIPEncapHdr), ENIPProbingParser, ENIPProbingParser)) {
        SCLogDebug(
  ...
  if (AppLayerParserConfParserEnabled("udp", proto_name)) {
    AppLayerParserRegisterParser(IPPROTO_UDP, ALPROTO_ENIP, STREAM_TOSERVER,
                                 ENIPParseRequest);
    AppLayerParserRegisterParser(IPPROTO_UDP, ALPROTO_ENIP, STREAM_TOCLIENT,
                                 ENIPParseResponse);
    ...
    AppLayerParserRegisterGetTxIterator(IPPROTO_UDP, ALPROTO_ENIP,
                                        ENIPGetTxIterator);
    AppLayerParserRegisterTxDataFunc(IPPROTO_UDP, ALPROTO_ENIP, ENIPGetTxData);
    AppLayerParserRegisterStateDataFunc(IPPROTO_UDP, ALPROTO_ENIP,
                                        ENIPGetStateData);
    ...
    AppLayerParserRegisterGetEventInfo(IPPROTO_UDP, ALPROTO_ENIP,
                                       ENIPStateGetEventInfo);
    AppLayerParserRegisterGetEventInfoById(IPPROTO_UDP, ALPROTO_ENIP,
                                           ENIPStateGetEventInfoById);
...
#ifdef UNITTESTS
  AppLayerParserRegisterProtocolUnittests(IPPROTO_UDP, ALPROTO_ENIP,
...
void RegisterENIPTCPParsers(void) {
  ...
  if (AppLayerProtoDetectConfProtoDetectionEnabledDefault("tcp", proto_name,
                                                          false)) {
    AppLayerProtoDetectRegisterProtocol(ALPROTO_ENIP, proto_name);
    ...
    if (RunmodeIsUnittests()) {
      ...
      if (!AppLayerProtoDetectPPParseConfPorts(
              "tcp", IPPROTO_TCP, proto_name, ALPROTO_ENIP, 0,
              sizeof(ENIPEncapHdr), ENIPProbingParser, ENIPProbingParser)) {
        return;
      }
    }
  ...
  if (AppLayerParserConfParserEnabled("tcp", proto_name)) {
    AppLayerParserRegisterParser(IPPROTO_TCP, ALPROTO_ENIP, STREAM_TOSERVER,
                                 ENIPParseRequest);
    AppLayerParserRegisterParser(IPPROTO_TCP, ALPROTO_ENIP, STREAM_TOCLIENT,
                                 ENIPParseResponse);
    ...
    AppLayerParserRegisterGetTxIterator(IPPROTO_TCP, ALPROTO_ENIP,
                                        ENIPGetTxIterator);
    AppLayerParserRegisterTxDataFunc(IPPROTO_TCP, ALPROTO_ENIP, ENIPGetTxData);
    AppLayerParserRegisterStateDataFunc(IPPROTO_TCP, ALPROTO_ENIP,
                                        ENIPGetStateData);
    ...
    AppLayerParserRegisterGetStateProgressFunc(IPPROTO_TCP, ALPROTO_ENIP,
                                               ENIPGetAlstateProgress);
    AppLayerParserRegisterStateProgressCompletionStatus(ALPROTO_ENIP, 1, 1);

    AppLayerParserRegisterGetEventInfo(IPPROTO_TCP, ALPROTO_ENIP,
                                       ENIPStateGetEventInfo);
...
#ifdef UNITTESTS
  AppLayerParserRegisterProtocolUnittests(IPPROTO_TCP, ALPROTO_ENIP,
                                          ENIPParserRegisterTests);
#endif
...
static uint8_t listIdentity[] = {/* List ID */ 0x63,
                                 0x00,
                                 /* Length */ 0x00,
                                 0x00,
                                 /* Session */ 0x00,
