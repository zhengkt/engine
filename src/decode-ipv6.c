 ...
 *
 */
static void DecodeIPv4inIPv6(ThreadVars *tv, DecodeThreadVars *dtv, Packet *p,
  ...
  if (IP_GET_RAW_VER(pkt) == 4) {
    Packet *tp =
        PacketTunnelPktSetup(tv, dtv, p, pkt, plen, DECODE_TUNNEL_IPV4);
    if (tp != NULL) {
...
static int DecodeIP6inIP6(ThreadVars *tv, DecodeThreadVars *dtv, Packet *p,
  ...
  if (IP_GET_RAW_VER(pkt) == 6) {
    Packet *tp =
        PacketTunnelPktSetup(tv, dtv, p, pkt, plen, DECODE_TUNNEL_IPV6);
    if (tp != NULL) {
      PKT_SET_SRC(tp, PKT_SRC_DECODER_IPV6);
      PacketEnqueueNoLock(&tv->decode_pq, tp);
...
static inline
#endif
    void
    DecodeIPV6FragHeader(Packet *p, const uint8_t *pkt, uint16_t hdrextlen,
  ...
  SCLogDebug("IPV6 FH: offset %u, mf %s, nh %u, id %u/%x", p->ip6eh.fh_offset,
             p->ip6eh.fh_more_frags_set ? "true" : "false", p->ip6eh.fh_nh,
...
static void DecodeIPV6ExtHdrs(ThreadVars *tv, DecodeThreadVars *dtv, Packet *p,
  ...
  while (1) {
    ...
    switch (nh) {
      ...
      case IPPROTO_ROUTING:
        IPV6_SET_L4PROTO(p, nh);
        hdrextlen =
            8 + (*(pkt + 1) * 8); /* 8 bytes + length in 8 octet units */
      ...
      case IPPROTO_DSTOPTS: {
        ...
        while (offset < optslen) {
          ...
          if (*ptr == IPV6OPT_PADN) /* PadN */
          {
            ...
            if (ip6_optlen == 0) ENGINE_SET_EVENT(p, IPV6_EXTHDR_ZERO_LEN_PADN);
          } else if (*ptr == IPV6OPT_RA) /* RA */
          {
            ...
            // printf("RA option: type %" PRIu32 " len %" PRIu32 " value %"
            // PRIu32 "\n",
          ...
          } else if (*ptr == IPV6OPT_JUMBO) /* Jumbo */
          {
            ...
            // printf("Jumbo option: type %" PRIu32 " len %" PRIu32 " payload
            // len %" PRIu32 "\n",
      ...
      case IPPROTO_AH: {
        ...
        if (*(pkt + 1) > 0) hdrextlen += ((*(pkt + 1) - 1) * 4);
...
static int DecodeIPV6Packet(ThreadVars *tv, DecodeThreadVars *dtv, Packet *p,
                            const uint8_t *pkt, uint16_t len) {
...
int DecodeIPV6(ThreadVars *tv, DecodeThreadVars *dtv, Packet *p,
               const uint8_t *pkt, uint16_t len) {
  ...
  if (SCLogDebugEnabled()) { /* only convert the addresses if debug is really
    ...
    SCLogDebug("IPV6 %s->%s - CLASS: %" PRIu32 " FLOW: %" PRIu32 " NH: %" PRIu32
               ...
               s, d, IPV6_GET_CLASS(p), IPV6_GET_FLOW(p), IPV6_GET_NH(p),
               IPV6_GET_PLEN(p), IPV6_GET_HLIM(p));
  }
#endif /* DEBUG */
...
static int DecodeIPV6FragTest01(void) {
  uint8_t raw_frag1[] = {
      ,
  ...
  uint8_t raw_frag2[] = {
      0x60, 0x0f, 0x1a, 0xcf, 0x00, 0x1c, 0x2c, 0x36, 0x20, 0x01, 0x04, 0x70,
      0x00, 0x01, 0x00, 0x18, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x02,
      0x20, 0x01, 0x09, 0x80, 0x32, 0xb2, 0x00, 0x01, 0x2e, 0x41, 0x38, 0xff,
      0xfe, 0xa7, 0xea, 0xeb, 0x06, 0x00, 0x05, 0xa0, 0xdf, 0xf8, 0x11, 0xd7,
      0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20,
...
static int DecodeIPV6RouteTest01(void) {
  uint8_t raw_pkt1[] = {
      0x60, 0x00, 0x00, 0x00, 0x00, 0x1c, 0x2b, 0x40, 0x20, 0x01, 0xaa, 0xaa,
      0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x02,
      0x20, 0x01, 0xaa, 0xaa, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x01, 0x06, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,

      0xb2, 0xed, 0x00, 0x50, 0x1b, 0xc7, 0x6a, 0xdf, 0x00, 0x00, 0x00, 0x00,
      0x50, 0x02, 0x20, 0x00, 0xfa, 0x87, 0x00, 0x00,
  };
  Packet *p1 = PacketGetFromAlloc();
...
static int DecodeIPV6HopTest01(void) {
  uint8_t
      raw_pkt1[] = {0x60, 0x00, 0x00, 0x00, 0x00, 0x20, 0x00, 0x01, 0xfe, 0x80,
                    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x02, 0x0f, 0xfe, 0xff,
                    0xfe, 0x98, 0x3d, 0x01, 0xff, 0x02, 0x00, 0x00, 0x00, 0x00,
                    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01,
                    0x3a, 0x00, 0xff, /* 0xff is a nonsense opt */
                    0x02, 0x00, 0x00, 0x00, 0x00, 0x82, 0x00, 0x1c, 0x6f, 0x27,
