...
static void *ParsePcapConfig(const char *iface) {
  ...
  if (ConfGetChildValueWithDefault(if_root, if_default, "threads",
    ...
    if (threadsstr != NULL) {
      if (StringParseInt32(&aconf->threads, 10, 0, (const char *)threadsstr) <
          0) {
        SCLogWarning(
            "Invalid value for "
  ...
  if (aconf->buffer_size == 0) {
    ...
    if (ret == 1 && s_limit) {
      ...
      if (ParseSizeStringU64(s_limit, &bsize) < 0) {
        ...
        if (bsize > INT_MAX) {
          SCLogError("Failed to set pcap buffer size: 2gb max. %" PRIu64
                     " > %d",
                     bsize, INT_MAX);
        } else {
...
int RunModeIdsPcapWorkers(void) {
  ...
  ret = RunModeSetLiveCaptureWorkers(ParsePcapConfig, PcapConfigGeThreadsCount,
                                     "ReceivePcap", "DecodePcap",
