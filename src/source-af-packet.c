...
#define TP_STATUS_TS_SYS_HARDWARE \
  BIT_U32(30) /* kernel comment says: "deprecated, never set" */
#endif
...
#define TP_STATUS_USER_BUSY                        \
  (uint32_t)((uint32_t)TP_STATUS_TS_SOFTWARE |     \
             (uint32_t)TP_STATUS_TS_SYS_HARDWARE | \
             (uint32_t)TP_STATUS_TS_RAW_HARDWARE)
#endif
#define FRAME_BUSY(tp_status)                                 \
  (((uint32_t)(tp_status) & (uint32_t)TP_STATUS_USER_BUSY) == \
   (uint32_t)TP_STATUS_USER_BUSY)
...
static void AFPPeerUpdate(AFPThreadVars *ptv) {
  ...
  (void)SC_ATOMIC_SET(ptv->mpeer->if_idx,
                      AFPGetIfnumByDev(ptv->socket, ptv->iface, 0));
...
static void AFPPeerClean(AFPPeer *peer) {
  if (peer->flags & AFP_SOCK_PROTECT) SCMutexDestroy(&peer->sock_protect);
  SCFree(peer);
}
...
static int AFPPeersListStarted(void) { return !peerslist.turn; }
...
static inline void AFPDumpCounters(AFPThreadVars *ptv) {
  ...
  if (getsockopt(ptv->socket, SOL_PACKET, PACKET_STATISTICS, &kstats, &len) >
      -1) {
    ...
    if (value > ptv->send_errors_logged) {
      StatsAddUI64(ptv->tv, ptv->capture_afp_send_err,
                   value - ptv->send_errors_logged);
 ...
 */
static void AFPReadFromRingSetupPacket(AFPThreadVars *ptv, union thdr h,
                                       const unsigned int tp_status,
                                       Packet *p) {
   ...
   * us in autofp mode. It will be cleared when the frame gets released to the
   * kernel. */
  ...
  if (h.h2->tp_len > h.h2->tp_snaplen) {
    SCLogDebug("Packet length (%d) > snaplen (%d), truncating", h.h2->tp_len,
               h.h2->tp_snaplen);
  }
  ...
  (void)PacketSetData(p, (unsigned char *)h.raw + h.h2->tp_mac,
                      h.h2->tp_snaplen);
  ...
  p->afp_v.peer =
      (p->afp_v.copy_mode == AFP_COPY_MODE_NONE) ? NULL : ptv->mpeer->peer;
 ...
 * packets (4) are ignored. The highest value in if_linux.h is PACKET_KERNEL
 * (7), this extra check is being overly cautious.
 *
 * \retval true if the frame should be ignored
 */
static inline bool AFPShouldIgnoreFrame(AFPThreadVars *ptv,
                                        const struct sockaddr_ll *sll) {
  if (unlikely(sll->sll_pkttype > 31)) return false;
...
static int AFPReadFromRing(AFPThreadVars *ptv) {
  ...
  if (AFPReadFromRingWaitForPacket(ptv) != AFP_READ_OK) return AFP_READ_FAILURE;
  ...
  while (1) {
    ...
    if (unlikely(AFPShouldIgnoreFrame(ptv, sll))) goto next_frame;
  ...
  next_frame:
    if (++ptv->frame_offset >= ptv->req.v2.tp_frame_nr) {
      ...
      if (ptv->frame_offset == start_pos) break;
    }
  }
  if (emergency_flush) {
    AFPDumpCounters(ptv);
...
static inline int AFPParsePacketV3(AFPThreadVars *ptv,
...
static inline int AFPWalkBlock(AFPThreadVars *ptv,
                               struct tpacket_block_desc *pbd) {
 ...
 */
static int AFPRefSocket(AFPPeer *peer) {
  if (unlikely(peer == NULL)) return 0;
...
static void AFPCloseSocket(AFPThreadVars *ptv) {
  if (ptv->mpeer != NULL) BUG_ON(SC_ATOMIC_GET(ptv->mpeer->sock_usage) != 0);
...
static int AFPReadAndDiscardFromRing(AFPThreadVars *ptv, struct timeval *synctv,
...
#endif
  {
    ...
    if (h.h2->tp_status == TP_STATUS_KERNEL) return 0;
...
TmEcode ReceiveAFPLoop(ThreadVars *tv, void *data, void *slot) {
    ...
    if (r < 0) {
      switch (-r) {
        ...
        case AFP_RECOVERABLE_ERROR:
          SCLogWarning("%s: failed to init socket for interface, retrying soon",
                       ptv->iface);
      }
    }
  ...
  // Indicate that the thread is actually running its application level code
  // (i.e., it can poll packets)
  ...
  while (1) {
    ...
    if (r > 0 && (fds.revents & (POLLHUP | POLLRDHUP | POLLERR | POLLNVAL))) {
      ...
      if (fds.revents & (POLLHUP | POLLRDHUP)) {
        ...
        SCLogWarning("%s: failed to poll interface: %s", ptv->iface,
                     strerror(errno));
        AFPSwitchState(ptv, AFP_STATE_DOWN);
        continue;
      } else if (fds.revents & POLLNVAL) {
        SCLogWarning("%s: invalid poll request: %s", ptv->iface,
                     strerror(errno));
        AFPSwitchState(ptv, AFP_STATE_DOWN);
        continue;
      }
...
static int AFPGetDevFlags(int fd, const char *ifname) {
  ...
  if (ioctl(fd, SIOCGIFFLAGS, &ifr) == -1) {
    SCLogError("%s: failed to get interface flags: %s", ifname,
               strerror(errno));
    return -1;
  }
...
static int AFPGetDevLinktype(int fd, const char *ifname) {
  ...
  if (ioctl(fd, SIOCGIFHWADDR, &ifr) == -1) {
    SCLogError("%s: failed to find interface type: %s", ifname,
               strerror(errno));
    return -1;
  }
...
int AFPGetLinkType(const char *ifname) {
  ...
  if (fd == -1) {
    SCLogError("%s: failed to create AF_PACKET socket: %s", ifname,
               strerror(errno));
    return LINKTYPE_RAW;
  }
...
frame size: TPACKET_ALIGN(snaplen + TPACKET_ALIGN(TPACKET_ALIGN(tp_hdrlen) +
sizeof(struct sockaddr_ll) + ETH_HLEN) - ETH_HLEN);
  ...
  if (snaplen == 0) {
    ...
    if (snaplen <= 0) {
      SCLogWarning("%s: unable to get MTU, setting snaplen default of 1514",
                   ptv->iface);
      snaplen = 1514;
    }
  }

  ptv->req.v2.tp_frame_size =
      TPACKET_ALIGN(snaplen +
                    TPACKET_ALIGN(TPACKET_ALIGN(tp_hdrlen) +
                                  sizeof(struct sockaddr_ll) + ETH_HLEN) -
                    ETH_HLEN);
  ...
  SCLogPerf("%s: rx ring: block_size=%d block_nr=%d frame_size=%d frame_nr=%d",
            ptv->iface, ptv->req.v2.tp_block_size, ptv->req.v2.tp_block_nr,
...
static int AFPComputeRingParamsV3(AFPThreadVars *ptv) {
  ...
  if (frames_per_block == 0) {
    SCLogError("%s: block size is too small, it should be at least %d",
               ptv->iface, ptv->req.v3.tp_frame_size);
    return -1;
  }
  ...
  SCLogPerf(
      "%s: rx ring params: block_size=%d block_nr=%d frame_size=%d frame_nr=%d "
      "(mem: %d)",
...
static int AFPSetupRing(AFPThreadVars *ptv, char *devname) {
...
#endif
  if (setsockopt(ptv->socket, SOL_PACKET, PACKET_VERSION, &val, sizeof(val)) <
      0) {
    SCLogError(
        "%s: failed to activate TPACKET_V2/TPACKET_V3 on packet socket: %s",
  ...
  if (setsockopt(ptv->socket, SOL_PACKET, PACKET_RESERVE, (void *)&reserve,
                 sizeof(reserve)) < 0) {
    SCLogError("%s: failed to activate reserve on packet socket: %s", devname,
               strerror(errno));
    return AFP_FATAL_ERROR;
  }
  ...
  if (ptv->flags & AFP_TPACKET_V3) {
    ...
    if (r < 0) {
      SCLogError("%s: failed to allocate RX Ring: %s", devname,
                 strerror(errno));
      return AFP_FATAL_ERROR;
    }
  } else {
    ...
    for (order = AFP_BLOCK_SIZE_DEFAULT_ORDER; order >= 0; order--) {
      if (AFPComputeRingParams(ptv, order) != 1) {
        SCLogError(
            "%s: ring parameters are incorrect. Please file a bug report",
            devname);
        return AFP_FATAL_ERROR;
      }
      ...
      if (r < 0) {
        if (errno == ENOMEM) {
          SCLogWarning("%s: memory issue with ring parameters. Retrying",
                       devname);
          continue;
        }
  ...
  if (ptv->flags & AFP_MMAP_LOCKED) mmap_flag |= MAP_LOCKED;
  ptv->ring_buf = mmap(0, ptv->ring_buflen, PROT_READ | PROT_WRITE, mmap_flag,
                       ptv->socket, 0);
  ...
  if (ptv->flags & AFP_TPACKET_V3) {
    ...
    for (i = 0; i < ptv->req.v3.tp_block_nr; ++i) {
      ptv->ring.v3[i].iov_base =
          ptv->ring_buf + (i * ptv->req.v3.tp_block_size);
...
postmmap_err:
  munmap(ptv->ring_buf, ptv->ring_buflen);
  if (ptv->ring.v2) SCFree(ptv->ring.v2);
  if (ptv->ring.v3) SCFree(ptv->ring.v3);
mmap_err:
...
int AFPIsFanoutSupported(uint16_t cluster_id) {
  ...
  int r = setsockopt(fd, SOL_PACKET, PACKET_FANOUT, (void *)&option,
                     sizeof(option));
  close(fd);
...
static int SockFanoutSeteBPF(AFPThreadVars *ptv) {
  ...
  if (setsockopt(ptv->socket, SOL_PACKET, PACKET_FANOUT_DATA, &pfd,
                 sizeof(pfd))) {
...
static int AFPCreateSocket(AFPThreadVars *ptv, char *devname, int verbose) {
  ...
  if (bind_address.sll_ifindex == -1) {
    if (verbose) SCLogWarning("%s: device for found", devname);
  ...
  if (ptv->promisc != 0) {
    ...
    r = setsockopt(ptv->socket, SOL_PACKET, PACKET_ADD_MEMBERSHIP,
                   (void *)&sock_params, sizeof(sock_params));
    if (r < 0) {
      SCLogError("%s: failed to set promisc mode: %s", devname,
                 strerror(errno));
      goto socket_err;
    }
  }
  ...
  if (ptv->checksum_mode == CHECKSUM_VALIDATION_KERNEL) {
    ...
    if (setsockopt(ptv->socket, SOL_PACKET, PACKET_AUXDATA, &val,
        ...
        errno != ENOPROTOOPT) {
      SCLogWarning(
          "%s: 'kernel' checksum mode not supported, falling back to full mode",
          devname);
  ...
  if (ptv->buffer_size != 0) {
    ...
    if (setsockopt(ptv->socket, SOL_SOCKET, SO_RCVBUF, &ptv->buffer_size,
      ...
      SCLogError("%s: failed to set buffer size to %d: %s", devname,
                 ptv->buffer_size, strerror(errno));
      goto socket_err;
    }
  }
  ...
  if (ptv->threads > 1) {
    ...
    r = setsockopt(ptv->socket, SOL_PACKET, PACKET_FANOUT, (void *)&option,
                   sizeof(option));
    if (r < 0) {
  ...
  if (ret != 0) goto socket_err;

  SCLogDebug("Using interface '%s' via socket %d", (char *)devname,
             ptv->socket);
...
TmEcode AFPSetBPFFilter(AFPThreadVars *ptv) {
  ...
  if (!ptv->bpf_filter) return TM_ECODE_OK;
  ...
  if (SCBPFCompile(default_packet_size, /* snaplen_arg */
    ...
    SCLogError("%s: failed to compile BPF \"%s\": %s", ptv->iface,
               ptv->bpf_filter, errbuf);
  ...
  rc = setsockopt(ptv->socket, SOL_SOCKET, SO_ATTACH_FILTER, &fcode,
                  sizeof(fcode));
...
static int AFPInsertHalfFlow(int mapd, void *key, unsigned int nr_cpus) {
  ...
  /* We use a per CPU structure so we have to set an array of values as the
   * kernel is not duplicating the data on each CPU by itself. */
  for (i = 0; i < nr_cpus; i++) {
...
static int AFPBypassCallback(Packet *p) {
  ...
  if (PKT_IS_IPV6(p) &&
    ...
    if (p->flow) EBPFUpdateFlow(p->flow, p, NULL);
 ...
 */
TmEcode ReceiveAFPThreadInit(ThreadVars *tv, const void *initdata,
                             void **data) {
  SCEnter();
  ...
  ptv->capture_kernel_packets =
      StatsRegisterCounter("capture.kernel_packets", ptv->tv);
  ptv->capture_kernel_drops =
      StatsRegisterCounter("capture.kernel_drops", ptv->tv);
  ptv->capture_errors = StatsRegisterCounter("capture.errors", ptv->tv);

  ptv->afpacket_spin =
      StatsRegisterAvgCounter("capture.afpacket.busy_loop_avg", ptv->tv);

  ptv->capture_afp_poll =
      StatsRegisterCounter("capture.afpacket.polls", ptv->tv);
  ptv->capture_afp_poll_signal =
      StatsRegisterCounter("capture.afpacket.poll_signal", ptv->tv);
  ptv->capture_afp_poll_timeout =
      StatsRegisterCounter("capture.afpacket.poll_timeout", ptv->tv);
  ptv->capture_afp_poll_data =
      StatsRegisterCounter("capture.afpacket.poll_data", ptv->tv);
  ptv->capture_afp_poll_err =
      StatsRegisterCounter("capture.afpacket.poll_errors", ptv->tv);
  ptv->capture_afp_send_err =
      StatsRegisterCounter("capture.afpacket.send_errors", ptv->tv);
#endif
  ...
  *data = (void *)ptv;
   ...
   * parsing of packet data (See Linux commit
...
#ifdef PACKET_STATISTICS
  AFPDumpCounters(ptv);
  SCLogPerf("%s: (%s) kernel: Packets %" PRIu64 ", dropped %" PRIu64 "",
            ptv->iface, tv->name,
