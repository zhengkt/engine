...
SCEnumCharMap smtp_reply_map[] = {
    {"211", SMTP_REPLY_211}, {"214", SMTP_REPLY_214}, {"220", SMTP_REPLY_220},
    {"221", SMTP_REPLY_221}, {"235", SMTP_REPLY_235}, {"250", SMTP_REPLY_250},
    {"251", SMTP_REPLY_251}, {"252", SMTP_REPLY_252},

    {"334", SMTP_REPLY_334}, {"354", SMTP_REPLY_354},

    {"421", SMTP_REPLY_421}, {"450", SMTP_REPLY_450}, {"451", SMTP_REPLY_451},
    {"452", SMTP_REPLY_452}, {"455", SMTP_REPLY_455},

    {"500", SMTP_REPLY_500}, {"501", SMTP_REPLY_501}, {"502", SMTP_REPLY_502},
    {"503", SMTP_REPLY_503}, {"504", SMTP_REPLY_504}, {"550", SMTP_REPLY_550},
    {"551", SMTP_REPLY_551}, {"552", SMTP_REPLY_552}, {"553", SMTP_REPLY_553},
    {"554", SMTP_REPLY_554}, {"555", SMTP_REPLY_555}, {NULL, -1},
};
...
static void SMTPConfigure(void) {
  ...
  if (config != NULL) {
    ...
    /* Parse extract-urls-schemes from mime config, add '://' suffix to found
     * schemes, and provide a default value of 'http' for the schemes to be
     * extracted if no schemes are found in the config */
    ...
    if (extract_urls_schemes) {
      ...
      TAILQ_FOREACH(scheme, &extract_urls_schemes->head, next) {
        /* new_val_len: scheme value from config e.g. 'http' + '://' + null
         * terminator */
        ...
        /* replace existing scheme value stored on the linked list with new
         * value including
         * '://' suffix */
        SCFree(scheme->val);
        scheme->val = new_val;
...
static void FlagDetectStateNewFile(SMTPTransaction *tx) {
  if (tx && tx->tx_data.de_state) {
    ...
    tx->tx_data.de_state->dir_state[0].flags |=
...
int SMTPProcessDataChunk(const uint8_t *chunk, uint32_t len,
  ...
  if (entity->ctnt_flags & CTNT_IS_ATTACHMENT) {
    ...
    if (state->body_begin) {
      ...
      if (FileOpenFileWithId(files, &smtp_config.sbcfg,
                             smtp_state->file_track_id++,
                             (uint8_t *)entity->filename, flen,
      ...
      if (state->body_end) {
        ...
        if (files->tail->state == FILE_STATE_OPENED) {
          ret = FileCloseFile(files, &smtp_config.sbcfg, (uint8_t *)NULL, 0,
                              flags);
          if (ret != 0) {
        ...
        depth = smtp_state->toserver_data_count -
                smtp_state->toserver_last_data_stamp;
      ...
      if (files->tail && files->tail->state == FILE_STATE_OPENED) {
        ret = FileCloseFile(files, &smtp_config.sbcfg, (uint8_t *)chunk, len,
                            flags);
        if (ret != 0) {
          SCLogDebug("FileCloseFile() failed: %d", ret);
          ret = MIME_DEC_ERR_DATA;
      ...
      uint32_t depth = smtp_state->toserver_data_count -
                       smtp_state->toserver_last_data_stamp;
      AppLayerParserTriggerRawStreamReassembly(flow, STREAM_TOSERVER);
      SCLogDebug("StreamTcpReassemblySetMinInspectDepth STREAM_TOSERVER %u",
                 depth);
      ...
      if (files->tail && files->tail->content_inspected == 0 &&
        ...
        /* expand the limit as long as we get file data, as the file data is
         * bigger on the wire due to base64 */
      } else {
        uint32_t depth = smtp_config.content_inspect_min_size +
                         (smtp_state->toserver_data_count -
                          smtp_state->toserver_last_data_stamp);
...
static AppLayerResult SMTPGetLine(SMTPState *state, SMTPInput *input,
  ...
  if (input->len <= 0) return APP_LAYER_ERROR;

  uint8_t *lf_idx = memchr(input->buf + input->consumed, 0x0a, input->len);
  bool discard_till_lf =
      (direction == 0) ? state->discard_till_lf_ts : state->discard_till_lf_tc;
  ...
  if (lf_idx == NULL) {
    ...
    /* There could be one chunk of command data that has LF but post the line
     * limit e.g. input_len = 5077 lf_idx = 5010 max_line_len = 4096 */
    ...
    if (line->len < 0) SCReturnStruct(APP_LAYER_ERROR);
    input->len -= line->len;
...
static int SMTPInsertCommandIntoCommandBuffer(uint8_t command, SMTPState *state,
  ...
  if (state->cmds_cnt >= state->cmds_buffer_len) {
    ...
    if ((int)(state->cmds_buffer_len + SMTP_COMMAND_BUFFER_STEPS) >
        (int)USHRT_MAX) {
...
static inline void SMTPTransactionComplete(SMTPState *state) {
  ...
  if (state->curr_tx) state->curr_tx->done = 1;
}
 ...
 *  \retval -1 error
 */
static int SMTPProcessCommandDATA(SMTPState *state, SMTPTransaction *tx,
                                  Flow *f, AppLayerParserState *pstate,
                                  const SMTPLine *line) {
  SCEnter();
  DEBUG_VALIDATE_BUG_ON(tx == NULL);
  ...
  if (line->len == 1 && line->buf[0] == '.') {
    ...
    // ProcessDataChunk
    FileAppendData(&tx->files_ts, &smtp_config.sbcfg, line->buf,
                   line->len + line->delim_len);
  }
  ...
  if (state->current_command == SMTP_COMMAND_DATA &&
    ...
    if (smtp_config.decode_mime && tx->mime_state != NULL) {
      int ret = MimeDecParseLine(line->buf, line->len, line->delim_len,
                                 tx->mime_state);
      if (ret != MIME_DEC_OK) {
        if (ret != MIME_DEC_ERR_STATE) {
          ...
          SCLogDebug("MimeDecParseLine() function returned an error code: %d",
                     ret);
          SMTPSetEvent(state, SMTP_DECODER_EVENT_MIME_PARSE_FAILED);
        }
...
static int SMTPProcessReply(SMTPState *state, Flow *f,
                            AppLayerParserState *pstate, SMTPThreadCtx *td,
  ...
  if (mpm_cnt == 0) {
    ...
    SMTPSetEvent(state, SMTP_DECODER_EVENT_INVALID_REPLY);
    SCLogDebug("invalid reply code %02x %02x %02x", line->buf[0], line->buf[1],
               line->buf[2]);
    SCReturnInt(-1);
  }
  enum SMTPCode reply_code =
      smtp_reply_map[td->pmq->rule_id_array[0]].enum_value;
...
static int SMTPParseCommandWithParam(SMTPState *state, const SMTPLine *line,
                                     uint8_t prefix_len, uint8_t **target,
                                     uint16_t *target_len) {
  int i = prefix_len + 1;
  ...
  /* rfc1870: with the size extension the mail from can be followed by an
     option. We use the space separator to detect it. */
  int spc_i = i;
...
static int SMTPParseCommandHELO(SMTPState *state, const SMTPLine *line) {
  ...
  return SMTPParseCommandWithParam(state, line, 4, &state->helo,
                                   &state->helo_len);
}
...
static int SMTPParseCommandMAILFROM(SMTPState *state, const SMTPLine *line) {
  ...
  return SMTPParseCommandWithParam(state, line, 9, &state->curr_tx->mail_from,
 ...
 *
 * Parse and decide the current command and set appropriate variables on the
 * state accordingly. Create transactions if needed or update the current
 * transaction with the appropriate data/params. Pass the control to the
 * respective command parser in the end.
 *
...
static int SMTPProcessRequest(SMTPState *state, Flow *f,
                              AppLayerParserState *pstate, SMTPInput *input,
                              const SMTPLine *line) {
  SCEnter();
  ...
  if (state->curr_tx == NULL ||
      (state->curr_tx->done && !NoNewTx(state, line))) {
  ...
  if (!(state->parser_state & SMTP_PARSER_STATE_COMMAND_DATA_MODE)) {
    ...
    if (line->len >= 8 && SCMemcmpLowercase("starttls", line->buf, 8) == 0) {
      ...
      if (smtp_config.raw_extraction) {
        ...
        if (FileOpenFileWithId(&tx->files_ts, &smtp_config.sbcfg,
                               state->file_track_id++, (uint8_t *)rawmsgname,
                               strlen(rawmsgname), NULL, 0,
      ...
      state->parser_state |= SMTP_PARSER_STATE_COMMAND_DATA_MODE;
               ...
               ((SCMemcmpLowercase("helo", line->buf, 4) == 0) ||
      ...
      state->current_command = SMTP_COMMAND_OTHER_CMD;
               ...
               SCMemcmpLowercase("mail from", line->buf, 9) == 0) {
      ...
      state->current_command = SMTP_COMMAND_OTHER_CMD;
               ...
               SCMemcmpLowercase("rcpt to", line->buf, 7) == 0) {
    ...
    if (SMTPInsertCommandIntoCommandBuffer(state->current_command, state, f) ==
        -1) {
      SCReturnInt(-1);
    }
 ...
 *         -1 for errors and inconsistent states
 * */
static int SMTPPreProcessCommands(SMTPState *state, Flow *f,
                                  AppLayerParserState *pstate, SMTPInput *input,
                                  SMTPLine *line) {
  DEBUG_VALIDATE_BUG_ON(
      (state->parser_state & SMTP_PARSER_STATE_COMMAND_DATA_MODE) == 0);
  DEBUG_VALIDATE_BUG_ON(line->len != 0);
...
static AppLayerResult SMTPParse(uint8_t direction, Flow *f, SMTPState *state,
  ...
  if (direction == 0) {
    ...
    while (res.status == 0) {
      ...
      if (retval != 0) SCReturnStruct(APP_LAYER_ERROR);
      ...
      /* If request was successfully parsed, reset line as it has already been
       * used wherever it had to be */
      ResetLine(&line);

      /* If DATA mode was entered in the middle of input parsing, exempt it from
       * GetLine as we don't want input limits to be exercised on DATA data.
       * Here, SMTPPreProcessCommands should either consume all the data or
       * return in case it encounters another boundary. In case of another
       * boundary, the control should be passed to SMTPGetLine */
...
void *SMTPStateAlloc(void *orig_state, AppProto proto_orig) {
  ...
  if (unlikely(smtp_state == NULL)) return NULL;
...
static SMTPString *SMTPStringAlloc(void) {
  ...
  if (unlikely(smtp_string == NULL)) return NULL;
...
static void SMTPLocalStorageFree(void *ptr) {
  ...
  if (td != NULL) {
    ...
    if (td->smtp_mpm_thread_ctx != NULL) {
      mpm_table[SMTP_MPM].DestroyThreadCtx(smtp_mpm_ctx,
                                           td->smtp_mpm_thread_ctx);
...
static void SMTPTransactionFree(SMTPTransaction *tx, SMTPState *state) {
  ...
  if (tx->tx_data.de_state != NULL) DetectEngineStateFree(tx->tx_data.de_state);

  if (tx->mail_from) SCFree(tx->mail_from);
...
static void SMTPSetMpmState(void) {
  ...
  for (i = 0; i < sizeof(smtp_reply_map) / sizeof(SCEnumCharMap) - 1; i++) {
    ...
    MpmAddPatternCI(smtp_mpm_ctx, (uint8_t *)map->enum_name, 3, 0 /* defunct */,
                    0 /* defunct */, i /* pattern id */, i /* rule id */,
                    0 /* no flags */);
  }
...
static int SMTPStateGetEventInfo(const char *event_name, int *event_id,
...
static int SMTPRegisterPatternsForProtocolDetection(void) {
  if (AppLayerProtoDetectPMRegisterPatternCI(IPPROTO_TCP, ALPROTO_SMTP, "EHLO",
                                             4, 0, STREAM_TOSERVER) < 0) {
    return -1;
  }
  if (AppLayerProtoDetectPMRegisterPatternCI(IPPROTO_TCP, ALPROTO_SMTP, "HELO",
                                             4, 0, STREAM_TOSERVER) < 0) {
    return -1;
  }
  if (AppLayerProtoDetectPMRegisterPatternCI(IPPROTO_TCP, ALPROTO_SMTP, "QUIT",
                                             4, 0, STREAM_TOSERVER) < 0) {
    return -1;
  }
...
static void SMTPStateTransactionFree(void *state, uint64_t tx_id) {
  ...
  TAILQ_FOREACH(tx, &smtp_state->tx_list, next) {
    ...
    if (tx == smtp_state->curr_tx) smtp_state->curr_tx = NULL;
    TAILQ_REMOVE(&smtp_state->tx_list, tx, next);
...
static void *SMTPStateGetTx(void *state, uint64_t id) {
  ...
  if (smtp_state) {
    ...
    if (smtp_state->curr_tx == NULL) return NULL;
    if (smtp_state->curr_tx->tx_id == id) return smtp_state->curr_tx;

    TAILQ_FOREACH(tx, &smtp_state->tx_list, next) {
      if (tx->tx_id == id) return tx;
    }
  }
  return NULL;
}
...
static AppLayerGetFileState SMTPGetTxFiles(void *state, void *txv,
                                           uint8_t direction) {
 ...
 */
static AppLayerGetTxIterTuple SMTPGetTxIterator(
    const uint8_t ipproto, const AppProto alproto, void *alstate,
    uint64_t min_tx_id, uint64_t max_tx_id, AppLayerGetTxIterState *state) {
...
void RegisterSMTPParsers(void) {
  ...
  if (AppLayerProtoDetectConfProtoDetectionEnabled("tcp", proto_name)) {
    ...
    if (SMTPRegisterPatternsForProtocolDetection() < 0) return;
  } else {
  ...
  if (AppLayerParserConfParserEnabled("tcp", proto_name)) {
    AppLayerParserRegisterStateFuncs(IPPROTO_TCP, ALPROTO_SMTP, SMTPStateAlloc,
                                     SMTPStateFree);
    ...
    AppLayerParserRegisterGetEventInfo(IPPROTO_TCP, ALPROTO_SMTP,
                                       SMTPStateGetEventInfo);
    AppLayerParserRegisterGetEventInfoById(IPPROTO_TCP, ALPROTO_SMTP,
                                           SMTPStateGetEventInfoById);

    AppLayerParserRegisterLocalStorageFunc(
        IPPROTO_TCP, ALPROTO_SMTP, SMTPLocalStorageAlloc, SMTPLocalStorageFree);

    AppLayerParserRegisterTxFreeFunc(IPPROTO_TCP, ALPROTO_SMTP,
                                     SMTPStateTransactionFree);
    AppLayerParserRegisterGetTxFilesFunc(IPPROTO_TCP, ALPROTO_SMTP,
                                         SMTPGetTxFiles);
    AppLayerParserRegisterGetStateProgressFunc(IPPROTO_TCP, ALPROTO_SMTP,
                                               SMTPStateGetAlstateProgress);
    AppLayerParserRegisterGetTxCnt(IPPROTO_TCP, ALPROTO_SMTP,
                                   SMTPStateGetTxCnt);
    AppLayerParserRegisterGetTx(IPPROTO_TCP, ALPROTO_SMTP, SMTPStateGetTx);
    AppLayerParserRegisterGetTxIterator(IPPROTO_TCP, ALPROTO_SMTP,
                                        SMTPGetTxIterator);
    AppLayerParserRegisterTxDataFunc(IPPROTO_TCP, ALPROTO_SMTP, SMTPGetTxData);
    AppLayerParserRegisterStateDataFunc(IPPROTO_TCP, ALPROTO_SMTP,
                                        SMTPGetStateData);
...
#ifdef UNITTESTS
  AppLayerParserRegisterProtocolUnittests(IPPROTO_TCP, ALPROTO_SMTP,
 ...
 */
void SMTPParserCleanup(void) { SMTPFreeMpmState(); }
...
static int SMTPParserTest01(void) {
  ...
  uint8_t welcome_reply[] = {
      0x32, 0x32, 0x30, 0x20, 0x6d, 0x78, 0x2e, 0x67, 0x6f, 0x6f, 0x67,
      0x6c, 0x65, 0x2e, 0x63, 0x6f, 0x6d, 0x20, 0x45, 0x53, 0x4d, 0x54,
      0x50, 0x20, 0x64, 0x31, 0x35, 0x73, 0x6d, 0x39, 0x38, 0x36, 0x32,
      0x38, 0x33, 0x77, 0x66, 0x6c, 0x2e, 0x36, 0x0d, 0x0a};
  uint32_t welcome_reply_len = sizeof(welcome_reply);

  /* EHLO [192.168.0.158]<CR><LF> */
  uint8_t request1[] = {0x45, 0x48, 0x4c, 0x4f, 0x20, 0x5b, 0x31, 0x39,
   ...
   */
  uint8_t reply1[] = {
      0x32, 0x35, 0x30, 0x2d, 0x6d, 0x78, 0x2e, 0x67, 0x6f, 0x6f, 0x67, 0x6c,
      0x65, 0x2e, 0x63, 0x6f, 0x6d, 0x20, 0x61, 0x74, 0x20, 0x79, 0x6f, 0x75,
      0x72, 0x20, 0x73, 0x65, 0x72, 0x76, 0x69, 0x63, 0x65, 0x2c, 0x20, 0x5b,
      0x31, 0x31, 0x37, 0x2e, 0x31, 0x39, 0x38, 0x2e, 0x31, 0x31, 0x35, 0x2e,
      0x35, 0x30, 0x5d, 0x0d, 0x0a, 0x32, 0x35, 0x30, 0x2d, 0x53, 0x49, 0x5a,
      0x45, 0x20, 0x33, 0x35, 0x38, 0x38, 0x32, 0x35, 0x37, 0x37, 0x0d, 0x0a,
      0x32, 0x35, 0x30, 0x2d, 0x38, 0x42, 0x49, 0x54, 0x4d, 0x49, 0x4d, 0x45,
      0x0d, 0x0a, 0x32, 0x35, 0x30, 0x2d, 0x53, 0x54, 0x41, 0x52, 0x54, 0x54,
      0x4c, 0x53, 0x0d, 0x0a, 0x32, 0x35, 0x30, 0x20, 0x45, 0x4e, 0x48, 0x41,
      0x4e, 0x43, 0x45, 0x44, 0x53, 0x54, 0x41, 0x54, 0x55, 0x53, 0x43, 0x4f,
      0x44, 0x45, 0x53, 0x0d, 0x0a};
  uint32_t reply1_len = sizeof(reply1);

  /* STARTTLS<CR><LF> */
  uint8_t request2[] = {0x53, 0x54, 0x41, 0x52, 0x54,
                        0x54, 0x4c, 0x53, 0x0d, 0x0a};
  uint32_t request2_len = sizeof(request2);
  /* 220 2.0.0 Ready to start TLS<CR><LF> */
  uint8_t reply2[] = {0x32, 0x32, 0x30, 0x20, 0x32, 0x2e, 0x30, 0x2e,
...
static int SMTPParserTest02(void) {
  ...
  /* EHLO boo.com<CR><LF> */
  uint8_t request1[] = {0x45, 0x48, 0x4c, 0x4f, 0x20, 0x62, 0x6f,
                        0x6f, 0x2e, 0x63, 0x6f, 0x6d, 0x0d, 0x0a};
  uint32_t request1_len = sizeof(request1);
  /* 250-mx.google.com at your service, [117.198.115.50]<CR><LF>
   * 250-SIZE 35882577<CR><LF>
   * 250-8BITMIME<CR><LF>
   ...
   */
  uint8_t reply1[] = {
      0x32, 0x35, 0x30, 0x2d, 0x70, 0x6f, 0x6f, 0x6e, 0x61, 0x5f, 0x73, 0x6c,
      0x61, 0x63, 0x6b, 0x5f, 0x76, 0x6d, 0x31, 0x2e, 0x6c, 0x6f, 0x63, 0x61,
      0x6c, 0x64, 0x6f, 0x6d, 0x61, 0x69, 0x6e, 0x0d, 0x0a, 0x32, 0x35, 0x30,
      0x2d, 0x53, 0x49, 0x5a, 0x45, 0x20, 0x31, 0x30, 0x32, 0x34, 0x30, 0x30,
      0x30, 0x30, 0x0d, 0x0a, 0x32, 0x35, 0x30, 0x2d, 0x56, 0x52, 0x46, 0x59,
      0x0d, 0x0a, 0x32, 0x35, 0x30, 0x2d, 0x45, 0x54, 0x52, 0x4e, 0x0d, 0x0a,
      0x32, 0x35, 0x30, 0x2d, 0x45, 0x4e, 0x48, 0x41, 0x4e, 0x43, 0x45, 0x44,
      0x53, 0x54, 0x41, 0x54, 0x55, 0x53, 0x43, 0x4f, 0x44, 0x45, 0x53, 0x0d,
      0x0a, 0x32, 0x35, 0x30, 0x2d, 0x38, 0x42, 0x49, 0x54, 0x4d, 0x49, 0x4d,
      0x45, 0x0d, 0x0a, 0x32, 0x35, 0x30, 0x20, 0x44, 0x53, 0x4e, 0x0d, 0x0a};
  uint32_t reply1_len = sizeof(reply1);

  /* MAIL FROM:asdff@asdf.com<CR><LF> */
  uint8_t request2[] = {0x4d, 0x41, 0x49, 0x4c, 0x20, 0x46, 0x52, 0x4f, 0x4d,
                        0x3a, 0x61, 0x73, 0x64, 0x66, 0x66, 0x40, 0x61, 0x73,
                        0x64, 0x66, 0x2e, 0x63, 0x6f, 0x6d, 0x0d, 0x0a};
  uint32_t request2_len = sizeof(request2);
  /* 250 2.1.0 Ok<CR><LF> */
  uint8_t reply2[] = {0x32, 0x35, 0x30, 0x20, 0x32, 0x2e, 0x31,
                      0x2e, 0x30, 0x20, 0x4f, 0x6b, 0x0d, 0x0a};
  uint32_t reply2_len = sizeof(reply2);

  /* RCPT TO:bimbs@gmail.com<CR><LF> */
  uint8_t request3[] = {0x52, 0x43, 0x50, 0x54, 0x20, 0x54, 0x4f, 0x3a, 0x62,
                        0x69, 0x6d, 0x62, 0x73, 0x40, 0x67, 0x6d, 0x61, 0x69,
                        0x6c, 0x2e, 0x63, 0x6f, 0x6d, 0x0d, 0x0a};
  uint32_t request3_len = sizeof(request3);
  /* 250 2.1.5 Ok<CR><LF> */
  uint8_t reply3[] = {0x32, 0x35, 0x30, 0x20, 0x32, 0x2e, 0x31,
                      0x2e, 0x35, 0x20, 0x4f, 0x6b, 0x0d, 0x0a};
  ...
  uint8_t reply4[] = {0x33, 0x35, 0x34, 0x20, 0x45, 0x6e, 0x64, 0x20,
  ...
  uint8_t request5_1[] = {0x46, 0x52, 0x4f, 0x4d, 0x3a, 0x61, 0x73,
                          0x64, 0x66, 0x66, 0x40, 0x61, 0x73, 0x64,
                          0x66, 0x2e, 0x63, 0x6f, 0x6d, 0x0d, 0x0a};
  uint32_t request5_1_len = sizeof(request5_1);
  /* TO:bimbs@gmail.com<CR><LF> */
  uint8_t request5_2[] = {0x54, 0x4f, 0x3a, 0x62, 0x69, 0x6d, 0x62,
                          0x73, 0x40, 0x67, 0x6d, 0x61, 0x69, 0x6c,
  ...
  uint8_t request5_4[] = {0x74, 0x68, 0x69, 0x73, 0x20, 0x69, 0x73,
                          0x20, 0x74, 0x65, 0x73, 0x74, 0x20, 0x6d,
                          0x61, 0x69, 0x6c, 0x31, 0x0d, 0x0a};
  ...
  uint8_t reply5[] = {0x32, 0x35, 0x30, 0x20, 0x32, 0x2e, 0x30, 0x2e, 0x30,
                      0x20, 0x4f, 0x6b, 0x3a, 0x20, 0x71, 0x75, 0x65, 0x75,
                      0x65, 0x64, 0x20, 0x61, 0x73, 0x20, 0x36, 0x41, 0x31,
                      0x41, 0x46, 0x32, 0x30, 0x42, 0x46, 0x32, 0x0d, 0x0a};
  uint32_t reply5_len = sizeof(reply5);

  /* MAIL FROM:asdfg@asdf.com<CR><LF> */
  uint8_t request6[] = {0x4d, 0x41, 0x49, 0x4c, 0x20, 0x46, 0x52, 0x4f, 0x4d,
                        0x3a, 0x61, 0x73, 0x64, 0x66, 0x67, 0x40, 0x61, 0x73,
                        0x64, 0x66, 0x2e, 0x63, 0x6f, 0x6d, 0x0d, 0x0a};
  uint32_t request6_len = sizeof(request6);
  /* 250 2.1.0 Ok<CR><LF> */
  uint8_t reply6[] = {0x32, 0x35, 0x30, 0x20, 0x32, 0x2e, 0x31,
                      0x2e, 0x30, 0x20, 0x4f, 0x6b, 0x0d, 0x0a};
  uint32_t reply6_len = sizeof(reply6);

  /* RCPT TO:bimbs@gmail.com<CR><LF> */
  uint8_t request7[] = {0x52, 0x43, 0x50, 0x54, 0x20, 0x54, 0x4f, 0x3a, 0x62,
                        0x69, 0x6d, 0x62, 0x73, 0x40, 0x67, 0x6d, 0x61, 0x69,
                        0x6c, 0x2e, 0x63, 0x6f, 0x6d, 0x0d, 0x0a};
  uint32_t request7_len = sizeof(request7);
  /* 250 2.1.5 Ok<CR><LF> */
  uint8_t reply7[] = {0x32, 0x35, 0x30, 0x20, 0x32, 0x2e, 0x31,
                      0x2e, 0x35, 0x20, 0x4f, 0x6b, 0x0d, 0x0a};
  ...
  /* 354 End data with <CR><LF>.<CR><LF>|<CR><LF>| */
  uint8_t reply8[] = {0x33, 0x35, 0x34, 0x20, 0x45, 0x6e, 0x64, 0x20,
                      0x64, 0x61, 0x74, 0x61, 0x20, 0x77, 0x69, 0x74,
                      0x68, 0x20, 0x3c, 0x43, 0x52, 0x3e, 0x3c, 0x4c,
                      0x46, 0x3e, 0x2e, 0x3c, 0x43, 0x52, 0x3e, 0x3c,
                      0x4c, 0x46, 0x3e, 0x0d, 0x0a};
  ...
  uint8_t request9_1[] = {0x46, 0x52, 0x4f, 0x4d, 0x3a, 0x61, 0x73, 0x64,
                          0x66, 0x67, 0x40, 0x67, 0x6d, 0x61, 0x69, 0x6c,
                          0x2e, 0x63, 0x6f, 0x6d, 0x0d, 0x0a};
  uint32_t request9_1_len = sizeof(request9_1);
  /* TO:bimbs@gmail.com<CR><LF> */
  uint8_t request9_2[] = {0x54, 0x4f, 0x3a, 0x62, 0x69, 0x6d, 0x62,
                          0x73, 0x40, 0x67, 0x6d, 0x61, 0x69, 0x6c,
                          0x2e, 0x63, 0x6f, 0x6d, 0x0d, 0x0a};
  ...
  uint8_t request9_4[] = {0x74, 0x68, 0x69, 0x73, 0x20, 0x69, 0x73,
                          0x20, 0x74, 0x65, 0x73, 0x74, 0x20, 0x6d,
                          0x61, 0x69, 0x6c, 0x32, 0x0d, 0x0a};
  ...
  uint8_t reply9[] = {0x32, 0x35, 0x30, 0x20, 0x32, 0x2e, 0x30, 0x2e, 0x30,
                      0x20, 0x4f, 0x6b, 0x3a, 0x20, 0x71, 0x75, 0x65, 0x75,
                      0x65, 0x64, 0x20, 0x61, 0x73, 0x20, 0x32, 0x38, 0x43,
  ...
  /* 221 2.0.0 Bye<CR><LF> */
  uint8_t reply10[] = {0x32, 0x32, 0x31, 0x20, 0x32, 0x2e, 0x30, 0x2e,
...
static int SMTPParserTest03(void) {
  ...
  uint8_t welcome_reply[] = {
      0x32, 0x32, 0x30, 0x20, 0x70, 0x6f, 0x6f, 0x6e, 0x61, 0x5f, 0x73, 0x6c,
      0x61, 0x63, 0x6b, 0x5f, 0x76, 0x6d, 0x31, 0x2e, 0x6c, 0x6f, 0x63, 0x61,
      0x6c, 0x64, 0x6f, 0x6d, 0x61, 0x69, 0x6e, 0x20, 0x45, 0x53, 0x4d, 0x54,
      0x50, 0x20, 0x50, 0x6f, 0x73, 0x74, 0x66, 0x69, 0x78, 0x0d, 0x0a};
  uint32_t welcome_reply_len = sizeof(welcome_reply);

  /* EHLO boo.com<CR><LF> */
  uint8_t request1[] = {0x45, 0x48, 0x4c, 0x4f, 0x20, 0x62, 0x6f,
                        0x6f, 0x2e, 0x63, 0x6f, 0x6d, 0x0a};
  uint32_t request1_len = sizeof(request1);
  ...
  uint8_t reply1[] = {
      ...
      0x61, 0x63, 0x6b, 0x5f, 0x76, 0x6d, 0x31, 0x2e, 0x6c, 0x6f, 0x63, 0x61,
      0x6c, 0x64, 0x6f, 0x6d, 0x61, 0x69, 0x6e, 0x0d, 0x0a, 0x32, 0x35, 0x30,
      0x2d, 0x50, 0x49, 0x50, 0x45, 0x4c, 0x49, 0x4e, 0x49, 0x4e, 0x47, 0x0d,
      0x0a, 0x32, 0x35, 0x30, 0x2d, 0x53, 0x49, 0x5a, 0x45, 0x20, 0x31, 0x30,
      0x32, 0x34, 0x30, 0x30, 0x30, 0x30, 0x0d, 0x0a, 0x32, 0x35, 0x30, 0x2d,
      0x56, 0x52, 0x46, 0x59, 0x0d, 0x0a, 0x32, 0x35, 0x30, 0x2d, 0x45, 0x54,
      0x52, 0x4e, 0x0d, 0x0a, 0x32, 0x35, 0x30, 0x2d, 0x45, 0x4e, 0x48, 0x41,
      0x4e, 0x43, 0x45, 0x44, 0x53, 0x54, 0x41, 0x54, 0x55, 0x53, 0x43, 0x4f,
      0x44, 0x45, 0x53, 0x0d, 0x0a, 0x32, 0x35, 0x30, 0x2d, 0x38, 0x42, 0x49,
      0x54, 0x4d, 0x49, 0x4d, 0x45, 0x0d, 0x0a, 0x32, 0x35, 0x30, 0x20, 0x44,
      0x53, 0x4e, 0x0d, 0x0a};
  uint32_t reply1_len = sizeof(reply1);
   ...
   */
  uint8_t request2[] = {
      0x4d, 0x41, 0x49, 0x4c, 0x20, 0x46, 0x52, 0x4f, 0x4d, 0x3a, 0x70, 0x62,
      0x73, 0x66, 0x40, 0x61, 0x73, 0x64, 0x66, 0x73, 0x2e, 0x63, 0x6f, 0x6d,
      0x0d, 0x0a, 0x52, 0x43, 0x50, 0x54, 0x20, 0x54, 0x4f, 0x3a, 0x70, 0x62,
      0x73, 0x66, 0x40, 0x61, 0x73, 0x64, 0x66, 0x73, 0x2e, 0x63, 0x6f, 0x6d,
      0x0d, 0x0a, 0x44, 0x41, 0x54, 0x41, 0x0d, 0x0a, 0x49, 0x6d, 0x6d, 0x65,
      0x64, 0x69, 0x61, 0x74, 0x65, 0x20, 0x64, 0x61, 0x74, 0x61, 0x0d, 0x0a,
  };
  uint32_t request2_len = sizeof(request2);
  /* 250 2.1.0 Ok<CR><LF>
   * 250 2.1.5 Ok<CR><LF>
   ...
   */
  uint8_t reply2[] = {
      0x32, 0x35, 0x30, 0x20, 0x32, 0x2e, 0x31, 0x2e, 0x30, 0x20, 0x4f,
      0x6b, 0x0d, 0x0a, 0x32, 0x35, 0x30, 0x20, 0x32, 0x2e, 0x31, 0x2e,
      0x35, 0x20, 0x4f, 0x6b, 0x0d, 0x0a, 0x33, 0x35, 0x34, 0x20, 0x45,
      0x6e, 0x64, 0x20, 0x64, 0x61, 0x74, 0x61, 0x20, 0x77, 0x69, 0x74,
      0x68, 0x20, 0x3c, 0x43, 0x52, 0x3e, 0x3c, 0x4c, 0x46, 0x3e, 0x2e,
...
static int SMTPParserTest05(void) {
  ...
  uint8_t reply2[] = {0x35, 0x30, 0x32, 0x20, 0x35, 0x2e, 0x35, 0x2e, 0x32,
                      0x20, 0x45, 0x72, 0x72, 0x6f, 0x72, 0x3a, 0x20, 0x63,
                      0x6f, 0x6d, 0x6d, 0x61, 0x6e, 0x64, 0x20, 0x6e, 0x6f,
                      0x74, 0x20, 0x72, 0x65, 0x63, 0x6f, 0x67, 0x6e, 0x69,
                      0x7a, 0x65, 0x64, 0x0d, 0x0a};
  uint32_t reply2_len = sizeof(reply2);

  /* QUIT<CR><LF> */
  uint8_t request3[] = {0x51, 0x55, 0x49, 0x54,
                        0x0d, 0x0a

  };
  uint32_t request3_len = sizeof(request3);
  /* 221 2.0.0 Bye<CR><LF> */
  uint8_t reply3[] = {0x32, 0x32, 0x31, 0x20, 0x32, 0x2e, 0x30, 0x2e,
                      0x30, 0x20, 0x42, 0x79, 0x65, 0x0d, 0x0a};
  uint32_t reply3_len = sizeof(reply3);
  ...
  if ((f.flags & FLOW_NOPAYLOAD_INSPECTION) ||
      ...
      (((TcpSession *)f.protoctx)->server.flags &
       STREAMTCP_STREAM_FLAG_NOREASSEMBLY) ||
      (((TcpSession *)f.protoctx)->client.flags &
       STREAMTCP_STREAM_FLAG_NOREASSEMBLY)) {
    goto end;
  }
...
static int SMTPParserTest06(void) {
  ...
  uint8_t welcome_reply[] = {
      };
  uint32_t welcome_reply_len = sizeof(welcome_reply);

  uint8_t request1[] = {0x45, 0x48, 0x4c, 0x4f, 0x20, 0x45, 0x58, 0x43, 0x48,
                        0x41, 0x4e, 0x47, 0x45, 0x32, 0x2e, 0x63, 0x67, 0x63,
                        0x65, 0x6e, 0x74, 0x2e, 0x6d, 0x69, 0x61, 0x6d, 0x69,
                        0x2e, 0x65, 0x64, 0x75, 0x0d, 0x0a};
  uint32_t request1_len = sizeof(request1);

  uint8_t reply1[] = {
      };
  uint32_t reply1_len = sizeof(reply1);
  ...
  /* BDAT 51<CR><LF> */
  uint8_t request4[] = {
      0x42, 0x44, 0x41, 0x54, 0x20, 0x35, 0x31, 0x0d, 0x0a,
  };
  uint32_t request4_len = sizeof(request4);

  uint8_t request5[] = {
      0x46, 0x52, 0x4f, 0x4d, 0x3a, 0x61, 0x73, 0x64, 0x66, 0x66, 0x40,
      0x61, 0x73, 0x64, 0x66, 0x2e, 0x66, 0x66, 0x40, 0x61, 0x73, 0x64,
      0x66, 0x2e, 0x66, 0x66, 0x40, 0x61, 0x73, 0x64, 0x0d, 0x0a,
  };
  uint32_t request5_len = sizeof(request5);

  uint8_t request6[] = {
      0x46, 0x52, 0x4f, 0x4d, 0x3a, 0x61, 0x73, 0x64, 0x66, 0x66,
      0x40, 0x61, 0x73, 0x64, 0x66, 0x2e, 0x66, 0x0d, 0x0a,
  };
  uint32_t request6_len = sizeof(request6);
...
static int SMTPParserTest12(void) {
  ...
  /* EHLO boo.com<CR><LF> */
  uint8_t request1[] = {
      0x45, 0x48, 0x4c, 0x4f, 0x20, 0x62, 0x6f,
      0x6f, 0x2e, 0x63, 0x6f, 0x6d, 0x0d, 0x0a,
  };
...
static int SMTPParserTest13(void) {
   ...
   */
  uint8_t request2[] = {
      0x4d, 0x41, 0x49, 0x4c, 0x20, 0x46, 0x52, 0x4f, 0x4d, 0x3a, 0x70,
      0x62, 0x73, 0x66, 0x40, 0x61, 0x73, 0x64, 0x66, 0x73, 0x2e, 0x63,
      0x6f, 0x6d, 0x0d, 0x0a, 0x52, 0x43, 0x50, 0x54, 0x20, 0x54, 0x4f,
      0x3a, 0x70, 0x62, 0x73, 0x66, 0x40, 0x61, 0x73, 0x64, 0x66, 0x73,
      0x2e, 0x63, 0x6f, 0x6d, 0x0d, 0x0a, 0x44, 0x41, 0x54, 0x41, 0x0d,
      0x0a, 0x53, 0x54, 0x41, 0x52, 0x54, 0x54, 0x4c, 0x53, 0x0d, 0x0a};
  uint32_t request2_len = sizeof(request2);
...
static int SMTPParserTest14(void) {
  ...
  /* EHLO boo.com<CR><LF> */
  static uint8_t request1[] = {0x45, 0x48, 0x4c, 0x4f, 0x20, 0x62, 0x6f,
                               0x6f, 0x2e, 0x63, 0x6f, 0x6d, 0x0d, 0x0a};
  ...
  /* MAIL FROM:asdff@asdf.com<CR><LF> */
  static uint8_t request2[] = {0x4d, 0x41, 0x49, 0x4c, 0x20, 0x46, 0x52,
                               0x4f, 0x4d, 0x3a, 0x61, 0x73, 0x64, 0x66,
                               0x66, 0x40, 0x61, 0x73, 0x64, 0x66, 0x2e,
                               0x63, 0x6f, 0x6d, 0x0d, 0x0a};
  static uint32_t request2_len = sizeof(request2);
  /* 250 2.1.0 Ok<CR><LF> */
  static uint8_t reply2[] = {0x32, 0x35, 0x30, 0x20, 0x32, 0x2e, 0x31,
                             0x2e, 0x30, 0x20, 0x4f, 0x6b, 0x0d, 0x0a};
  static uint32_t reply2_len = sizeof(reply2);

  /* RCPT TO:bimbs@gmail.com<CR><LF> */
  static uint8_t request3[] = {0x52, 0x43, 0x50, 0x54, 0x20, 0x54, 0x4f,
                               0x3a, 0x62, 0x69, 0x6d, 0x62, 0x73, 0x40,
                               0x67, 0x6d, 0x61, 0x69, 0x6c, 0x2e, 0x63,
                               0x6f, 0x6d, 0x0d, 0x0a};
  static uint32_t request3_len = sizeof(request3);
  /* 250 2.1.5 Ok<CR><LF> */
  static uint8_t reply3[] = {0x32, 0x35, 0x30, 0x20, 0x32, 0x2e, 0x31,
                             0x2e, 0x35, 0x20, 0x4f, 0x6b, 0x0d, 0x0a};
  ...
  /* 354 End data with <CR><LF>.<CR><LF>|<CR><LF>| */
  static uint8_t reply4[] = {0x33, 0x35, 0x34, 0x20, 0x45, 0x6e, 0x64, 0x20,
                             0x64, 0x61, 0x74, 0x61, 0x20, 0x77, 0x69, 0x74,
                             0x68, 0x20, 0x3c, 0x43, 0x52, 0x3e, 0x3c, 0x4c,
                             0x46, 0x3e, 0x2e, 0x3c, 0x43, 0x52, 0x3e, 0x3c,
                             0x4c, 0x46, 0x3e, 0x0d, 0x0a};
  ...
  static uint8_t request4_msg[] = {
      ...
      };
  ...
  static uint8_t reply4_end[] = {
      0x32, 0x35, 0x30, 0x20, 0x32, 0x2e, 0x30, 0x2e, 0x30, 0x20, 0x4f, 0x6b,
      0x3a, 0x20, 0x71, 0x75, 0x65, 0x75, 0x65, 0x64, 0x20, 0x61, 0x73, 0x20,
      0x36, 0x41, 0x31, 0x41, 0x46, 0x32, 0x30, 0x42, 0x46, 0x32, 0x0d, 0x0a};
  ...
  /* 221 2.0.0 Bye<CR><LF> */
  static uint8_t reply5[] = {0x32, 0x32, 0x31, 0x20, 0x32, 0x2e, 0x30, 0x2e,
                             0x30, 0x20, 0x42, 0x79, 0x65, 0x0d, 0x0a};
  ...
  if ((smtp_state->helo_len != 7) ||
      strncmp("boo.com", (char *)smtp_state->helo, 7)) {
  ...
  if ((smtp_state->curr_tx->mail_from_len != 14) ||
    ...
    printf("incorrect parsing of MAIL FROM field '%s' (%d)\n",
           smtp_state->curr_tx->mail_from, smtp_state->curr_tx->mail_from_len);
    goto end;
  }
  ...
  if (files != NULL && files->head != NULL) {
    ...
    if (FileTrackedSize(file) != filesize) {
      printf("smtp-mime file size %" PRIu64 " is incorrect",
             FileDataSize(file));
      goto end;
    }
    static uint8_t org_binary[] = {
        ...
        0x04, 0x00, 0x03, 0x01, 0x0B, 0x01, 0x08, 0x00, 0x01, 0x00, 0x00, 0x80,
        0x00, 0x00, 0x00, 0x00, 0x79, 0x00, 0x00, 0x00, 0x0C, 0x00, 0x00, 0x00,
        0x79, 0x00, 0x00, 0x00, 0x0C, 0x00, 0x00, 0x00, 0x00, 0x00, 0x40, 0x00,
        0x04, 0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 0x74, 0x00, 0x00, 0x00,
        0x20, 0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x04, 0x01, 0x00, 0x00, 0x88, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x5C, 0x5C, 0x36, 0x36,
        0x2E, 0x39, 0x33, 0x2E, 0x36, 0x38, 0x2E, 0x36, 0x5C, 0x7A, 0x00, 0x00,
        0x38,
    };

    if (StreamingBufferCompareRawData(file->sb, org_binary,
                                      sizeof(org_binary)) != 1) {
