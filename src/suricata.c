...
int SuriHasSigFile(void) { return (suricata.sig_file != NULL); }

int EngineModeIsUnknown(void) { return (g_engine_mode == ENGINE_MODE_UNKNOWN); }
...
void EngineModeSetIPS(void) { g_engine_mode = ENGINE_MODE_IPS; }

void EngineModeSetIDS(void) { g_engine_mode = ENGINE_MODE_IDS; }

#ifdef UNITTESTS
int RunmodeIsUnittests(void) {
  if (run_mode == RUNMODE_UNITTEST) return 1;

  return 0;
}
#endif

int RunmodeGetCurrent(void) { return run_mode; }
...
static void SignalHandlerSigint(/*@unused@*/ int sig) { sigint_count = 1; }
static void SignalHandlerSigterm(/*@unused@*/ int sig) { sigterm_count = 1; }
#ifndef OS_WIN32
#if HAVE_LIBUNWIND
#define UNW_LOCAL_ONLY
#include <libunwind.h>
static void SignalHandlerUnexpected(int sig_num, siginfo_t *info,
                                    void *context) {
  ...
  if ((r = unw_init_local(&cursor, (unw_context_t *)(context)) != 0)) {
    SCLogError("unable to obtain stack trace: unw_init_local: %s",
               unw_strerror(r));
    goto terminate;
  }

  char *temp = msg;
  int cw = snprintf(temp, SC_LOG_MAX_LOG_MSG_LEN - (temp - msg),
  ...
  while (r > 0) {
    if (unw_is_signal_frame(&cursor) == 0) {
      ...
      if (unw_get_proc_name(&cursor, name, sizeof(name), &off) == UNW_ENOMEM) {
        cw =
            snprintf(temp, SC_LOG_MAX_LOG_MSG_LEN - (temp - msg), "[unknown]:");
      } else {
        cw = snprintf(temp, SC_LOG_MAX_LOG_MSG_LEN - (temp - msg),
                      "%s+0x%08" PRIx64, name, off);
      }
      temp += cw;
    }
...
static void SignalHandlerSigusr2(int sig) {
  if (sigusr2_count < 2) sigusr2_count++;
}
 ...
 * it.
 */
static void SignalHandlerSigHup(/*@unused@*/ int sig) { sighup_count = 1; }
#endif
...
void EngineStop(void) { suricata_ctl_flags |= SURICATA_STOP; }
 ...
 */
void EngineDone(void) { suricata_ctl_flags |= SURICATA_DONE; }
...
static int SetBpfString(int argc, char *argv[]) {
  ...
  if (bpf_len == 0) return TM_ECODE_OK;

  bpf_filter = SCMalloc(bpf_len);
  if (unlikely(bpf_filter == NULL)) return TM_ECODE_FAILED;
...
static void PrintUsage(const char *progname) {
  ...
  printf(
      ...
      "(use colon to specify a range of queues)\n");
#endif /* NFQ */
#ifdef IPFW
  printf(
      "\t-d <divert port>                     : run in inline ipfw divert "
      "mode\n");
#endif /* IPFW */
  printf(
      "\t-s <path>                            : path to signature file loaded "
      "in addition to suricata.yaml settings (optional)\n");
  printf(
      "\t-S <path>                            : path to signature file loaded "
  ...
  printf(
      "\t--service-change-params              : change service startup "
      "parameters\n");
#endif /* OS_WIN32 */
  printf(
  ...
  printf(
      "\t-v                                   : be more verbose (use multiple "
      "times to increase verbosity)\n");
#ifdef UNITTESTS
  printf(
      "\t-u                                   : run the unittests and exit\n");
  printf(
      "\t-U, --unittest-filter=REGEX          : filter unittests with a "
      "regex\n");
  printf("\t--list-unittests                     : list unit tests\n");
  printf(
      "\t--fatal-unittests                    : enable fatal failure on "
      "unittest error\n");
  printf(
      "\t--unittests-coverage                 : display unittest coverage "
      "report\n");
#endif /* UNITTESTS */
  printf(
      ...
      "protocols\n");
  printf(
      "\t--list-keywords[=all|csv|<kword>]    : list keywords implemented by "
      "the engine\n");
  ...
  printf(
      "\t--pcap-file-continuous               : when running in pcap mode with "
      "a directory, continue checking directory for pcaps until interrupted\n");
  printf(
      "\t--pcap-file-delete                   : when running in replay mode "
      "(-r with directory or file), will delete pcap files that have been "
      "processed when done\n");
  printf(
      "\t--pcap-file-recursive                : will descend into "
      "subdirectories when running in replay mode (-r)\n");
...
#ifdef HAVE_AF_XDP
  printf(
      "\t--af-xdp[=<dev>]                     : run in af-xdp mode, no value "
      "select "
      "interfaces from suricata.yaml\n");
#endif
#ifdef HAVE_NETMAP
  printf(
      "\t--netmap[=<dev>]                     : run in netmap mode, no value "
      "select interfaces from suricata.yaml\n");
#endif
#ifdef HAVE_PFRING
  printf(
      "\t--pfring[=<dev>]                     : run in pfring mode, use "
      "interfaces from suricata.yaml\n");
  printf(
      "\t--pfring-int <dev>                   : run in pfring mode, use "
      "interface <dev>\n");
  printf("\t--pfring-cluster-id <id>             : pfring cluster id \n");
  printf(
      "\t--pfring-cluster-type <type>         : pfring cluster type for "
      "PF_RING 4.1.2 and later cluster_round_robin|cluster_flow\n");
#endif /* HAVE_PFRING */
  printf(
...
#ifdef HAVE_DAG
  printf(
      "\t--dag <dagX:Y>                       : process ERF records from DAG "
      "interface X, stream Y\n");
#endif
#ifdef HAVE_NAPATECH
  printf(
      "\t--napatech                           : run Napatech Streams using the "
      "API\n");
#endif
...
#ifdef WINDIVERT
  printf(
      "\t--windivert <filter>                 : run in inline WinDivert "
      "mode\n");
  printf(
      "\t--windivert-forward <filter>         : run in inline WinDivert mode, "
      "as a gateway\n");
#endif
#ifdef HAVE_LIBNET11
  printf(
      "\t--reject-dev <dev>                   : send reject packets from this "
      "interface\n");
#endif
  printf(
      "\t--include <path>                     : additional configuration "
      "file\n");
  printf(
...
static TmEcode LoadYamlConfig(SCInstance *suri) {
  ...
  if (suri->conf_filename == NULL) suri->conf_filename = DEFAULT_CONF_FILE;
  ...
  if (suri->additional_configs) {
    for (int i = 0; suri->additional_configs[i] != NULL; i++) {
      SCLogConfig("Loading additional configuration file %s",
...
static TmEcode LogVersion(SCInstance *suri) {
  ...
  SCLogNotice("This is %s version %s running in %s mode", PROG_NAME,
              GetProgramVersion(), mode);
  return TM_ECODE_OK;
}
...
static int ParseCommandLinePcapLive(SCInstance *suri, const char *in_arg) {
  ...
  /* If running on Windows without Npcap, bail early as live capture is not
   * supported. */
  FatalError(
      "Live capture not available. To support live capture compile against "
      "Npcap.");
#endif
  memset(suri->pcap_dev, 0, sizeof(suri->pcap_dev));
  ...
  if (in_arg != NULL) {
    ...
    if (strlen(in_arg) > 9 && strncmp(in_arg, "DeviceNPF", 9) == 0) {
      snprintf(suri->pcap_dev, sizeof(suri->pcap_dev), "\\Device\\NPF%s",
               in_arg + 9);
    } else {
      strlcpy(suri->pcap_dev, in_arg, sizeof(suri->pcap_dev));
    ...
    } else if (strlen(suri->pcap_dev) > 0 &&
               isdigit((unsigned char)suri->pcap_dev[0])) {
...
static bool IsLogDirectoryWritable(const char *str) {
  if (access(str, W_OK) == 0) return true;
  return false;
}
...
static TmEcode ParseCommandLine(int argc, char **argv, SCInstance *suri) {
    ...
    switch (opt) {
      case 0:
        if (strcmp((long_opts[option_index]).name, "pfring") == 0 ||
          ...
          return TM_ECODE_FAILED;
#endif /* HAVE_PFRING */
        } else if (strcmp((long_opts[option_index]).name, "capture-plugin") ==
                   0) {
          ...
          if (suri->run_mode == RUNMODE_UNKNOWN) {
            ...
            if (optarg) {
              ...
              SCLogInfo(
                  "Multiple netmap option without interface on each is "
                  "useless");
              break;
            }
          } else {
        ...
        } else if (strcmp((long_opts[option_index]).name, "simulate-ips") ==
                   0) {
        ...
        } else if (strcmp((long_opts[option_index]).name, "list-unittests") ==
                   0) {
#ifdef UNITTESTS
          ...
          return TM_ECODE_FAILED;
#endif /* UNITTESTS */
        } else if (strcmp((long_opts[option_index]).name, "list-runmodes") ==
                   0) {
          suri->run_mode = RUNMODE_LIST_RUNMODES;
          return TM_ECODE_OK;
        } else if (strcmp((long_opts[option_index]).name, "list-keywords") ==
                   0) {
          if (optarg) {
        ...
        } else if (strcmp((long_opts[option_index]).name, "engine-analysis") ==
                   0) {
          // do nothing for now
        }
#ifdef OS_WIN32
        else if (strcmp((long_opts[option_index]).name, "service-install") ==
                 0) {
          suri->run_mode = RUNMODE_INSTALL_SERVICE;
          return TM_ECODE_OK;
        } else if (strcmp((long_opts[option_index]).name, "service-remove") ==
                   0) {
          ...
          if (suri->pid_filename == NULL) {
            ...
            return TM_ECODE_FAILED;
                          ...
                          "disable-detection") == 0) {
          g_detect_disabled = suri->disabled_detect = 1;
        } else if (strcmp((long_opts[option_index]).name, "disable-hashing") ==
                   0) {
          g_disable_hashing = true;
        } else if (strcmp((long_opts[option_index]).name, "fatal-unittests") ==
                   0) {
#ifdef UNITTESTS
          unittests_fatal = 1;
#else
          ...
          return TM_ECODE_FAILED;
#endif /* HAVE_NAPATECH */
        } else if (strcmp((long_opts[option_index]).name, "pcap-buffer-size") ==
                   0) {
#ifdef HAVE_PCAP_SET_BUFF
...
#else
          SCLogError(
              "Libnet 1.1 support not enabled. Compile Suricata with libnet "
              "support.");
          return TM_ECODE_FAILED;
#endif
        ...
        } else if (strcmp((long_opts[option_index]).name, "pcap-file-delete") ==
                   0) {
      ...
      case 'q':
        ...
        if (suri->run_mode == RUNMODE_UNKNOWN) {
          ...
          EngineModeSetIPS();
          if (NFQParseAndRegisterQueues(optarg) == -1) return TM_ECODE_FAILED;
...
#else
        SCLogError(
            "NFQUEUE not enabled. Make sure to pass --enable-nfqueue to "
            "configure when "
            "building.");
        return TM_ECODE_FAILED;
#endif /* NFQ */
      ...
      case 'd':
        ...
        if (suri->run_mode == RUNMODE_UNKNOWN) {
          ...
          EngineModeSetIPS();
          if (IPFWRegisterQueue(optarg) == -1) return TM_ECODE_FAILED;
...
#else
        SCLogError(
            "IPFW not enabled. Make sure to pass --enable-ipfw to configure "
            "when "
            "building.");
        return TM_ECODE_FAILED;
#endif /* IPFW */
      ...
      case 'U':
        ...
        if (strlen(suri->regex_arg) == 0) suri->regex_arg = NULL;
#endif
        break;
      case 'V':
  ...
  if (list_app_layer_protocols) suri->run_mode = RUNMODE_LIST_APP_LAYERS;
  if (list_keywords) suri->run_mode = RUNMODE_LIST_KEYWORDS;
  if (list_unittests) suri->run_mode = RUNMODE_LIST_UNITTEST;
  if (dump_config) suri->run_mode = RUNMODE_DUMP_CONFIG;
  if (dump_features) suri->run_mode = RUNMODE_DUMP_FEATURES;
  if (conf_test) suri->run_mode = RUNMODE_CONF_TEST;
  if (engine_analysis) suri->run_mode = RUNMODE_ENGINE_ANALYSIS;
...
static int MayDaemonize(SCInstance *suri) {
  ...
  if (suri->pid_filename != NULL &&
      SCPidfileTestRunning(suri->pid_filename) != 0) {
    SCFree(suri->pid_filename);
    suri->pid_filename = NULL;
    return TM_ECODE_FAILED;
  }
...
static int InitRunAs(SCInstance *suri) {
  ...
  if (suri->do_setuid == TRUE) {
    if (SCGetUserID(suri->user_name, suri->group_name, &suri->userid,
                    &suri->groupid) != 0) {
...
void PreRunInit(const int runmode) {
  ...
  if (runmode == RUNMODE_UNIX_SOCKET) return;
...
void PostRunDeinit(const int runmode, struct timeval *start_time) {
  ...
  TmqResetQueues();
#ifdef PROFILING
  if (profiling_rules_enabled) SCProfilingDump();
  SCProfilingDestroy();
#endif
}
...
static void SetupDelayedDetect(SCInstance *suri) {
      ...
      if (decnf != NULL) {
        TAILQ_FOREACH(denode, &decnf->head, next) {
          if (strcmp(denode->val, "delayed-detect") == 0) {
            (void)ConfGetChildValueBool(denode, "delayed-detect",
                                        &suri->delayed_detect);
          }
        }
      }
  ...
  SCLogConfig("Delayed detect %s",
              suri->delayed_detect ? "enabled" : "disabled");
...
static int LoadSignatures(DetectEngineCtx *de_ctx, SCInstance *suri) {
  if (SigLoadSignatures(de_ctx, suri->sig_file, suri->sig_file_exclusive) < 0) {
    ...
    if (de_ctx->failure_fatal) return TM_ECODE_FAILED;
  }
...
static int ConfigGetCaptureValue(SCInstance *suri) {
  ...
  if (tmp_max_pending_packets < 1 || tmp_max_pending_packets >= UINT16_MAX) {
    SCLogError(
        "Maximum max-pending-packets setting is 65534 and must be greater than "
        "0. "
  ...
  if ((ConfGet("default-packet-size", &temp_default_packet_size)) != 1) {
    ...
    switch (suri->run_mode) {
      ...
      case RUNMODE_PFRING:
        ...
        if (default_packet_size) break;
        /* fall through */
      default:
        default_packet_size = DEFAULT_PACKET_SIZE;
    }
  } else {
    if (ParseSizeStringU32(temp_default_packet_size, &default_packet_size) <
        0) {
      SCLogError(
...
void PostConfLoadedDetectSetup(SCInstance *suri) {
  ...
  if (!suri->disabled_detect) {
    ...
    if (mt_enabled) (void)ConfGetBool("multi-detect.default", &default_tenant);
    if (DetectEngineMultiTenantSetup(suri->unix_socket_enabled) == -1) {
      FatalError(
          "initializing multi-detect "
    ...
    } else if (mt_enabled && !default_tenant &&
    ...
    if (de_ctx->type == DETECT_ENGINE_TYPE_NORMAL) {
      if (LoadSignatures(de_ctx, suri) != TM_ECODE_OK) exit(EXIT_FAILURE);
    }
...
int PostConfLoadedSetup(SCInstance *suri) {
  ...
  RunModeEngineIsIPS(suricata.run_mode, suricata.runmode_custom_mode,
                     suricata.capture_plugin_name);
  ...
  if (ConfGet("umask", &custom_umask) == 1) {
    uint16_t mask;
    if (StringParseUint16(&mask, 8, (uint16_t)strlen(custom_umask),
                          custom_umask) > 0) {
      umask((mode_t)mask);
    }
  }
...
#ifdef NFQ
  if (suri->run_mode == RUNMODE_NFQ) NFQInitConfig(false);
#endif
  ...
  if (DetectAddressTestConfVars() < 0) {
    SCLogError("basic address vars test failed. Please check %s for errors",
               suri->conf_filename);
    SCReturnInt(TM_ECODE_FAILED);
  }
  if (DetectPortTestConfVars() < 0) {
    SCLogError("basic port vars test failed. Please check %s for errors",
               suri->conf_filename);
    SCReturnInt(TM_ECODE_FAILED);
  }
  ...
  if (MayDaemonize(suri) != TM_ECODE_OK) SCReturnInt(TM_ECODE_FAILED);
...
int SuricataMain(int argc, char **argv) {
   ...
   * logging module. */
  SCLogLoadConfig(suricata.daemon, suricata.verbose, suricata.userid,
                  suricata.groupid);
  ...
  if (ParseInterfacesList(suricata.aux_run_mode, suricata.pcap_dev) !=
      TM_ECODE_OK) {
    exit(EXIT_FAILURE);
  }
  ...
  if (limit_nproc) {
    SCLogWarning(
        "\"security.limit-noproc\" (setrlimit()) not set when using address "
        "sanitizer");
    limit_nproc = 0;
  }
#endif
  ...
  /* Must ensure all threads are fully operational before continuing with init
   * process */
