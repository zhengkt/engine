...
again:
  ...
  if ((cur_ts.tv_sec - start_ts.tv_sec) > 60) {
    FatalError(
        "unable to get all flow manager "
 ...
 *  \retval 1 timed out
 */
static int FlowManagerFlowTimeout(Flow *f, SCTime_t ts, uint32_t *next_ts,
                                  const bool emerg) {
  uint32_t flow_times_out_at = f->timeout_at;
  if (emerg) {
    extern FlowProtoTimeout flow_timeouts_delta[FLOW_PROTO_MAX];
    flow_times_out_at -= FlowGetFlowTimeoutDirect(flow_timeouts_delta,
 ...
 */
static inline int FlowBypassedTimeout(Flow *f, SCTime_t ts,
...
static uint32_t ProcessAsideQueue(FlowManagerTimeoutThread *td,
                                  FlowTimeoutCounters *counters) {
 ...
 */
static void FlowManagerHashRowTimeout(FlowManagerTimeoutThread *td, Flow *f,
                                      SCTime_t ts, int emergency,
                                      FlowTimeoutCounters *counters,
                                      uint32_t *next_ts) {
  uint32_t checked = 0;
  Flow *prev_f = NULL;
  ...
  if (checked > counters->rows_maxlen) counters->rows_maxlen = checked;
}
 ...
 */
static uint32_t FlowTimeoutHash(FlowManagerTimeoutThread *td, SCTime_t ts,
                                const uint32_t hash_min,
...
#define TYPE uint32_t
  ...
  for (uint32_t idx = hash_min; idx < hash_max; idx += BITS) {
    ...
    for (uint32_t i = 0; i < check; i++) {
      ...
      check_bits |=
          (TYPE)(SC_ATOMIC_LOAD_EXPLICIT(
                     fb->next_ts, SC_ATOMIC_MEMORY_ORDER_RELAXED) <= ts_secs)
          << (TYPE)i;
    }
    if (check_bits == 0) continue;
    ...
    for (uint32_t i = 0; i < check; i++) {
      ...
      if ((check_bits & ((TYPE)1 << (TYPE)i)) != 0 &&
        ...
        if (fb->evicted != NULL || fb->head != NULL) {
          ...
          if (fb->head != NULL) {
            uint32_t next_ts = 0;
            FlowManagerHashRowTimeout(td, fb->head, ts, emergency, counters,
                                      &next_ts);
...
static uint32_t FlowTimeoutHashInChunks(FlowManagerTimeoutThread *td,
                                        SCTime_t ts, const uint32_t hash_min,
                                        const uint32_t hash_max,
                                        FlowTimeoutCounters *counters,
 ...
 */
static uint32_t FlowManagerHashRowCleanup(Flow *f, FlowQueuePrivate *recycle_q,
                                          const int mode) {
  uint32_t cnt = 0;
...
static void FlowCountersInit(ThreadVars *t, FlowCounters *fc) {
  ...
  fc->flow_emerg_mode_enter =
      StatsRegisterCounter("flow.emerg_mode_entered", t);
  ...
  fc->flow_mgr_flows_checked =
      StatsRegisterCounter("flow.mgr.flows_checked", t);
  fc->flow_mgr_flows_notimeout =
      StatsRegisterCounter("flow.mgr.flows_notimeout", t);
  fc->flow_mgr_flows_timeout =
      StatsRegisterCounter("flow.mgr.flows_timeout", t);
...
static void FlowCountersUpdate(ThreadVars *th_v,
                               const FlowManagerThreadData *ftd,
                               const FlowTimeoutCounters *counters) {
  StatsAddUI64(th_v, ftd->cnt.flow_mgr_flows_checked,
               (uint64_t)counters->flows_checked);
  StatsAddUI64(th_v, ftd->cnt.flow_mgr_flows_notimeout,
               (uint64_t)counters->flows_notimeout);

  StatsAddUI64(th_v, ftd->cnt.flow_mgr_flows_timeout,
               (uint64_t)counters->flows_timeout);
  StatsAddUI64(th_v, ftd->cnt.flow_mgr_flows_aside,
               (uint64_t)counters->flows_aside);
  StatsAddUI64(th_v, ftd->cnt.flow_mgr_flows_aside_needs_work,
               (uint64_t)counters->flows_aside_needs_work);

  StatsAddUI64(th_v, ftd->cnt.flow_bypassed_cnt_clo,
               (uint64_t)counters->bypassed_count);
  StatsAddUI64(th_v, ftd->cnt.flow_bypassed_pkts,
               (uint64_t)counters->bypassed_pkts);
  StatsAddUI64(th_v, ftd->cnt.flow_bypassed_bytes,
               (uint64_t)counters->bypassed_bytes);

  StatsSetUI64(th_v, ftd->cnt.flow_mgr_rows_maxlen,
               (uint64_t)counters->rows_maxlen);
}

static TmEcode FlowManagerThreadInit(ThreadVars *t, const void *initdata,
                                     void **data) {
 ...
 */
static void GetWorkUnitSizing(const uint32_t rows, const uint32_t mp,
                              const bool emergency, uint64_t *wu_sleep,
                              uint32_t *wu_rows, uint32_t *rows_sec) {
  if (emergency) {
    *wu_rows = rows;
    *wu_sleep = 250;
    return;
  ...
  const uint32_t rows_per_sec =
      (uint32_t)((float)rows * (float)((float)emp / (float)100));
  ...
  SCLogDebug("mp %u emp %u rows %u rows_sec %u sleep %ums", mp, emp, rows,
             rows_per_sec, sleep_per_unit);
...
static TmEcode FlowManager(ThreadVars *th_v, void *thread_data) {
  ...
  while (!TimeModeIsReady()) {
    if (suricata_ctl_flags != 0) return TM_ECODE_OK;
    usleep(10);
  }
  ...
  while (1) {
    ...
    if (ts_ms >= next_run_ms) {
      ...
      if (emerg) {
        ...
        SCLogDebug("hash %u:%u slice starting at %u with %u rows", ftd->min,
                   ftd->max, pos, rows_per_wu);
      ...
      GetWorkUnitSizing(rows, mp, emerg, &sleep_per_wu, &rows_per_wu,
                        &rows_sec);
      if (pmp != mp) {
        StatsSetUI64(th_v, ftd->cnt.flow_mgr_rows_sec, rows_sec);
      }
    ...
    SCLogDebug("woke up... %s",
               SC_ATOMIC_GET(flow_flags) & FLOW_EMERGENCY ? "emergency" : "");
...
void FlowManagerThreadSpawn(void) {
  ...
  for (uint32_t u = 0; u < flowmgr_number; u++) {
    ...
    ThreadVars *tv_flowmgr =
        TmThreadCreateMgmtThreadByName(name, "FlowManager", 0);
...
static TmEcode FlowRecyclerThreadInit(ThreadVars *t, const void *initdata,
                                      void **data) {
  FlowRecyclerThreadData *ftd = SCCalloc(1, sizeof(FlowRecyclerThreadData));
  if (ftd == NULL) return TM_ECODE_FAILED;
  if (OutputFlowLogThreadInit(t, NULL, &ftd->output_thread_data) !=
      TM_ECODE_OK) {
  ...
  ftd->counter_queue_avg =
      StatsRegisterAvgCounter("flow.recycler.queue_avg", t);
  ftd->counter_queue_max =
      StatsRegisterMaxCounter("flow.recycler.queue_max", t);

  ftd->counter_flow_active = StatsRegisterCounter("flow.active", t);
  ftd->counter_tcp_active_sessions =
      StatsRegisterCounter("tcp.active_sessions", t);
...
void FlowRecyclerThreadSpawn(void) {
  ...
  for (uint32_t u = 0; u < flowrec_number; u++) {
    ...
    ThreadVars *tv_flowrec =
        TmThreadCreateMgmtThreadByName(name, "FlowRecycler", 0);
