...
static int JsonSmtpLogger(ThreadVars *tv, void *thread_data, const Packet *p,
  ...
  JsonBuilder *jb = CreateEveHeaderWithTxId(p, LOG_DIR_FLOW, "smtp", NULL,
                                            tx_id, jhl->emaillog_ctx->eve_ctx);
  if (unlikely(jb == NULL)) return TM_ECODE_OK;
...
bool EveSMTPAddMetadata(const Flow *f, uint64_t tx_id, JsonBuilder *js) {
  ...
  if (smtp_state) {
    SMTPTransaction *tx =
        AppLayerParserGetTx(IPPROTO_TCP, ALPROTO_SMTP, smtp_state, tx_id);
    if (tx) {
...
static OutputInitResult OutputSmtpLogInitSub(ConfNode *conf,
                                             OutputCtx *parent_ctx) {
  OutputInitResult result = {NULL, false};
  OutputJsonCtx *ojc = parent_ctx->data;

  OutputJsonEmailCtx *email_ctx = SCMalloc(sizeof(OutputJsonEmailCtx));
  if (unlikely(email_ctx == NULL)) return result;
...
static TmEcode JsonSmtpLogThreadInit(ThreadVars *t, const void *initdata,
                                     void **data) {
...
void JsonSmtpLogRegister(void) {
  ...
  OutputRegisterTxSubModule(LOGGER_JSON_TX, "eve-log", "JsonSmtpLog",
                            "eve-log.smtp", OutputSmtpLogInitSub, ALPROTO_SMTP,
                            JsonSmtpLogger, JsonSmtpLogThreadInit,
