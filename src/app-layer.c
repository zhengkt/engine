...
struct AppLayerThreadCtx_ {
  /* App layer protocol detection thread context, from
...
static inline int ProtoDetectDone(const Flow *f, const TcpSession *ssn,
                                  uint8_t direction) {
...
void AppLayerIncParserErrorCounter(ThreadVars *tv, Flow *f) {
  const uint16_t id =
      applayer_counters[f->protomap][f->alproto].parser_error_id;
  if (likely(tv && id > 0)) {
    StatsIncr(tv, id);
  }
}
...
void AppLayerIncInternalErrorCounter(ThreadVars *tv, Flow *f) {
  const uint16_t id =
      applayer_counters[f->protomap][f->alproto].internal_error_id;
  if (likely(tv && id > 0)) {
    StatsIncr(tv, id);
  }
}
...
static void DisableAppLayer(ThreadVars *tv, Flow *f, Packet *p) {
  SCLogDebug("disable app layer for flow %p alproto %u ts %u tc %u", f,
  ...
  SCLogDebug("disabled app layer for flow %p alproto %u ts %u tc %u", f,
             f->alproto, f->alproto_ts, f->alproto_tc);
}
 ...
 */
static void TCPProtoDetectCheckBailConditions(ThreadVars *tv, Flow *f,
...
static int TCPProtoDetectTriggerOpposingSide(ThreadVars *tv,
  ...
  enum StreamUpdateDir dir =
      StreamTcpInlineMode() ? UPDATE_DIR_OPPOSING : UPDATE_DIR_PACKET;
  int ret =
      StreamTcpReassembleAppLayer(tv, ra_ctx, ssn, opposing_stream, p, dir);
  return ret;
}
...
static int TCPProtoDetect(ThreadVars *tv, TcpReassemblyThreadCtx *ra_ctx,
                          AppLayerThreadCtx *app_tctx, Packet *p, Flow *f,
                          TcpSession *ssn, TcpStream **stream, uint8_t *data,
  ...
  *alproto = AppLayerProtoDetectGetProto(app_tctx->alpd_tctx, f, data, data_len,
  ...
  if (*alproto != ALPROTO_UNKNOWN) {
    ...
    if ((ssn->data_first_seen_dir & (STREAM_TOSERVER | STREAM_TOCLIENT)) &&
      ...
      if (FlowChangeProto(f)) {
        /* We have the first data which requested a protocol change from P1 to
         * P2 even if it was not recognized at first as being P1 As the second
         * data was recognized as P1, the protocol did not change !
         */
    ...
    int r = AppLayerParserParse(tv, app_tctx->alp_tctx, f, f->alproto, flags,
                                data, data_len);
    ...
    if ((ssn->flags & STREAMTCP_FLAG_MIDSTREAM) &&
      ...
      if (FLOW_IS_PM_DONE(f, STREAM_TOSERVER) &&
          FLOW_IS_PP_DONE(f, STREAM_TOSERVER)) {
    ...
    if (*alproto_otherdir != ALPROTO_UNKNOWN) {
      uint8_t first_data_dir;
      first_data_dir =
          AppLayerParserGetFirstDataDir(f->proto, *alproto_otherdir);
  ...
  SCReturnInt(0);
parser_error:
  ExceptionPolicyApply(p, g_applayerparser_error_policy,
...
int AppLayerHandleTCPData(ThreadVars *tv, TcpReassemblyThreadCtx *ra_ctx,
  ...
  if (flags & STREAM_GAP) {
    ...
    PACKET_PROFILING_APP_START(app_tctx, f->alproto);
    r = AppLayerParserParse(tv, app_tctx->alp_tctx, f, f->alproto, flags, data,
                            data_len);
    PACKET_PROFILING_APP_END(app_tctx, f->alproto);
    p->app_update_direction = (uint8_t)dir;
  ...
  if (alproto == ALPROTO_UNKNOWN && (flags & STREAM_START)) {
    ...
    if (TCPProtoDetect(tv, ra_ctx, app_tctx, p, f, ssn, stream, data, data_len,
                       flags, dir) != 0) {
      goto failure;
    }
    ...
    // we delay AppLayerParserStateCleanup because we may need previous parser
    // state
    ...
    int rd = TCPProtoDetect(tv, ra_ctx, app_tctx, p, f, ssn, stream, data,
                            data_len, flags, dir);
    if (f->alproto == ALPROTO_UNKNOWN) {
      ...
      FlowUnsetChangeProtoFlag(f);
      AppLayerParserStateProtoCleanup(f->protomap, f->alproto_orig,
                                      alstate_orig, alparser);
    ...
    SCLogDebug("stream data (len %" PRIu32
               " alproto "
               "%" PRIu16 " (flow %p)",
    ...
    if (f->alproto != ALPROTO_UNKNOWN) {
      PACKET_PROFILING_APP_START(app_tctx, f->alproto);
      r = AppLayerParserParse(tv, app_tctx->alp_tctx, f, f->alproto, flags,
                              data, data_len);
      PACKET_PROFILING_APP_END(app_tctx, f->alproto);
      p->app_update_direction = (uint8_t)dir;
      if (r != 1) {
 ...
 *  \retval -1 error
 */
int AppLayerHandleUdp(ThreadVars *tv, AppLayerThreadCtx *tctx, Packet *p,
                      Flow *f) {
  SCEnter();
  AppProto *alproto;
  AppProto *alproto_otherdir;
  ...
  if (*alproto == ALPROTO_UNKNOWN) {
    ...
    *alproto = AppLayerProtoDetectGetProto(tctx->alpd_tctx, f, p->payload,
                                           p->payload_len, IPPROTO_UDP, flags,
                                           &reverse_flow);
    ...
    switch (*alproto) {
      ...
      default:
        if (*alproto_otherdir != ALPROTO_UNKNOWN &&
            *alproto_otherdir != ALPROTO_FAILED) {
          if (*alproto_otherdir != *alproto) {
            AppLayerDecoderEventsSetEventRaw(
                &p->app_layer_events,
                APPLAYER_MISMATCH_PROTOCOL_BOTH_DIRECTIONS);
            // data already sent to parser, we cannot change the protocol to use
    ...
    if (f->alproto != ALPROTO_UNKNOWN && f->alproto != ALPROTO_FAILED) {
      ...
      r = AppLayerParserParse(tv, tctx->alp_tctx, f, f->alproto, flags,
...
void AppLayerListSupportedProtocols(void) {
  ...
  for (alproto = 0; alproto < ALPROTO_MAX; alproto++) {
    if (alprotos[alproto] == 1) printf("%s\n", AppLayerGetProtoName(alproto));
  }
...
AppLayerThreadCtx *AppLayerGetCtxThread(ThreadVars *tv) {
  ...
  if (app_tctx == NULL) goto error;
  memset(app_tctx, 0, sizeof(*app_tctx));

  if ((app_tctx->alpd_tctx = AppLayerProtoDetectGetCtxThread()) == NULL)
    goto error;
  if ((app_tctx->alp_tctx = AppLayerParserThreadCtxAlloc()) == NULL) goto error;
...
void AppLayerSetupCounters(void) {
  ...
  for (uint8_t p = 0; p < IPPROTOS_MAX; p++) {
    ...
    const uint8_t other_ipproto =
        ipproto == IPPROTO_TCP ? IPPROTO_UDP : IPPROTO_TCP;
      ...
      if (alprotos[alproto] == 1) {
        applayer_counters[ipproto_map][alproto].counter_id =
            StatsRegisterCounter(
                applayer_counter_names[ipproto_map][alproto].name, tv);
...
static int AppLayerTest01(void) {
  ...
  uint8_t request[] = {
      ...
      0x2e, 0x30, 0x0d, 0x0a, 0x48, 0x6f, 0x73, 0x74, 0x3a, 0x20, 0x6c,
      0x6f, 0x63, 0x61, 0x6c, 0x68, 0x6f, 0x73, 0x74, 0x0d, 0x0a, 0x55,
      0x73, 0x65, 0x72, 0x2d, 0x41, 0x67, 0x65, 0x6e, 0x74, 0x3a, 0x20,
      0x41, 0x70, 0x61, 0x63, 0x68, 0x65, 0x42, 0x65, 0x6e, 0x63, 0x68,
      0x2f, 0x32, 0x2e, 0x33, 0x0d, 0x0a, 0x41, 0x63, 0x63, 0x65, 0x70,
      0x74, 0x3a, 0x20, 0x2a, 0x2f, 0x2a, 0x0d, 0x0a, 0x0d, 0x0a};
  p->tcph->th_ack = htonl(1);
  p->tcph->th_seq = htonl(1);
...
static int AppLayerTest03(void) {
  ...
  /* rubbish response */
  uint8_t response[] = {
      0x58, 0x54, 0x54, 0x50, 0x2f, 0x31, 0x2e, 0x31, 0x20, 0x32, 0x30, 0x30,
      0x20, 0x4f, 0x4b, 0x0d, 0x0a, 0x44, 0x61, 0x74, 0x65, 0x3a, 0x20, 0x46,
      0x72, 0x69, 0x2c, 0x20, 0x32, 0x33, 0x20, 0x53, 0x65, 0x70, 0x20, 0x32,
      0x30, 0x31, 0x31, 0x20, 0x30, 0x36, 0x3a, 0x32, 0x39, 0x3a, 0x33, 0x39,
      0x20, 0x47, 0x4d, 0x54, 0x0d, 0x0a, 0x53, 0x65, 0x72, 0x76, 0x65, 0x72,
...
static int AppLayerTest04(void) {
  ...
  FAIL_IF(ssn->data_first_seen_dir !=
          STREAM_TOSERVER);  // TOSERVER data now seen
  ...
  uint8_t response2[] = {
      ...
      ,
...
static int AppLayerTest05(void) {
  ...
  /* full request */
  uint8_t request[] = {
      0x48, 0x45, 0x54, 0x20, 0x2f, 0x69, 0x6e, 0x64, 0x65, 0x78, 0x2e,
      0x68, 0x74, 0x6d, 0x6c, 0x20, 0x48, 0x54, 0x54, 0x50, 0x2f, 0x31,
      0x2e, 0x30, 0x0d, 0x0a, 0x48, 0x6f, 0x73, 0x74, 0x3a, 0x20, 0x6c,
      0x6f, 0x63, 0x61, 0x6c, 0x68, 0x6f, 0x73, 0x74, 0x0d, 0x0a, 0x55,
      0x73, 0x65, 0x72, 0x2d, 0x41, 0x67, 0x65, 0x6e, 0x74, 0x3a, 0x20,
...
static int AppLayerTest07(void) {
  ...
  /* full response - request ack */
  uint8_t response[] = {
      0x05, 0x00, 0x4d, 0x42, 0x00, 0x01, 0x2e, 0x31, 0x20, 0x32, 0x30, 0x30,
      0x20, 0x4f, 0x4b, 0x0d, 0x0a, 0x44, 0x61, 0x74, 0x65, 0x3a, 0x20, 0x46,
      0x72, 0x69, 0x2c, 0x20, 0x32, 0x33, 0x20, 0x53, 0x65, 0x70, 0x20, 0x32,
      0x30, 0x31, 0x31, 0x20, 0x30, 0x36, 0x3a, 0x32, 0x39, 0x3a, 0x33, 0x39,
      0x20, 0x47, 0x4d, 0x54, 0x0d, 0x0a, 0x53, 0x65, 0x72, 0x76, 0x65, 0x72,
...
static int AppLayerTest08(void) {
  ...
  /* full request */
  uint8_t request[] = {
      0x05, 0x00, 0x54, 0x20, 0x00, 0x01, 0x6e, 0x64, 0x65, 0x78, 0x2e,
      0x68, 0x74, 0x6d, 0x6c, 0x20, 0x48, 0x54, 0x54, 0x50, 0x2f, 0x31,
      0x2e, 0x30, 0x0d, 0x0a, 0x48, 0x6f, 0x73, 0x74, 0x3a, 0x20, 0x6c,
      0x6f, 0x63, 0x61, 0x6c, 0x68, 0x6f, 0x73, 0x74, 0x0d, 0x0a, 0x55,
      0x73, 0x65, 0x72, 0x2d, 0x41, 0x67, 0x65, 0x6e, 0x74, 0x3a, 0x20,
...
static int AppLayerTest09(void) {
  ...
  /* full response - request ack */
  uint8_t response[] = {
      0x55, 0x74, 0x54, 0x50, 0x2f, 0x31, 0x2e, 0x31, 0x20, 0x32, 0x30, 0x30,
      0x20, 0x4f, 0x4b, 0x0d, 0x0a, 0x44, 0x61, 0x74, 0x65, 0x3a, 0x20, 0x46,
      0x72, 0x69, 0x2c, 0x20, 0x32, 0x33, 0x20, 0x53, 0x65, 0x70, 0x20, 0x32,
      0x30, 0x31, 0x31, 0x20, 0x30, 0x36, 0x3a, 0x32, 0x39, 0x3a, 0x33, 0x39,
      0x20, 0x47, 0x4d, 0x54, 0x0d, 0x0a, 0x53, 0x65, 0x72, 0x76, 0x65, 0x72,
...
static int AppLayerTest10(void) {
  ...
  /* full request */
  uint8_t request1[] = {0x47, 0x47, 0x49, 0x20, 0x2f, 0x69, 0x6e, 0x64, 0x47,
                        0x47, 0x49, 0x20, 0x2f, 0x69, 0x6e, 0x64, 0xff};
  p->tcph->th_ack = htonl(1);
  p->tcph->th_seq = htonl(1);
  p->tcph->th_flags = TH_PUSH | TH_ACK;
  p->flowflags = FLOW_PKT_TOSERVER;
